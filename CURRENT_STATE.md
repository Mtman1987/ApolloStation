...
Chat Gateway now exists as a bounded Green service with a normalized Twitch/Discord/Kick message contract, durable provider-message dedupe, per-consumer delivery and retry, tenant isolation, canonical SPMT identity when linked, provider-scoped fallback identity when unlinked, and provider-neutral egress. Its deterministic connection supervisor durably leases provider configurations across workers, obtains only ephemeral SPMT grants, resumes persisted cursors, pauses reauthorization failures, and applies donor-aligned Twitch/Discord/Kick reconnect backoff. Chat Tag and StreamWeaver are real consumers. Concrete provider protocol/socket drivers, Commlink consumption, and external/live sandbox verification remain incomplete.

StreamWeaver now has the first normalized-chat persona-routing slice. Tenant-specific configuration selects the persona, aliases, owner identity, and home channels; the owner can casually summon that persona into another channel for an exact durable ten-minute window, while other users require an explicit mention and bots cannot re-enter the loop. Accepted work uses a stable delivery-derived job identity and produces no fake success text; unavailable execution is reported truthfully through gateway egress. Tenant configuration migration, real Stellar job/result streaming, provider I/O, commands, actions, redeems, TTS, overlays, games, research, and Companion separation remain active parity work.
