...
Chat Gateway now exists as a bounded Green service with a normalized Twitch/Discord/Kick message contract, durable provider-message dedupe, per-consumer delivery and retry, tenant isolation, canonical SPMT identity when linked, provider-scoped fallback identity when unlinked, and provider-neutral egress. Its deterministic connection supervisor durably leases provider configurations across workers, obtains only ephemeral SPMT grants, resumes persisted cursors, and applies donor-aligned Twitch/Discord/Kick reconnect backoff. SPMT now owns an AES-256-GCM encrypted provider credential authority, officially shaped Twitch/Discord/Kick OAuth refresh adapters, rotated-refresh-token persistence, cross-worker refresh fencing, scoped grant/recovery endpoints, replace-only bot-token rotation, explicit reauthorization state, and a one-way non-overwriting donor import. An active socket authentication rejection performs one fenced recovery before the account is paused. Concrete Twitch IRC, Discord Gateway/REST, and Kick Pusher/REST drivers are implemented, and Nebula Arcade, StreamWeaver, and Commlink are real consumers. Production credentials remain disabled; the controlled two-tenant live-socket rehearsal, data reconciliation, and external-provider acceptance are cutover gates.
