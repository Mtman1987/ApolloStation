...
The active ApolloStation parity branch now contains the first donor-backed product implementation slices. The original Chat Tag core was audited against `Mtman1987/chat-tag` commit `8170c51` and implemented inside the bounded Nebula Arcade module.
...
Chat Gateway now exists as a bounded Green service with a normalized Twitch/Discord/Kick message contract, durable provider-message dedupe, per-consumer delivery and retry, tenant isolation, canonical SPMT identity when linked, provider-scoped fallback identity when unlinked, and provider-neutral egress. Its deterministic connection supervisor durably leases provider configurations across workers, obtains only ephemeral SPMT grants, resumes persisted cursors, pauses reauthorization failures, and applies donor-aligned Twitch/Discord/Kick reconnect backoff. Concrete Twitch IRC, Discord Gateway/REST, and Kick Pusher/REST drivers are implemented, and Chat Tag, StreamWeaver, and Commlink are real consumers. Production OAuth-refresh authority, credentialed multi-tenant socket rehearsal, and external/live-provider verification remain incomplete.

StreamWeaver now has a complete normalized-chat-to-Stellar result loop behind the shadow provider route. Tenant-specific configuration selects the persona presentation, aliases, owner identity, and home channels; the owner can casually summon that presentation into another channel for an exact durable ten-minute window, while other users require an explicit mention and bots cannot re-enter the loop. The SPMT adapter creates a metered Stellar job with stable delivery-derived identity, and an app-private SQLite outbox survives restart while reconciling queued/running/terminal job state into exactly one logical provider-neutral reply. Only the authenticated requesting service may fetch its known cross-owner job; it cannot enumerate Stellar jobs. Unavailable, cancelled, exhausted-retry, invalid-result, and provider-send states remain truthful and credential-redacted. Tenant persona-instruction and memory-policy migration, live provider I/O, commands, actions, redeems, TTS, overlays, games, research, and full Companion separation remain active parity work.

Chat Tag now includes a controls-free visual browser source mounted behind a verified SPMT output principal. The renderer includes donor-equivalent one-second polling, current-it/free-for-all status, tag/new-it/free-for-all broadcasts, leaderboard/history cycling, generated tones, confetti, crown-decorated text, transparent output, strict no-inline-script CSP, and no tenant/provider credential in its URL. A standalone Green sandbox host exposes that renderer and a same-origin gameplay console for the private Sprite test while provider egress is disabled. SPMT can issue a one-time opaque browser-source URL and resolve it internally to the Chat Tag principal; deploying that resolver in the production output gateway and recording a real OBS capture remain production gates.
...
MountainView and Companion now include their first donor-backed cross-app slice. MountainView plans common voice intents without direct donor URLs: community-wide live status goes to DSH, Chat Tag activity to Nebula Arcade, music to HearMeOut, stream actions to StreamWeaver, and OBS scenes only to a paired local Companion. SPMT is the sole pairing/revocation authority; Companion rejects wrong-source, cross-tenant, revoked, unknown, and over-capability commands, dedupes completed local work, and retains redacted retry evidence for temporary adapter failures. The mobile/BLE/camera/RDGlass bridge, complete command/profile/visual/QR catalog, public device APIs, desktop relay connection, real OBS adapter, local media/FFmpeg jobs, and installer remain active parity work.

The complete production donor baseline was refreshed on 2026-08-23 in `docs/donor-audits/PRODUCTION_REPO_BASELINES_2026-08-23.md`. Six flagship product manifests are present, but a manifest alone is scaffolding rather than a ported app. The original Chat Tag app is the first product to reach the private Sprite sandbox gate; its production provider/output/import evidence and every broader Nebula Arcade module, product, and worker remain active rebuild work.
