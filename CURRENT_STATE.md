...
The repository currently passes 84 tests. This includes the two-player, two-tenant, restart, replay, immunity, pass, moderator, SPMT contract, and DSH projection cases plus the first audited SPMT identity-parity slice: active provider links can be listed and unlinked through the same human-only API/SDK/CLI/MCP contract, revocations remain durable across restart, and service identities cannot manage a person's linked accounts. Chat Tag provider ingress, timers, overlay rendering, crowns, live state migration, and the remaining SPMT capability families remain explicit child slices; Blue production remains authoritative.
