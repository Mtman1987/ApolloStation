...
4. `spmt join` and `spmt leave` operate on the persistent player pool across chats. Game-specific exclusions are retained. Other ambiguous commands ask for `spmt N`, with the choice retained for 30 seconds across restarts. Safe team-color commands can fan out to both compatible games.
...
Automatic Tag rotation uses fresh Twitch presence from Chat Gateway. A failed or stale poll never becomes an empty live-user list; the runtime reports `presence-required` when it has no usable observation. Optional live-player channel following is explicitly configured. Durable announcement and media queues retry independently of gameplay receipts. Production activation still requires the controlled two-tenant provider rehearsal, canonical presence projection, donor-state reconciliation, deployed overlay proof, and owner acceptance.
...
See [the release runbook](../NEBULA_RELEASE.md) for the authenticated web host, shared database, media workers, live import and production preflight.
