...
| 12. Check-ins | Partial | Explicit owner Discord role import/refresh, invitation overrides and durable retry outcomes implemented. Reward-backed web/chat/Twitch-menu check-ins now reuse local/XP settlement and first-claim policies. AI/fallback greetings, public speech and optional configured Discord delivery now use durable presentation tasks. Bulk rides/front-seat selection and bonuses remain. |
...
- Check-in presentation follow-up: repository build and all 928 offline tests passed, including atomic queueing, stable retries, AI job ownership, fallback, owner controls and inactive-operation suppression. Promotion remains to be verified.
