...
- Pokémon completion follow-up: repository build and all 904 offline tests passed after preserving the concurrent AI draft-recovery change. Commit `8422988f610b3b01df6f025236bb72d61396d409` passed Green Contracts. Sprite promotion initially failed on an occupied ephemeral test port; failed-job retry is being verified.
- Discord role-import and check-in retry follow-up: repository build and all 908 offline tests passed. Deployment of this follow-up remains to be verified.
...
| 12. Check-ins | Partial | Explicit owner Discord role import/refresh, invitation overrides and durable retry outcomes implemented. Bulk rides/front-seat selection and bonuses, greeting and reward-flow details remain. |
...
| 30 | Check-ins | Persistent check-ins, source/count displays and overlay implemented. Owner-scoped Discord role import/refresh and invitation overrides now available; bulk rides/front-seat bonuses, greetings and reward-flow details remain. |
