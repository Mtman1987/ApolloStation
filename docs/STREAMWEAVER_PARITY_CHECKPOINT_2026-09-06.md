...
| 13 | Image providers | SeaArt, Eden and Cloudflare FLUX.1 Schnell production providers and fallback are constructed. Binary images use managed private/public media storage. Other Cloudflare models, Perchance and Pollinations remain; this is not five-provider parity. |
...
- Cloudflare/binary-image follow-up: repository build and all 943 offline tests passed, including bounded provider output, private/public media isolation, fenced upload/publication, generation reuse, canonical simulation policy and independent catalog refresh. Promotion remains to be verified.
