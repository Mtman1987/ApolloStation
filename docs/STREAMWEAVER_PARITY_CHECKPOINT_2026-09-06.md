...
| 5. Image Studio completion | Partial | SeaArt/Eden generation and worker-backed Eden model discovery/selection implemented. Separate public/private prompt checking is now enforced at the worker. Cloudflare FLUX.1 Schnell now has managed private/public binary delivery. SeaArt catalog/LoRA browsing, character workflow, additional Cloudflare models and Perchance remain. Pollinations now has authenticated generation and image-model discovery. |
...
| 13 | Image providers | SeaArt, Eden, Cloudflare FLUX.1 Schnell and Pollinations production providers and fallback are constructed. Binary images use managed private/public media storage. Other Cloudflare models and Perchance remain; this is not five-provider parity. |
...
- Pollinations follow-up: repository build and all 947 offline tests passed, including model filtering, private model selection through the worker, server-only authentication, binary validation and inactive/sandbox configuration boundaries. Promotion remains to be verified.
