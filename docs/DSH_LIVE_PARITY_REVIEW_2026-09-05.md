...
| Web calendar and event maintenance | Month grid and CRUD restored here. Full visual avatar treatment and some live conveniences remain smaller than the original. | Members must see which day is claimed and owners must be able to move or remove a mission. | Keep this DSH calendar and store; bring across useful visual cues and accessibility. |
| Interactive Discord calendar and mission log | Apollo currently publishes a text summary. Image calendar, second mission-log embed and month/claim/mission buttons remain missing. | A member should claim a day from Discord and see the same changed event in Apollo and the room preview. | Port renderer, components and real interaction handlers together. A button payload alone is insufficient. |
