...
Validation: all 1,106 offline tests pass after the broadcast changes. Local real-ffmpeg HTTP integration confirms one encoder across repeated window reads, continued operation with no members, lease fencing, access revocation and deletion cleanup. A deterministic 210-second/180-second room-clock case passes. The required browser job now checks a real two-language central broadcast, independent local volume, zero-volume silence without pausing, zero windows and return to the live edge. GitHub browser testing exposed Chromium advertising native HLS but failing on the initial starting-feed response. The shared HLS loader is now preferred wherever supported, so its bounded startup retry applies. Checkpoint `46134cd1` passes required contracts and real RTC/HLS/media/console browser checks in run `34785842617` (RTC `103801046420`, contracts `103801046564`). The HLS check crosses the real common ingress and verifies two audio languages, normal playback speed, independent volume, continued output without windows and joining the current broadcast. A final guard also fences deletion/close after asynchronous cache-directory creation; all four focused broadcast tests pass after that guard.
...
Rotator access finding: `Mtman1987/fly-machine-rotator` currently exposes bounded Fly Machine observation/rotation, coding and allowlisted worker operations. Its source/configuration contains no Sprite control route, and this session exposes no Rotator connector tools. Its configured Fly organization is `mtman-new`; the release Sprite belongs to `testing-968`, so access must not be inferred. Apollo already uses environment-owned `SPRITES_TOKEN` in the existing Sprite promotion workflow. That saved deployment route is preferable to copying credentials, but the tools exposed in this session do not include workflow dispatch. No access gate, provider permission or production Activity mapping was changed.


### Approved broadcast test activation after PR #99

PR #99 merged at `821dc29ac8489cc31aaf48e481493a8f95e4190c`. Sprite promotion `34786497949` failed before switching the running release because the broadcast integration test required `/usr/bin/ffprobe`; the installer only provisioned FFmpeg. The continuation provisions FFprobe from the same SHA-256 pinned b6.1.1 release, resolves test binaries from the installed PATH, and provisions SHA-256 pinned yt-dlp 2026.08.19.

The owner approved finishing deployment and opening the isolated HMO/Discord test. Release activation reuses the transferred owner tenant, room database, voice configuration, service authentication and existing HMO execution jobs. Only the HMO media worker uses its production resolver with that one tenant and the Sprite execution target; the other app workers retain their sandbox settings. The broadcast proxy may open DNS-pinned public HTTP(S) media sockets, constrained further by the Sprite's existing DNS policy, extended with YouTube/media CDN domains. External chat posting remains disabled.

The release workflow opens URL authentication only after the deployed HMO broadcaster, assigned worker lease, Activity state and account/private-room boundaries pass through the real ingress. Failed public verification restores organization-only URL access. Review remains protected. The HMO configuration is backed up privately before activation and restored on failed deployment.

The existing HMO public application ID is `1279582181768957963`. Its production URL mapping remains unchanged. For the isolated test use Discord Developer Mode's Application URL Override `https://web-terminal-bvesa.sprites.app/activity?sessionId=discord-watch-room`, and join the **Discord Activity** room at `/apps/hearmeout` in the same Apollo owner workspace. This local developer override does not test Discord's production proxy; a separate development application mapping is required for that acceptance check. Live HearMeOut stays running.

Deployment success and an actually playable external request still need to be recorded after this change reaches the Sprite. This entry does not claim test readiness or complete HMO/DSH parity.

### Existing media preparation feeding the room broadcast — 14 September

The owner asked to reuse the working live HMO media-loading path as the input to one isolated Apollo broadcaster. The live worker already exposes authenticated prepared YouTube HLS, with its existing source preparation and cache. Apollo's local adapter had only connected yt-dlp even though an upstream resolver hook already existed. The continuation connects that hook to the configured existing worker through HMO's existing tenant-scoped execution jobs. The room's queue and playback clock remain in `room-media-core`; no live DJ or voice-room control is issued by the media adapter.

The broadcaster consumes that prepared HLS through its loopback source proxy. The worker credential stays in the server-side adapter, outside FFmpeg arguments and public room state. Nested manifests retain their video ID and pinned worker machine; redirects, unrelated origins, private worker routes and cross-tenant sources are rejected. A private input capability prevents an unrelated source redirect from using the prepared-media proxy. Media-worker network access permits the required reads without enabling live DJ, voice or cache administration.

Validation: 1,112 offline tests passed before the final private-input capability guard; the focused real-video, activation and network tests also pass with that guard. A real FFmpeg integration consumes authenticated prepared HLS, produces one room encoder for multiple windows, and advances after all members leave. Actual live-worker/provider acceptance and public test access remain outstanding. This bridge does not claim Watch catalog/Xtream, uploads/downloads, remaining provider functions or full P09/P33 parity.

Deployment evidence: PRs #100–#103 merged. Worker readiness passed during promotion of #101, but no playable manifest was produced. The fixed external sample returned HTTP 403 in #102's preflight; its exact response body is still unknown because both #103 deployment attempts timed out during Git checkout before running diagnostics. The previously running release was preserved. PR #104 repairs repeated checkout downloads by reusing and dissociating existing Git objects while retaining the exact-SHA and deployment gates. The Sprite still requires organization login; live HMO and its Discord mapping have not been redirected or shut down.

Promotion of merged PR #104 reached the diagnostic successfully. Google returned an XML AccessDenied response: anonymous callers cannot read the old gtv-videos-bucket sample object (or it no longer exists). The connection and proxy worked; the selected fixture was not publicly accessible. The preflight and deployed check now share an unencrypted HLS sample listed in Shaka Player's maintained demo catalog, and movie acceptance requires a video variant.


## Continuation: prepared-media runtime repair — 14 September 2026

PR #105 merged as `c9ca1c96b77aade0761ac7eae646813c469dce52`. Sprite promotion run `34793648342` failed the new authenticated prepared-HLS integration test before release activation. Downloading and verifying the exact installed binaries reproduced the failure locally: both local-file and loopback HTTP MPEG-TS HLS made FFprobe exit with `SIGSEGV`, while system FFprobe read the same fixture. The `b6.1.1` assets actually identify as FFmpeg 7.0.2-static; the failure is independent of room permissions, provider authorization and Discord.

The replacement runtime uses BtbN's retained monthly `autobuild-2026-08-31-13-27`, release-branch build `n8.1.2-50-g1a748fe2cd`. Its archive and extracted FFmpeg/FFprobe binaries are SHA-256 pinned. FFmpeg's download page lists BtbN as a Linux build provider. A separate runtime directory preserves the previous installed binaries. All 1,112 offline tests pass with the replacement FFmpeg/FFprobe binaries, including authenticated prepared video, one encoder and continued playback after all room members leave. The fixture permits the broadcaster's existing 20-second source-probe budget and reports bounded, opt-in diagnostics only for its local test source.

The replacement Shaka HLS sample manifest returns HTTP success on a fresh public read. Sprite deployment, a real prepared-worker request, public ingress and Discord Activity acceptance still need verification; neither this runtime repair nor passing CI establishes test readiness. Live HMO and its production Discord mapping remain unchanged.
