...
| Commlink Mail | SPMT | SPMT + SpaceMountain UI | PRESERVE | Green SpaceMountain discovers same-tenant recipients, composes idempotent private mail, lists inbox/sent history, tracks each user's unread state, searches history, and replies through the public SPMT contract. Cross-user and cross-tenant reads fail closed; only production donor-record reconciliation remains | two-account compose/replay/conflict/inbox/sent/read/restart isolation test passes; donor migration rehearsal remains |
...
| Discord bot/chat bridge behavior | StreamWeaver | Chat Gateway provider I/O + StreamWeaver command/persona consumer | IMPROVE | The provider-neutral consumer rejects bot-authored messages, applies explicit mention and channel scope rules, and sends unavailable or terminal replies only through gateway egress. Concrete Discord/Twitch drivers and SPMT-owned OAuth/replace-only credential rotation exist; the controlled credentialed socket proof remains | Discord/Twitch cross-path, refresh/reconnect, loop, scope, result, and live-credential tests |
| Tenant bot dispatch | StreamWeaver | StreamWeaver | PRESERVE | A supervised production consumer now loads tenant-private persona/settings state, frozen-donor commands, local currency, and the restart-safe Stellar result outbox under a distinct least-privilege StreamWeaver service identity. Chat Gateway remains the sole provider socket/egress owner. Dispatch uses canonical identity when linked, a provider-scoped fallback otherwise, stable delivery-derived job identity, and rejects bot re-entry and foreign jobs | implemented authentication/routing/restart/result/retry/command/economy/loop cases pass; live-provider two-tenant delivery and donor data import remain |
| Tenant persona/personality/model requests (Athena for the owner) | StreamWeaver | StreamWeaver persona runtime + shared Stellar Core execution | IMPROVE | Configured persona routing preserves the owner's casual summon, requires explicit mentions from other users, allows the home channel without a summon, and persists the exact ten-minute external summon scope across restart. Versioned app-private settings and a non-overwriting one-way importer now carry tenant instructions and `off`/persona-scoped `conversation` memory. SPMT derives presentation ownership from the authenticated app, humans cannot inject a presentation, private instructions are redacted from their job/export views, and Stellar history matches tenant/user/source-app/persona/conversation. Athena remains this owner's configured presentation rather than a global default. Voice, research, and broader capability routing remain | routing, bot-loop, expiry, settings restart/import, accepted/unavailable, stable-job, requester-isolation, instruction redaction, memory-off, two-tenant distinct-persona, ordered-result, and retry tests pass; live provider/voice matrix remains |
...
| Shared normalized chat feed | StreamWeaver | provider-neutral Chat Gateway | IMPROVE | A supervised Chat Gateway normalizes Twitch/Discord/Kick messages, preserves canonical or provider-scoped actor identity, durably dedupes provider messages, queues per-consumer delivery, retries failures, isolates tenants, and feeds Nebula Arcade, StreamWeaver, and Commlink. Its production Commlink consumer writes only through the service-authenticated SPMT `/v1/commlink/live` contract; the browser can read its authorized tenant projection but cannot ingest or obtain provider credentials. A deterministic multi-worker supervisor leases provider configs, obtains ephemeral SPMT grants, resumes durable cursors, and applies donor-aligned reconnect backoff. The Green Sprite starts the real worker with an empty connection set and outbound access disabled. SPMT seals provider credentials, fences and rotates refresh tokens, supports replace-only bot credentials, and lets a rejected socket recover once before pausing for reauthorization. Controlled concurrent two-tenant live sockets and external-client proof remain | API tenant/service denial, durable projection/replay, supervised sandbox, dedupe/consumer/lease/cursor/backoff/refresh/reauthorization/driver tests pass; live-provider and external-client conformance remain |
...
| Donor surface/capability | Donor owner | Green owner | Disposition | Compatibility/migration | Proof required |
|---|---|---|---|---|---|
| Live stream monitoring | DSH | DSH module | PRESERVE | The durable tenant-isolated live projection is now composed into a separately authenticated supervised worker. Versioned public config supplies provider-account references and tracked canonical members; a non-overlapping ten-minute scheduler obtains ephemeral SPMT grants, batches Helix at 100 logins/request, rejects incomplete-poll mass-offline reconciliation, and drives the retryable Discord publisher. The production Sprite runs a zero-tenant, no-egress proof. Reconciled production config and live two-tenant provider acceptance remain | live/offline, restart/retry, Helix batching, incomplete-poll, scoped service identity, configuration safety, supervised cycle, and Sprite tests pass; live-provider proof remains |
| Shoutouts and routing groups | DSH | DSH module | PRESERVE | migrate group/routing rules | representative guild test |
| Spotlight rotation | DSH | DSH module | PRESERVE | The donor's ten-minute alphabetical all-group rotation, one-live-member behavior, current-member invalidation, offline clearing, durable cursor, poll replay, pinned Discord embed replacement, and tenant isolation are implemented and wired into the supervised cycle. Clip/GIF source migration and live Discord proof remain | core rotation/recovery/publisher tests pass; media/live-provider matrix remains |
...
| DJ/music requests/queue | HearMeOut + HMO worker | HearMeOut + elastic DJ/media worker | PRESERVE | The canonical room media authority gives music its own durable lane and idempotent queue. A separately authenticated supervised worker now claims fenced jobs for the donor catalog, recent-user cache and safe `yt-dlp` resolution, with zero-tenant Sprite rehearsal and no canonical worker state. TTS priority, auto-radio, announcements and live provider playback remain controlled-acceptance work | request→queue→play/replay/restart plus worker lease/catalog/cache/resolver/sandbox tests pass; live provider matrix remains |
...
## Nebula Arcade (historical ChatTag donor)

`ChatTag` is retained only as the historical donor-repository label. Under amended D-32, **Nebula Arcade** is the sole current app/service/integration owner. Tagging capability is the internal `tag` game and has no separate public product identity. No donor capability is removed merely because ownership and identifiers are corrected.

| Donor surface/capability | Donor owner | Green owner | Disposition | Compatibility/migration | Proof required |
|---|---|---|---|---|---|
| Tagging game behavior | historical game donor | Nebula Arcade `tag` module | PRESERVE | **SUPERVISED GREEN RUNTIME.** Donor commit `8170c51` is covered by persistent tenant game state; join/leave; it/FFA transfer; scoring; immunity; ordinary-chat activity wakeup; passes; moderator controls; fixed crowns; exact rotation planning; live/chatting player paging; Pin ranking; durable support, overlay-mode/message, channel opt-out, normalized ingress, provider-neutral replies, visual output, and one-time import. The separately authenticated Nebula consumer now runs inside the Chat Gateway cohort with zero production tenants by default. Automatic rotation fails closed pending fresh canonical presence rather than treating missing monitoring as everyone offline. Current routes, workers, events, XP, and overlay grants are Nebula-owned. Production cutover still requires data reconciliation, live-provider and presence conformance, deployed output, private Sprite/OBS evidence, and owner acceptance | core/experience/provider-runtime/grant/renderer/migration/replay/restart/outage and supervised sandbox suites pass; live-provider, presence, deployed-gateway, import, and OBS acceptance matrix remains |
| Quackverse | ChatTag | Nebula Arcade bounded game module | PRESERVE | preserve overlay/profile behavior | game/overlay test |
| Bingo | ChatTag | Nebula Arcade bounded game module | PRESERVE | deep audit required | game regression test |
...
| Durable per-channel game runtime actions | historical game donor | Nebula Arcade runtime | PRESERVE | Provider delivery-derived receipts now fence membership, action feeds, and score activity against replay; migrate active state only when safe | provider action replay/dedupe/restart tests pass |
...
| Provider command worker | historical donor bot | supervised Nebula Arcade provider consumer | IMPROVE | Implemented as a co-hosted logical consumer with its own SPMT identity; Chat Gateway alone owns sockets and provider grants; zero-tenant config opens nothing | strict-config/channel-isolation/replay/service-composition tests pass; controlled live reconnect proof remains |
| Game-specific state | ChatTag | Nebula Arcade private authority | PRESERVE | one-way legacy table/game-ID migration; classify retention/limits | restart/restore test |
| XP/rewards | ChatTag + SPMT | SPMT ledger; Nebula Arcade emits outcomes | IMPROVE | migrate provenance/idempotency | one-award-only test |
...
| Error classification/repair audit | Rotator/Stella Coder | operations/Stellar Core capability; Stella is the coder and Athena remains the owner's personal bot | IMPROVE | no autonomous production write without approved policy; other users/apps select Stella or their configured persona | dry-run/audit/rollback/identity-isolation test |
...
| MountainView device records | Rotator/MountainView | SPMT device authority + MountainView module | IMPROVE | SPMT now exposes the canonical user-private device API/SDK plus a hashed, expiring, single-use Companion bootstrap. Exchange creates a unique tenant-scoped machine identity; revoke invalidates its live token and credential. Provider tokens and device-local preferences are not stored in shared records. Donor record reconciliation remains | pairing/revocation/replay/expiry/cross-user/capability/API/SDK tests pass; donor migration remains |
| MountainView voice-command routing | MountainView | MountainView planner + owning app public contracts | IMPROVE | The provider-neutral planner now keeps DSH community live state distinct from Nebula Arcade game activity, sends music requests to HearMeOut, stream actions to StreamWeaver, and OBS only to a paired Companion. Unknown intent returns clarification rather than executing a guessed path. The complete donor command/profile/visual/QR inventory remains | implemented route-ownership and no-device cases pass; complete command/visual/device matrix remains |
...
| Companion local relay/compute | Companion | Companion | PRESERVE | The local adapter accepts only paired, non-revoked, tenant/device-matching capability grants. A Companion job worker now claims only tenant-scoped `executionOwner=companion` jobs, verifies job-owner/source equality and fencing, dedupes completed commands, and keeps transient failures retryable with redacted evidence. Desktop packaging and real hardware remain | allow/deny/dedupe/retry/bootstrap/revoke/job-claim/end-to-end command tests pass; real desktop/hardware acceptance remains |
...
| `Mtman1987/chat-tag` | IN PROGRESS — donor tagging behavior is implemented as Nebula Arcade game `tag` and ready for private Sprite sandbox at baseline `8170c51`; production live-provider/output-gateway/import evidence plus Quackverse, Bingo, Arena, and wider Game Hub slices remain | yes |
