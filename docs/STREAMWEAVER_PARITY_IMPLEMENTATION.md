...
## Research and private assistant memory

The Research page saves enabled state, live-search preference, knowledge packs, source-domain allowlist, result count and cache duration. It submits a private question through the same SPMT assistant admission and job-result API as other apps, renders the actual answer and source links, and preserves unfinished input during snapshots. Explicit chat research phrases and the two-minute “I have a question” follow-up route use the same typed research request. Follow-up state is scoped to tenant, provider, channel and actor, survives restart and retains an idempotent result for retried input.

Stellar owns retrieval and its bounded cache. The live Vocaloid knowledge pack is migrated intact into the shared worker. Live retrieval filters source domains, refuses credential-bearing/non-HTTPS source links, and includes missing-provider or no-evidence warnings in the returned result and reply. Cache entries include tenant, query and all selected options. Private requests bypass the cache. The app stores preferences and temporary chat-routing receipts, not a second search provider. The existing assistant job remains the admission, routing and metering contract.

The worker uses `STELLAR_BRAVE_SEARCH_CREDENTIAL` for the optional server-side Brave adapter; the browser never receives it. Its fixed endpoint and parameters follow the [Brave Search documentation](https://api-dashboard.search.brave.com/app/documentation/web-search). `SPMT_OUTBOUND_MODE=disabled` prevents configuring that external adapter. No Sprite network policy was expanded and no credentialed live search was performed.

Explicit `remember:false` now takes precedence over a persona configured to remember conversations. Such turns do not load context/history and are excluded from later remembered turns. Earlier StreamWeaver private-conversation IDs also receive ephemeral retention treatment, correcting the previous presentation override without exposing their contents.

Validation: all 794 offline tests passed. The new tests exercise real SDK/API admission, shared-worker execution, evidence in the inference context and actual job result, changed-option idempotency conflicts, private memory, source filtering, cache isolation and expiry, unavailable retrieval, exact search request construction, and durable scoped follow-ups. DOM execution covers mounted URLs, zero cache, private submission, draft preservation, warnings and safe source links. Authenticated phone rendering, external search and live model acceptance remain unobserved.

## Shared media foundation (2026-09-06)

SPMT now owns binary media storage and its HTTP/SDK contracts. StreamWeaver's Media Files page operates on that same store, including in standalone mode. It uploads privately, previews images/audio/video, paginates the owner's files, reports exact workspace bytes against the existing plan's storage allowance, publishes an opaque URL on explicit request, revokes publication, and deletes files. Public URLs support cross-origin overlay consumers; private content requires the owner's authenticated session. Both support byte ranges for playback/seek. Avatar appearance fields and media controls can use a published URL without maintaining another file copy.

Uploads are capped at 8 MiB and checked against raster/audio/video signatures (HTML and SVG are rejected). SQLite transactions enforce the plan limit and replay signatures; a retry cannot create another copy or replace a prior file's content. A workspace may hold at most 20,000 files. Private recordings expire within 24 hours (one hour by default); expired/deleted bytes free capacity. The media list reports actual storage occupancy; this is an existing-plan storage ceiling, not a new per-upload charge or a second currency. Other usage counters remain unchanged.

Stellar workers receive media scopes, but must also present their current canonical job lease and fencing epoch. They can read only that job's explicit `mediaAssetIds` or its own output files, cannot change input files, and cannot publish generated output unless the admitted job explicitly requests `mediaVisibility: public`. Uploaded output ownership is derived from the job's billed user and source app, never worker-supplied user IDs. Private jobs keep media references in the bounded job envelope; no binary/base64 payload bypasses the job limits.

ApolloStation and standalone product pages now permit same-origin/HTTPS media playback and local blob playback. The shell permits same-origin microphone requests; recording still requires a user action and browser permission. Camera and other device permissions remain unchanged. Existing signed-in sessions need a fresh sign-in to receive the new media scopes.

Acceptance: persistent storage/retry/conflict/quota/expiry tests; real SDK/HTTP owner and tenant isolation, stale lease denial and explicit publication; shell and standalone proxy reads and a 3 MiB upload; byte range, suffix, invalid range, HEAD and deletion; DOM interaction checks for safe filenames, preserved forms, confirmation, copy, revoke and delete. Real browser audio/device acceptance remains outstanding. This foundation does not claim that synthesis, transcription, image controls or relay attachments are complete; those are the next consumers to connect.
