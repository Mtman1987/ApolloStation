...
| Tenant bot dispatch | StreamWeaver | StreamWeaver | PRESERVE | A tenant-selected persona config controls owner identity, aliases, home channels, and the durable external summon window. Dispatch uses canonical identity when linked, a provider-scoped fallback otherwise, stable delivery-derived job identity, and a restart-safe result outbox. Accepted jobs now produce one logical provider reply; foreign jobs are rejected | implemented routing/restart/result/retry cases pass; live-provider two-tenant delivery remains |
