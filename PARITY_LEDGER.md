...
| Donor surface/capability | Donor owner | Green owner | Disposition | Compatibility/migration | Proof required |
|---|---|---|---|---|---|
| Original Chat Tag game | ChatTag | ChatTag core module | PRESERVE | Core game state, join/leave, it/FFA transfer, scoring, immunity, sleep/wake, passes and fixed monthly crown rewards are implemented from donor commit `8170c51`; Nebula Arcade persists tenant-isolated state and permanent command/crown dedupe in its private SQLite authority, consumes a provider-neutral Twitch/Discord/Kick command envelope, and uses a durable idempotent outbox for SPMT events/XP. Real Chat Gateway/provider connections, timers, overlays and donor-state migration remain child slices | two-player, two-tenant, immunity, pass, crown, normalized-ingress, replay, restart and SPMT outage/retry tests now pass; provider-connection/overlay/migration matrix remains |
