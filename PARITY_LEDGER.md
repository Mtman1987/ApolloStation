...
| Durable per-channel game runtime actions | ChatTag | Nebula Arcade runtime | PRESERVE | migrate active state only when safe | action replay/dedupe test |
...
| Bot worker | historical donor bot | elastic/lease-aware Nebula Arcade provider-ingress worker | IMPROVE | bot owns no canonical state; no legacy service identity | reconnect/duplicate command test |
