...
| Shared normalized chat feed | StreamWeaver | provider-neutral Chat Gateway | IMPROVE | A bounded Chat Gateway normalizes Twitch/Discord/Kick messages, preserves canonical or provider-scoped actor identity, durably dedupes provider messages, queues per-consumer delivery, retries failures, isolates tenants, and feeds Nebula Arcade, StreamWeaver, and Commlink. A deterministic multi-worker supervisor leases provider configs, obtains ephemeral SPMT grants, resumes durable cursors, and applies donor-aligned reconnect backoff. SPMT now seals provider credentials, fences and rotates OAuth refresh tokens, supports replace-only bot credentials, and lets a rejected socket recover once before pausing for reauthorization. Concrete Twitch/Discord/Kick protocol drivers exist; controlled two-tenant live sockets and sandbox/external proof remain | dedupe/replay/consumer/lease/cursor/backoff/refresh/reauthorization/driver tests pass; live-provider and external-client conformance remain |
