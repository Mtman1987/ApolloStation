...
| Donor surface/capability | Donor owner | Green owner | Disposition | Compatibility/migration | Proof required |
|---|---|---|---|---|---|
| Original Chat Tag game | ChatTag | ChatTag core module | PRESERVE | Core game state, join/leave, it/FFA transfer, scoring, immunity, sleep/wake, passes, fixed monthly crown rewards, restart-safe rotation planning, read-only overlay projection and a one-time donor-state importer are implemented from donor commit `8170c51`; Nebula Arcade persists tenant-isolated state and permanent command/crown/migration dedupe in its private SQLite authority, consumes a provider-neutral Twitch/Discord/Kick command envelope, and uses a durable idempotent outbox for new SPMT events/XP. Historical donor scores never replay as new XP. Real Chat Gateway/provider connections and the visual overlay renderer remain child slices | two-player, two-tenant, immunity, pass, crown, normalized-ingress, active/inactive/forced rotation, overlay projection, donor import, replay, restart and SPMT outage/retry tests now pass; provider-connection and visual-renderer matrix remains |
