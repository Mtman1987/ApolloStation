...
function unique(value: unknown, name: string, maximum: number) { if (!Array.isArray(value) || value.length > maximum) throw new ProviderGrantError("invalid", `${name} is invalid`); const result = [...new Set(value.map((item) => id(item, name)))].sort(); if (!result.length && name !== "requiredScopes") throw new ProviderGrantError("invalid", `${name} is empty`); return result; }
