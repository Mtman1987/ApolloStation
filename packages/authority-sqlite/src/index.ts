...
import type { MeteredResourceV1, UsageEventV1 } from "@spmt/contracts";
import type { UsageLedgerStoreV1 } from "@spmt/monetization";

export class SqliteAuthorityStore implements AuthorityStore, AuthStore, ControlStore, UsageLedgerStoreV1 {
  ...
  find(tenantId: string, idempotencyKey: string) { return this.oneJson<UsageEventV1>("SELECT body FROM usage_events WHERE tenant_id = ? AND idempotency_key = ?", tenantId, idempotencyKey); }
  total(tenantId: string, userId: string, period: string, resource: MeteredResourceV1, executionTarget: UsageEventV1["executionTarget"]) { const row = this.db.prepare("SELECT COALESCE(SUM(delta), 0) AS used FROM usage_events WHERE tenant_id = ? AND user_id = ? AND period = ? AND resource = ? AND execution_target = ?").get(tenantId, userId, period, resource, executionTarget) as { used?: number } | undefined; return Math.max(0, Number(row?.used ?? 0)); }
  commit(event: UsageEventV1, limit: number | null) { return this.transaction(() => { const prior = this.find(event.tenantId, event.idempotencyKey); if (prior) { if (JSON.stringify(prior) !== JSON.stringify(event)) throw new Error("Usage idempotency key was reused for a different event"); return { duplicate: true, used: this.total(event.tenantId, event.userId, event.period, event.resource, event.executionTarget), event: prior }; } const used = this.total(event.tenantId, event.userId, event.period, event.resource, event.executionTarget), delta = event.operation === "release" ? -event.quantity : event.quantity, next = Math.max(0, used + delta); if (limit !== null && next > limit) throw new Error("usage-limit-exceeded"); this.db.prepare("INSERT INTO usage_events(tenant_id, user_id, idempotency_key, period, resource, execution_target, delta, occurred_at, body) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)").run(event.tenantId, event.userId, event.idempotencyKey, event.period, event.resource, event.executionTarget, delta, event.occurredAt, json(event)); this.journal("usage", `${event.tenantId}:${event.idempotencyKey}`, event, event.tenantId); return { duplicate: false, used: next, event }; }); }
  list(tenantId: string, userId: string, period: string) { return this.allJson<UsageEventV1>("SELECT body FROM usage_events WHERE tenant_id = ? AND user_id = ? AND period = ? ORDER BY occurred_at, idempotency_key", tenantId, userId, period); }
  ...
  private migrate() {
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS usage_events(tenant_id TEXT NOT NULL, user_id TEXT NOT NULL, idempotency_key TEXT NOT NULL, period TEXT NOT NULL, resource TEXT NOT NULL, execution_target TEXT NOT NULL, delta INTEGER NOT NULL, occurred_at TEXT NOT NULL, body TEXT NOT NULL, PRIMARY KEY(tenant_id, idempotency_key)) STRICT;
    ...
    `);
    const usageColumns = this.db.prepare("PRAGMA table_info(usage_events)").all() as Array<{ name?: string }>;
    if (!usageColumns.some((column) => column.name === "user_id")) this.db.exec("ALTER TABLE usage_events ADD COLUMN user_id TEXT NOT NULL DEFAULT 'legacy-tenant'");
    this.db.exec("CREATE INDEX IF NOT EXISTS usage_events_user_totals ON usage_events(tenant_id, user_id, period, resource, execution_target)");
  }
