...
import type { AppInstallV1, AppManifestV1, ControlStore, EntitlementV1, TenantRecordV1 } from "@spmt/control-core";
...
export class SqliteAuthorityStore implements AuthorityStore, AuthStore, ControlStore {
  ...
  putProviderLink(link: ProviderLinkV1) { this.db.prepare("INSERT INTO provider_links(provider, provider_user_id, user_id, body) VALUES(?, ?, ?, ?)").run(link.provider, link.providerUserId, link.userId, json(link)); this.journal("provider-link", `${link.provider}:${link.providerUserId}`, link); }
