...
export class PlatformApiAdapter {
  ...
  handle(request: ApiRequestV1): ApiResponseV1 {
    try {
      const headers = lowerHeaders(request.headers ?? {}); const accessToken = bearer(headers.authorization); const tenantId = headers["x-spmt-tenant"]; const correlationId = headers["x-correlation-id"];
      const body = request.body && typeof request.body === "object" && !Array.isArray(request.body) ? request.body as Record<string, unknown> : {};
      let operation: PlatformOperationNameV1; let input: Record<string, unknown>;
      const url = new URL(`https://spmt.invalid${request.path}`);
      ...
      else if (request.method === "GET" && /^\/v1\/apps\/[^/]+$/.test(url.pathname)) { operation = "apps.get"; input = { appId: decodeURIComponent(url.pathname.split("/")[3] ?? "") }; }
      else if (request.method === "POST" && /^\/v1\/apps\/[^/]+\/install$/.test(url.pathname)) { operation = "apps.install"; input = { tenantId: requiredTenant(tenantId), appId: decodeURIComponent(url.pathname.split("/")[3] ?? ""), grantedScopes: body.grantedScopes }; }
      else if (request.method === "POST" && /^\/v1\/apps\/[^/]+\/disable$/.test(url.pathname)) { operation = "apps.disable"; input = { tenantId: requiredTenant(tenantId), appId: decodeURIComponent(url.pathname.split("/")[3] ?? "") }; }
      ...
      if (error instanceof PlatformOperationError) { const status = error.code === "unauthorized" ? 403 : error.code === "not_found" ? 404 : error.code === "conflict" ? 409 : 400; return { status, body: { error: error.code, message: error.message } }; }
      return { status: 500, body: { error: "internal" } };
    }
  }
}
function lowerHeaders(headers: Record<string, string | undefined>) { return Object.fromEntries(Object.entries(headers).map(([key, value]) => [key.toLowerCase(), value])); }
function bearer(value?: string) { if (!value?.startsWith("Bearer ") || value.length <= 7) throw new PlatformOperationError("unauthorized", "Bearer access token is required"); return value.slice(7); }
function requiredTenant(value?: string) { if (!value) throw new PlatformOperationError("invalid", "x-spmt-tenant is required"); return value; }
