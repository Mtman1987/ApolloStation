...
export function createProductAppWebServer(options: ProductAppWebServerOptionsV1) {
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      // Keep app APIs under the app's ingress path as well as their canonical
      // API alias. Front doors may reserve /api for their own application.
      const appApiPrefix = `/apps/${descriptor.appId}/api/`;
      if (url.pathname.startsWith(appApiPrefix)) url.pathname = `/api/${descriptor.appId}/${url.pathname.slice(appApiPrefix.length)}`;
...
export function renderProductAppWebPage(app: ProductAppWebDescriptorV1, buildSha = "dev", extra = "", extraCss = "", browserJs = "") {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="robots" content="noindex,nofollow"><meta name="theme-color" content="#f97316"><title>${esc(app.name)} · SpaceMountain</title><style>${PRODUCT_UI_CSS}${CSS}${WORKSPACE_OVERLAY_CSS}${extraCss}</style></head><body class="spmt-product-surface owned" data-app="${esc(app.appId)}" data-surface="standalone" style="--spmt-app-backdrop-image:url('${safeUrl(app.sceneUrl)}')"><div class="spmt-product-backdrop" aria-hidden="true"><span class="spmt-product-backdrop-image"></span><span class="spmt-product-backdrop-tint"></span><span class="spmt-product-backdrop-shade"></span><span class="spmt-star-layer"><i></i><i></i><i></i></span></div><iframe class="spmt-personal-overlay" data-spmt-personal-overlay hidden title="Personal workspace overlay" aria-hidden="true"></iframe><main><nav class="tabs spmt-product-glass" data-spmt-depth="2">${tabs}</nav><section class="home" data-page="home"><div class="hero"><div class="mark"><img data-logo src="/assets/product/app-icons/solar-flare/${esc(app.appId)}.png" alt=""><span class="spmt-product-kicker">${esc(app.kicker)}</span></div><h1>${esc(app.name)}</h1><p>${esc(app.tagline)}</p><div class="home-summary" data-spmt-home-summary aria-live="polite"><span>Connecting to the SPMT developer surface…</span></div><div class="actions"><button class="button primary" data-first>Open ${esc(app.sections[0]?.label ?? "app")}</button><button class="button" data-refresh>Refresh data</button></div><footer><span class="spmt-product-status" data-runtime>Checking runtime</span><small>Build ${esc(buildSha.slice(0, 12))}</small></footer></div></section>${pages}${extra}</main><aside class="spmt-overlay-footer spmt-product-glass" data-spmt-overlay-footer><details data-spmt-workspace-menu><summary>Workspace</summary><div class="spmt-workspace-menu-body"><details open><summary>Personal overlays</summary><div class="spmt-workspace-menu-section"><label>Overlay group<select data-spmt-personal-scene aria-label="Personal overlay group"></select></label><label class="spmt-overlay-switch"><input type="checkbox" role="switch" data-spmt-personal-toggle checked>Show selected overlay <strong data-spmt-personal-state>On</strong></label><span data-spmt-overlay-status role="status"></span><details><summary>Overlay links</summary><div class="spmt-workspace-menu-actions"><button type="button" data-spmt-copy-public>Copy Public URL</button><button type="button" data-spmt-copy-personal>Copy Personal URL</button></div></details></div></details><details><summary>Shared tools</summary><div class="spmt-workspace-menu-actions"><button type="button" data-spmt-open-commlink>Commlink</button><button type="button" data-spmt-open-overlay-bay>Overlay Bay</button><button type="button" data-spmt-open-simulation-rooms>Simulation Rooms</button><button type="button" data-spmt-open-workspace-settings>Settings</button></div></details><button type="button" data-spmt-popout-workspace>Pop out workspace</button></div></details></aside><script>${FRAME_JS}${pageJs}${SNAPSHOT_MODE_JS}${WORKSPACE_OVERLAY_JS}${browserJs}${catalogJs}</script></body></html>`;
}
...
export async function fetchAppPlatformSnapshot(input: { appId: string; spmtOrigin: string; request: IncomingMessage; sources?: readonly ProductAppSnapshotSourceV1[]; liveRead?: ProductAppLiveReadOptionsV1 | null }) {
  ...
  const values: Record<ProductAppSnapshotSourceV1, unknown> = {
    runtime: [], events: [], jobs: [], workers: [], operations: [], devices: [], overlayWidgets: [], overlayOutputs: [], xpWallet: null, xpLedger: [], stellarCapabilities: [], providerLinks: [], communityLive: null, workspace: null, tenantOutputs: null,
  };
  const paths: Record<ProductAppSnapshotSourceV1, string> = {
    ...
    // The existing live-community read contract is blue-v1; Green has no replacement yet.
    communityLive: "",
  ...
  if (liveRead && liveProtocol === "blue-v1") Object.assign(paths, {
    ...
    communityLive: "/api/live-community",
  ...
  if (requestedSources.includes("operations") && (!liveRead || liveProtocol === "green-v1")) {
  ...
  return {
    ...
    communityLive: values.communityLive,
...
const WORKSPACE_OVERLAY_CSS = `.spmt-personal-overlay{position:fixed;inset:0;width:100%;height:100%;border:0;z-index:8;pointer-events:none;background:transparent;color-scheme:only light}.spmt-personal-overlay[hidden]{display:none}.spmt-overlay-footer{position:fixed;z-index:20;left:50%;bottom:max(8px,env(safe-area-inset-bottom));transform:translateX(-50%);padding:6px 10px;border-radius:14px;color:var(--spmt-ink,#eff4ff)}.spmt-overlay-footer summary{cursor:pointer;padding:10px;font-size:12px;font-weight:800}.spmt-workspace-menu-body{position:absolute;bottom:calc(100% + 8px);left:50%;transform:translateX(-50%);box-sizing:border-box;width:min(380px,calc(100vw - 24px));max-height:70dvh;overflow:auto;display:grid;gap:12px;padding:14px;border:1px solid var(--spmt-border);border-radius:16px;background:var(--spmt-surface-depth-3,#080d18);box-shadow:0 16px 60px #0009}.spmt-workspace-menu-body details{border:1px solid var(--spmt-border);border-radius:10px}.spmt-workspace-menu-section{display:grid;gap:12px;padding:10px}.spmt-workspace-menu-section>label{display:grid;gap:6px;font-size:12px}.spmt-workspace-menu-section>label.spmt-overlay-switch{display:flex;align-items:center;gap:8px}.spmt-overlay-switch input{width:20px;height:20px;accent-color:var(--spmt-accent)}.spmt-overlay-switch strong{margin-left:auto}.spmt-workspace-menu-actions{display:flex;flex-wrap:wrap;gap:8px;padding:10px}.spmt-overlay-footer select,.spmt-overlay-footer button{box-sizing:border-box;min-height:40px;padding:8px 10px;border:1px solid var(--spmt-border);border-radius:10px;background:var(--spmt-surface-depth-3,#101828);color:var(--spmt-ink,#eff4ff);font-size:12px}.spmt-overlay-footer select{width:100%;color-scheme:dark}.owned[data-surface="shell"]>.spmt-overlay-footer,.owned[data-surface="shell"]>.spmt-personal-overlay{display:none!important}@media(max-width:620px){.owned main{padding-bottom:64px}}`;

const WORKSPACE_OVERLAY_JS = String.raw`;(()=>{
 const frame=document.querySelector('[data-spmt-personal-overlay]'),footer=document.querySelector('[data-spmt-overlay-footer]'),toggle=footer?.querySelector('[data-spmt-personal-toggle]'),select=footer?.querySelector('[data-spmt-personal-scene]'),status=footer?.querySelector('[data-spmt-overlay-status]');
 let outputs=null,workspace=null,tenantId='',readOnly=true,busy=false,lastRevision='';
 function sync(){
  const visible=workspace?.personalOverlayEnabled!==false;
  const raw=outputs?.personal?.url;
  const url=raw?new URL(new URL(raw,location.origin).pathname,location.origin).href:'';
  if(frame){frame.hidden=window.parent!==window||!visible||!url||!(workspace?.activePersonalOverlaySceneId??outputs?.personal?.sceneId);if(url&&lastRevision!==String(workspace?.revision)){frame.src=url;lastRevision=String(workspace?.revision)}}
  if(toggle){toggle.checked=visible;toggle.disabled=readOnly||busy}const state=footer?.querySelector('[data-spmt-personal-state]');if(state)state.textContent=visible?'On':'Off';
  if(select){select.replaceChildren();for(const scene of workspace?.overlayScenes||[]){const option=document.createElement('option');option.value=scene.id;option.textContent=scene.name;option.selected=scene.id===(workspace.activePersonalOverlaySceneId??outputs?.personal?.sceneId);select.append(option)}select.disabled=readOnly||busy||!select.options.length}
 }
 async function save(patch){if(!tenantId||readOnly||busy)return;busy=true;sync();try{const response=await fetch('/v1/workspace/profile',{method:'PATCH',credentials:'same-origin',headers:{'content-type':'application/json','x-spmt-tenant':tenantId},body:JSON.stringify({expectedRevision:workspace.revision,patch})});if(!response.ok)throw new Error('Overlay change could not be saved. Refresh and try again.');workspace=await response.json();if(status)status.textContent='Saved';}catch(error){if(status)status.textContent=error.message}finally{busy=false;sync()}}
 async function refresh(){if(!tenantId||busy||document.visibilityState!=='visible'||window.parent!==window)return;try{const response=await fetch('/v1/workspace/profile',{credentials:'same-origin',headers:{'x-spmt-tenant':tenantId}});if(response.ok){const next=await response.json();if(next.revision!==workspace?.revision){workspace=next;sync()}}}catch{}}
 async function copy(name){const url=outputs?.[name]?.url;if(!url)return;if(navigator.clipboard?.writeText)await navigator.clipboard.writeText(url);else window.prompt('Copy '+name+' overlay URL',url)}
 toggle?.addEventListener('change',()=>void save({personalOverlayEnabled:toggle.checked}));
 select?.addEventListener('change',()=>void save({activePersonalOverlaySceneId:select.value}));
 footer?.querySelector('[data-spmt-workspace-menu]')?.addEventListener('keydown',event=>{if(event.key==='Escape'){event.currentTarget.open=false;event.currentTarget.querySelector('summary')?.focus()}});
 footer?.querySelector('[data-spmt-open-commlink]')?.addEventListener('click',()=>window.open('/?surface=workspace-popout&tool=commlink','spmt-workspace','popup,width=1440,height=920'));
 footer?.querySelector('[data-spmt-open-workspace-settings]')?.addEventListener('click',()=>window.open('/?surface=workspace-popout&tool=settings','spmt-workspace','popup,width=1440,height=920'));
 footer?.querySelector('[data-spmt-popout-workspace]')?.addEventListener('click',()=>window.open('/?surface=workspace-popout','spmt-workspace','popup,width=1440,height=920'));
 footer?.querySelector('[data-spmt-copy-public]')?.addEventListener('click',()=>void copy('public'));
 footer?.querySelector('[data-spmt-copy-personal]')?.addEventListener('click',()=>void copy('personal'));
 function openRooms(roomId){const selected=typeof roomId==='string'&&roomId.length<=200?roomId:undefined;if(window.parent!==window)window.parent.postMessage({protocol:'spmt.surface',version:1,type:'simulation.open',appId:document.body.dataset.app||'',...(selected?{roomId:selected}:{})},location.origin);else location.assign('/?view=workspace&surface=simulation'+(selected?'&roomId='+encodeURIComponent(selected):''))}
 footer?.querySelector('[data-spmt-open-simulation-rooms]')?.addEventListener('click',()=>openRooms());window.addEventListener('spmt:open-simulation-rooms',event=>openRooms(event.detail?.roomId));
 footer?.querySelector('[data-spmt-open-overlay-bay]')?.addEventListener('click',()=>{if(window.parent!==window)window.parent.postMessage({protocol:'spmt.surface',version:1,type:'shell.navigate',appId:document.body.dataset.app||'',view:'workspace'},location.origin);else location.assign('/?view=workspace')});
 window.addEventListener('spmt:snapshot',event=>{const next=event.detail;if(!next?.workspace||!next.tenantOutputs)return;workspace=next.workspace;outputs=next.tenantOutputs;tenantId=next.tenantId||'';readOnly=next.operationMode==='read-only';sync()});
 window.setInterval(()=>void refresh(),5000);window.addEventListener('focus',()=>void refresh());sync();
})();`;
