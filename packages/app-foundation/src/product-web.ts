...
const WORKSPACE_OVERLAY_JS = String.raw`;(()=>{
 ...
 function sync(){
  ...
  if(frame){frame.hidden=!visible||!url;if(url&&lastRevision!==String(workspace?.revision)){frame.src=url;lastRevision=String(workspace?.revision)}}
  if(toggle){toggle.textContent='Overlay '+(visible?'On':'Off');toggle.setAttribute('aria-pressed',String(visible));toggle.disabled=readOnly||busy}
 ...
 toggle?.addEventListener('click',()=>void save({personalOverlayEnabled:workspace?.personalOverlayEnabled===false}));
