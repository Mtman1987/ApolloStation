...
export async function proxyAppMedia(input:{appId:string;spmtOrigin:string;request:IncomingMessage;response:ServerResponse;url:URL;fetchImpl?:typeof fetch}):Promise<boolean>{
  const {request,response,url}=input;if(!url.pathname.startsWith("/v1/media/")&&!url.pathname.startsWith("/v1/assistant/")&&url.pathname!=="/v1/identity/providers/youtube/connections"&&!/^\/v1\/commlink\/(operator|filters|ingestion-errors|social-stream)$/.test(url.pathname))return false;
