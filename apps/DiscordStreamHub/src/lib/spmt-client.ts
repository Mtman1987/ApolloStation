...
export async function grandfatherDiscordIdentity(input: {
  ...
  if (!isSpmtEnabled()) return null;
  try {
    const response = await spmtPlatformFetch('identity:write', `${SPMT_BASE_URL.replace(/\/$/, '')}/api/platform/identity/grandfather`, {
      method: 'POST',
...
export async function onboardVerifiedSpmtIdentity(input: {
  ...
  try {
    response = await spmtPlatformFetch('identity:write', `${SPMT_BASE_URL.replace(/\/$/, '')}/api/platform/identity/onboard`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${SPMT_API_KEY}` },
...
export async function publishSpmtEvent(event: SpmtEventInput) {
  if (!isSpmtEnabled()) return { skipped: true, reason: 'SPMT_API_KEY not configured' };

  try {
    const response = await spmtPlatformFetch('events:write', `${SPMT_BASE_URL.replace(/\/$/, '')}/api/platform/events`, {
      method: 'POST',
      headers: {
...
export async function awardSpmtXp(input: SpmtXpAwardInput) {
  ...
  try {
    const response = await spmtPlatformFetch('xp:write', `${SPMT_BASE_URL.replace(/\/$/, '')}/api/platform/xp`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${SPMT_API_KEY}`,
...
async function platformXpRequest<T>(path: string, body: Record<string, unknown>): Promise<T | null> {
  if (!isSpmtEnabled()) return null;
  try {
    const response = await spmtPlatformFetch('xp:write', `${SPMT_BASE_URL.replace(/\/$/, '')}/api/platform/xp${path}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${SPMT_API_KEY}` },
...
export async function migrateSpmtXpBalance(input: {
  ...
  if (!isSpmtEnabled()) return { skipped: true, reason: 'SPMT_API_KEY not configured' };
  try {
    const response = await spmtPlatformFetch('xp:write', `${SPMT_BASE_URL.replace(/\/$/, '')}/api/platform/xp/migrate-balance`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${SPMT_API_KEY}`,
