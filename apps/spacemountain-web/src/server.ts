...
const ASSETS = new Map<string, { file: string; type: string }>([
  ...
  ["/assets/sdk/suite-actions.js", { file: resolve(REPOSITORY_ROOT, "packages/sdk/dist/suite-actions.js"), type: "text/javascript; charset=utf-8" }],
  ["/assets/ui/community-calendar.js", { file: resolve(REPOSITORY_ROOT, "packages/ui/dist/community-calendar.js"), type: "text/javascript; charset=utf-8" }],
  ...
  ["/assets/spacemountain/shell-ui-base.js", { file: resolve(REPOSITORY_ROOT, "apps/spacemountain/dist/shell-ui-base.js"), type: "text/javascript; charset=utf-8" }],
  ["/assets/spacemountain/overlay-bay-ui.js", { file: resolve(REPOSITORY_ROOT, "apps/spacemountain/dist/overlay-bay-ui.js"), type: "text/javascript; charset=utf-8" }],
  ["/assets/spacemountain/overlay-scenes.js", { file: resolve(REPOSITORY_ROOT, "apps/spacemountain/dist/overlay-scenes.js"), type: "text/javascript; charset=utf-8" }],
...
function browserProxyAllowed(method: string, pathname: string) {
  if(pathname==="/v1/identity/twitch-bots")return ["GET","POST"].includes(method);
  if(/^\/v1\/identity\/twitch-bots\/(start|callback)$/.test(pathname))return method==="GET";
  if (["/v1/assistant/conversation", "/v1/assistant/conversation/feed", "/v1/assistant/public-memory", "/v1/assistant/streams"].includes(pathname)) return method === "GET" || (method === "POST" && pathname === "/v1/assistant/conversation");
  if (/^\/v1\/assistant\/(conversation\/(control|clear|condense)|public-memory\/(clear|adjust|condense)|persona\/optimize)$/.test(pathname)) return method === "POST";
  if (/^\/v1\/assistant\/jobs\/[A-Za-z0-9._:-]+$/.test(pathname)) return method === "GET";
