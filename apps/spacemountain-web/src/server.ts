  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/workspace/overlay") {
        ...
        return html(response, 200, `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Personal overlay</title><style>html,body{margin:0;width:100%;height:100%;background:transparent;color-scheme:normal}iframe{width:100%;height:100%;border:0;pointer-events:none;background:transparent;color-scheme:normal}iframe[hidden]{display:none}</style></head><body><iframe data-overlay-frame title="Personal overlay" hidden></iframe><script type="module" nonce="${nonce}" src="/assets/web/personal-overlay-client.js"></script></body></html>`);
      }
