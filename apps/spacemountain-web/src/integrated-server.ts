...
import { createIntegratedSpaceMountainWebHost as createBaseIntegratedHost, type IntegratedSpaceMountainWebHostOptions as BaseIntegratedSpaceMountainWebHostOptions } from "./integrated-server-base.js";
...
export function createIntegratedSpaceMountainWebHost(options:IntegratedSpaceMountainWebHostOptions){
  ...
  const outer=createServer(async(request,response)=>{try{const url=new URL(request.url??"/","http://spacemountain.parity");
    ...
    if(HEARMEOUT_APP.test(url.pathname)&&options.hearMeOutOrigin){
      const path=url.pathname.startsWith("/health/hearmeout")?"/health/ready":url.pathname+url.search;
      return proxyDirect(request,response,options.hearMeOutOrigin,path,true);
    }
...
if(process.argv[1]&&import.meta.url===pathToFileURL(resolve(process.argv[1])).href){const checked=validateSandboxWebEnvironment(process.env);const hearMeOutOrigin=process.env.HEARMEOUT_ORIGIN;const host=createIntegratedSpaceMountainWebHost({spmtOrigin:checked.spmtOrigin,port:Number(process.env.PORT??8080),host:process.env.HOST??"0.0.0.0",buildSha:process.env.BUILD_SHA??"dev",...(checked.nebulaArcadeOrigin?{nebulaArcadeOrigin:checked.nebulaArcadeOrigin}:{}),...(hearMeOutOrigin?{hearMeOutOrigin}:{}),...(checked.candidateManifest?{candidateManifest:checked.candidateManifest}:{})});await host.listen();}
