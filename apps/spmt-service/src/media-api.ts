...
async function readBytes(request:IncomingMessage) { const declared=Number(header(request,"content-length")??0);if(declared>MEDIA_ASSET_MAX_BYTES)throw new MediaAssetError(413,"Media cannot exceed 8 MiB");const chunks:Buffer[]=[];let total=0;for await(const chunk of request){const bytes=Buffer.from(chunk);total+=bytes.length;if(total>MEDIA_ASSET_MAX_BYTES)throw new MediaAssetError(413,"Media cannot exceed 8 MiB");chunks.push(bytes);}return Buffer.concat(chunks); }
