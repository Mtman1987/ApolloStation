...
export class PublicPersonaApi {
 ...
 async handle(req:IncomingMessage,res:ServerResponse,url:URL){if(url.pathname!==ROOT&&url.pathname!==ROOT+'/invocations')return false;
  try{const token=this.options.accessToken(req),tenant=typeof req.headers['x-spmt-tenant']==='string'?req.headers['x-spmt-tenant']:'';if(!token||!tenant)return json(res,401,{message:'Sign in to a workspace'});const actor=this.options.auth.authorize(token,'assistants:invoke',tenant);if(actor.actorType!=='user')return json(res,403,{message:'Use your signed-in account'});const workspace=this.options.control.getTenant(tenant);if(workspace.status!=='active')return json(res,403,{message:'Workspace is suspended'});
