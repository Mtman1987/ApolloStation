...
export class CompanionWorkflowClaimCoordinator {
  ...
  claim(jobId:string){return this.store.claim(jobId,this.claimantId,{now:this.now(),...(this.options.leaseMs===undefined?{}:{leaseMs:this.options.leaseMs})});}
  heartbeat(jobId:string){return this.store.renew(jobId,this.claimantId,{now:this.now(),...(this.options.leaseMs===undefined?{}:{leaseMs:this.options.leaseMs})});}
