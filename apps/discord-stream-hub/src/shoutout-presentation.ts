...
export function buildDshTierShoutout(view:DshShoutoutView,options:{templates?:DshEmbedTemplates;timestamp?:string;partnerDiscordUrl?:string}={}){
  ...
  if(view.group==='Crew'||view.group==='Partners'){
    ...
    return{embeds:crew&&view.bannerUrl?[{image:{url:view.bannerUrl},color},embed]:[embed],...(!crew?{components:[{type:1,components:[{type:2,style:5,label:'Watch on Twitch',url,emoji:{name:'📺'}},{type:2,style:5,label:'Stream schedule',url:url+'/schedule'},...(discord?[{type:2,style:5,label:'Join Their Discord',url:discord,emoji:{name:'💬'}}]:[])]}]}:{}),allowed_mentions:{parse:[]}};
  }
