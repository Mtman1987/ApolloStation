...
export class DshProposalControls {
  ...
  private async channels(tenantId: string) {
    const channels: Array<{id:string;name:string;guildName:string}> = [];
    if (!this.discord) return channels;
    const configured = new Set(this.tenant(tenantId).discordGuildIds ?? []);
    for (const guild of await this.discord.listGuilds(tenantId)) {
      if (!guild.id || (!isSimulationDiscordId(guild.id) && !configured.has(guild.id))) continue;
      for (const channel of await this.discord.listGuildChannels(tenantId, guild.id)) {
        if (channel.id && channel.type === 0) channels.push({id:channel.id,name:channel.name ?? channel.id,guildName:guild.name ?? guild.id});
      }
    }
    return channels;
  }
  ...
  private async state(response:ServerResponse,context:SessionContext) {
    return sendJson(response,200,{schemaVersion:1,channels:await this.channels(context.tenantId),proposals:this.list(context.tenantId,100)});
  }

  private async validateDestination(tenantId: string, channelId: string) {
    if (!(await this.channels(tenantId)).some(channel => channel.id === channelId)) {
      throw new Error("Proposal channel is invalid for this tenant's Discord servers or shadow rooms");
    }
  }

  private async post(response:ServerResponse,context:SessionContext,body:Record<string,unknown>){
    const now=this.now(),referenceUrl=optionalUrl(body.referenceUrl),proposal:DshProposalV1={schemaVersion:1,id:randomUUID(),tenantId:cleanId(context.tenantId,"tenantId"),channelId:snowflake(body.channelId,"channelId"),title:cleanText(body.title,"title",256),description:cleanText(body.description,"description",4096),audience:audience(body.audience),approveLabel:cleanText(body.approveLabel||"Approve","approveLabel",80),denyLabel:cleanText(body.denyLabel||"Deny","denyLabel",80),approveEmoji:cleanEmoji(body.approveEmoji||"✅","approveEmoji"),denyEmoji:cleanEmoji(body.denyEmoji||"❌","denyEmoji"),color:parseColor(body.color),...(referenceUrl?{referenceUrl}:{}),createdBy:cleanId(String(context.session.actorId),"createdBy"),createdAt:now,updatedAt:now,delivery:{status:"pending",attempts:0}};
    await this.validateDestination(context.tenantId, proposal.channelId);
  ...
  private async retry(response:ServerResponse,context:SessionContext,body:Record<string,unknown>){const proposal=this.get(context.tenantId,cleanId(body.proposalId,"proposalId"));if(!proposal)throw new Error("Proposal not found");if(proposal.delivery.status==="delivered")return sendJson(response,200,{schemaVersion:1,proposal});await this.validateDestination(context.tenantId,proposal.channelId);const delivered=await this.deliver(proposal);return sendJson(response,delivered.delivery.status==="delivered"?200:502,{schemaVersion:1,proposal:delivered});}
  ...
  private async deliver(proposal:DshProposalV1){
    ...
    try{
      const messageId=proposal.delivery.messageId ?? await this.discord.createMessage(proposal.tenantId,proposal.channelId,buildDshProposalMessage(proposal));
      // Preserve the created message before adding reactions so a retry cannot repost it.
      proposal={...proposal,delivery:{...proposal.delivery,messageId}};
      this.write(proposal);
  ...
  private delivery(value:DshProposalV1,result:{status:"delivered"|"failed";messageId?:string;error?:string}){const updatedAt=this.now();return{...value,updatedAt,delivery:{status:result.status,attempts:value.delivery.attempts+1,...((result.messageId??value.delivery.messageId)?{messageId:result.messageId??value.delivery.messageId}:{}),...(result.error?{error:result.error.slice(0,1000)}:{})}} as DshProposalV1;}
