...
export const DSH_TENANT_SETTINGS_V1: AppSettingsDefinitionV1 = {
  ...
  fields: [
    ...
    { key: "raidPileMaxSize", label: "Raid Pile split size", description: "Create another balanced Raid Pile after a pile exceeds this many members.", type: "number", sensitive: false, required: true, defaultValue: 40, minimum: 2, maximum: 500 },
    { key: "raidPileMinSize", label: "Raid Pile merge size", description: "Merge a pile tier down when membership falls below this threshold per remaining pile.", type: "number", sensitive: false, required: true, defaultValue: 10, minimum: 1, maximum: 499 },
    { key: "raidPilePointsReward", label: "Raid Pile points", description: "Canonical XP reward associated with a compliant Raid Pile handoff.", type: "number", sensitive: false, required: true, defaultValue: 25, minimum: 0, maximum: 100000 },
    { key: "raidPileHandoffHours", label: "Raid Pile handoff hours", description: "Hours before the bot recommends handing the pile onward and advances the canonical target.", type: "number", sensitive: false, required: true, defaultValue: 4, minimum: 1, maximum: 48 },
    { key: "raidPileMonthlyStrikeLimit", label: "Raid Pile monthly strike limit", description: "Off-pile raids allowed in a UTC month before automatic Raid Pile removal.", type: "number", sensitive: false, required: true, defaultValue: 3, minimum: 1, maximum: 20 },
  ],
};
...
export interface DshTenantSettingsV1 { schemaVersion: 1; tenantId: string; roleMappings: Record<string, Record<string, DshMemberGroupV1>>; captainMinimumDays: number; spotlightChannelId?: string; signalChannelId?: string; gifStorageChannelId?: string; groupChannels: Record<string, string>; embedTemplates?: DshEmbedTemplates; pollIntervalSeconds: number; spotlightEnabled: boolean; signalSeekerEnabled: boolean; raidPileMaxSize: number; raidPileMinSize: number; raidPilePointsReward: number; raidPileHandoffHours: number; raidPileMonthlyStrikeLimit: number; revision: number; }
