...
export interface DshDiscordGrantSourceV1 { getGrant(input:{tenantId:string;capability:"messages:write"}):Promise<DshDiscordGrantV1>; }
...
export interface DshTrackedDiscordMessageV1 { tenantId:string; kind:"shoutout"|"spotlight"; key:string; channelId:string; messageId:string; updatedAt:string; }
...
export class DshDiscordApi {
  ...
  private async request<T=unknown>(tenantId:string,path:string,method:string,body?:unknown):Promise<T|undefined>{const grant=await this.grants.getGrant({tenantId:cleanId(tenantId,"tenantId"),capability:"messages:write"});if(!grant?.authorization||/[\r\n]/.test(grant.authorization))throw new Error("Discord grant is unavailable");if(grant.expiresAt&&Date.parse(grant.expiresAt)<=Date.now())throw new Error("Discord grant is expired");const response=await this.fetchImpl(`${this.origin.replace(/\/$/,"")}${path}`,{method,headers:{authorization:grant.authorization,accept:"application/json",...(body===undefined?{}:{"content-type":"application/json"})},...(body===undefined?{}:{body:JSON.stringify(body)})});if(response.status===204)return undefined;const text=await response.text();let payload:unknown=undefined;if(text){try{payload=JSON.parse(text);}catch{payload={message:text.slice(0,500)};}}if(!response.ok)throw new DshDiscordError(response.status,payload);return payload as T;}
}
