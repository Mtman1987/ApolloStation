...
export class DshDiscordApplicationInteractions {
  ...
  async handle(request: IncomingMessage, response: ServerResponse, url: URL) {
    ...
    if (!type || !/^\d{5,30}$/.test(guildId) || embeddedGuildId !== guildId) return json(response, 200, ephemeral("This application action is invalid or belongs to another server."));
    const tenant = this.options.config.tenants.find((item) => (item.discordGuildIds?.length ? item.discordGuildIds.includes(guildId) : this.options.config.tenants.length === 1));
    if (!tenant) return json(response, 200, ephemeral("This Discord server is not connected to an SPMT tenant."));
    if (interaction.type === 3 && action === "application_inquiry") return json(response, 200, { type: 4, data: { flags: 64, ...buildDshInquiryMessage(type, guildId, this.options.publicOrigin) } });
    if (interaction.type === 3 && action === "application_start") return json(response, 200, { type: 9, data: buildDshApplicationModal(type, guildId) });
    if (interaction.type === 5 && action === "application_submit") {
      ...
      const saved = this.options.store.submit({ tenantId: tenant.tenantId, guildId, interactionId: String(interaction.id ?? ""), type, applicantDiscordId: String(user.id ?? ""), applicantUsername: String(user.global_name ?? user.username ?? user.id ?? "Discord member"), answers: normalized });
      return json(response, 200, ephemeral(saved.duplicate ? "Your application was already received." : "Application received. The SPMT owner can now review it privately in Discord Stream Hub."));
    }
    return json(response, 200, ephemeral("This application action is not available."));
  }
}
