...
export class DshApplicationControls {
  ...
  private async decide(response:ServerResponse,context:SessionContext,body:Record<string,unknown>){const decision=body.decision==="approved"?"approved":body.decision==="rejected"?"rejected":undefined;if(!decision)throw new Error("Application decision is invalid");const application=this.store.decide(context.tenantId,text(body.applicationId,"applicationId",300),decision,String(context.session.actorId),optionalText(body.note,1000),this.now()),delivered=await this.deliverDecision(context,application,false);return sendJson(response,200,{schemaVersion:1,application:delivered.application,notification:delivered.notification});}
  private async notify(response:ServerResponse,context:SessionContext,body:Record<string,unknown>,resend:boolean){const application=this.store.get(context.tenantId,text(body.applicationId,"applicationId",300));if(!application||application.status==="pending")throw new Error("Application has no final decision");const delivered=await this.deliverDecision(context,application,resend);return sendJson(response,200,{schemaVersion:1,application:delivered.application,notification:delivered.notification});}
  private async deliverDecision(context:SessionContext,application:DshApplicationV1,resend:boolean){
    if(application.status==="pending")throw new Error("Application has no final decision");if(!resend&&application.notification?.status==="delivered")return{application,notification:"sent" as const};let current=application,agreementUrl:string|undefined;
    if(application.status==="approved"){const offer=this.store.createAgreementOffer(context.tenantId,application.id,String(context.session.actorId),this.now());current=offer.application;if(!offer.rawToken)throw new Error("Accepted agreements cannot be resent");agreementUrl=this.agreementUrl(application.id,offer.rawToken);}
    const template=this.store.templates(context.tenantId)[dshTemplateKey(application.type,application.status)],payload=buildDshApplicationDecisionMessage({type:application.type,decision:application.status,customMessage:template,decisionNote:application.decisionNote,agreementUrl,now:this.now()});
    ...
    try{const messageId=await this.discord.sendDirectMessage(context.tenantId,application.applicantDiscordId,payload);current=this.store.setNotificationResult(context.tenantId,application.id,{status:"delivered",messageId},this.now());return{application:current,notification:"sent" as const};}catch(error){current=this.store.setNotificationResult(context.tenantId,application.id,{status:"failed",error:safeError(error)},this.now());return{application:current,notification:"unavailable" as const};}
  }
  ...
  private async agreementGet(request:IncomingMessage,response:ServerResponse,url:URL,context:SessionContext){const applicationId=text(url.searchParams.get("applicationId"),"applicationId",300),token=text(url.searchParams.get("token"),"token",300),application=this.store.get(context.tenantId,applicationId);this.verifyOffer(application,token);if(url.searchParams.get("format")==="receipt"){if(!application!.agreementAcceptance)return sendJson(response,404,{error:"receipt_not_found",message:"No acceptance receipt exists."});return jsonDownload(response,application!.agreementAcceptance,`spmt-acceptance-${application!.agreementAcceptance.acceptanceId}.json`);}const identity=await this.identity(request,context),acceptance=application!.agreementAcceptance;return sendJson(response,200,{schemaVersion:1,applicationId,role:application!.type,applicantDiscordId:application!.applicantDiscordId,document:application!.agreementOffer!.document,acceptanceSchedule:application!.agreementOffer!.acceptanceSchedule,authenticated:true,account:identity,identityMatches:Boolean(identity.discordUserId&&identity.discordUserId===application!.applicantDiscordId),accepted:acceptance?{acceptanceId:acceptance.acceptanceId,acceptedAt:acceptance.acceptedAt,receiptUrl:this.agreementApiUrl(applicationId,token,"receipt")}:null});}
