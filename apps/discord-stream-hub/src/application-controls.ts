...
export class DshApplicationControls {
  ...
  private async deliverDecision(context:SessionContext,application:DshApplicationV1,resend:boolean){
    ...
    const template=this.store.templates(context.tenantId)[dshTemplateKey(application.type,application.status)],payload=buildDshApplicationDecisionMessage({type:application.type,decision:application.status,now:this.now(),...(template?{customMessage:template}:{}),...(application.decisionNote?{decisionNote:application.decisionNote}:{}),...(agreementUrl?{agreementUrl}:{})});
