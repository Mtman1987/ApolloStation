...
import { buildDshApplicationDecisionMessage, dshTemplateKey, type DshApplicationTemplateKeyV1 } from "./application-flow.js";
...
export interface DshApplicationControlOptionsV1 {
  spmtOrigin:string;
  publicOrigin?:string;
  databasePath:string;
...
export class DshApplicationControls {
  ...
  constructor(private readonly options:DshApplicationControlOptionsV1){
    this.store=new SqliteDshApplicationStore(options.databasePath);
    ...
    if(options.credential){
      ...
      const reads=new DshDiscordApi(grants,options.fetchImpl);
      this.discord=new DshSimulationRoomDiscordTransport(reads,client,{guildIds:tenantId=>this.tenant(tenantId).discordGuildIds??[],now:this.now,liveWrites:options.operationMode!=="read-only"});
    }
  }
  ...
  async handle(request:IncomingMessage,response:ServerResponse,url:URL):Promise<boolean>{
    ...
    try{
      ...
      const path=url.pathname.slice("/api/discord-stream-hub/control/applications/".length);
  ...
  private reviews(response:ServerResponse,context:SessionContext){
    const owner=this.isOwner(context),crew=this.canVote(context);if(!owner&&!crew)return sendJson(response,403,{error:"crew_access_required",message:"Recognized Crew access is required to review applications."});
    const applications=this.store.list(context.tenantId,undefined,100);
    if(owner)return sendJson(response,200,{schemaVersion:1,role:"owner",applications,templates:this.store.templates(context.tenantId)});
    const actor=String(context.session.actorId??"");
    return sendJson(response,200,{schemaVersion:1,role:"crew",applications:applications.filter(item=>item.status==="pending").map(item=>({...item,votes:undefined,myVote:item.votes.find(vote=>vote.voterUserId===actor)?.vote??null,decidedByUserId:undefined,decisionNote:undefined,notification:undefined,agreementOffer:undefined,agreementAcceptance:undefined})),templates:{}});
  }

  private vote(response:ServerResponse,context:SessionContext,body:Record<string,unknown>){
    if(!this.canVote(context))throw new Error("Recognized Crew access is required to vote on applications");const vote:DshApplicationVoteValueV1=body.vote==="approve"?"approve":body.vote==="reject"?"reject":(()=>{throw new Error("Vote must be approve or reject")})();
    const result=this.store.vote(context.tenantId,text(body.applicationId,"applicationId",300),{userId:String(context.session.actorId),username:String(context.session.displayName??context.session.username??context.session.actorId)},vote,this.now());return sendJson(response,200,{schemaVersion:1,action:result.action,myVote:result.application.votes.find(item=>item.voterUserId===String(context.session.actorId))?.vote??null});
  }

  private async decide(response:ServerResponse,context:SessionContext,body:Record<string,unknown>){
    const decision=body.decision==="approved"?"approved":body.decision==="rejected"?"rejected":undefined;if(!decision)throw new Error("Application decision is invalid");
    const application=this.store.decide(context.tenantId,text(body.applicationId,"applicationId",300),decision,String(context.session.actorId),optionalText(body.note,1000),this.now());
    const delivered=await this.deliverDecision(context,application,false);return sendJson(response,200,{schemaVersion:1,application:delivered.application,notification:delivered.notification});
  }

  private async notify(response:ServerResponse,context:SessionContext,body:Record<string,unknown>,resend:boolean){
    const application=this.store.get(context.tenantId,text(body.applicationId,"applicationId",300));if(!application||application.status==="pending")throw new Error("Application has no final decision");
    const delivered=await this.deliverDecision(context,application,resend);return sendJson(response,200,{schemaVersion:1,application:delivered.application,notification:delivered.notification});
  }

  private async deliverDecision(context:SessionContext,application:DshApplicationV1,resend:boolean){
    if(!resend&&application.notification?.status==="delivered")return{application,notification:"sent" as const};
    let current=application,agreementUrl:string|undefined;
    if(application.status==="approved"){
      const offer=this.store.createAgreementOffer(context.tenantId,application.id,String(context.session.actorId),this.now());current=offer.application;
      if(!offer.rawToken)throw new Error("Accepted agreements cannot be resent");agreementUrl=this.agreementUrl(application.id,offer.rawToken);
    }
    const template=this.store.templates(context.tenantId)[dshTemplateKey(application.type,application.status)];
    const payload=buildDshApplicationDecisionMessage({type:application.type,decision:application.status,customMessage:template,decisionNote:application.decisionNote,agreementUrl,now:this.now()});
    if(!this.discord){current=this.store.setNotificationResult(context.tenantId,application.id,{status:"failed",error:"Discord delivery is not connected"},this.now());return{application:current,notification:"unavailable" as const};}
    try{const messageId=await this.discord.sendDirectMessage(context.tenantId,application.applicantDiscordId,payload);current=this.store.setNotificationResult(context.tenantId,application.id,{status:"delivered",messageId},this.now());return{application:current,notification:"sent" as const};}
    catch(error){current=this.store.setNotificationResult(context.tenantId,application.id,{status:"failed",error:safeError(error)},this.now());return{application:current,notification:"unavailable" as const};}
  }
  ...
  private async agreementGet(request:IncomingMessage,response:ServerResponse,url:URL,context:SessionContext){
    const applicationId=text(url.searchParams.get("applicationId"),"applicationId",300),token=text(url.searchParams.get("token"),"token",300),application=this.store.get(context.tenantId,applicationId);this.verifyOffer(application,token);
    if(url.searchParams.get("format")==="receipt"){if(!application!.agreementAcceptance)return sendJson(response,404,{error:"receipt_not_found",message:"No acceptance receipt exists."});return jsonDownload(response,application!.agreementAcceptance,`spmt-acceptance-${application!.agreementAcceptance.acceptanceId}.json`);}
    const identity=await this.identity(request,context),acceptance=application!.agreementAcceptance;
    return sendJson(response,200,{schemaVersion:1,applicationId,role:application!.type,applicantDiscordId:application!.applicantDiscordId,document:application!.agreementOffer!.document,acceptanceSchedule:application!.agreementOffer!.acceptanceSchedule,authenticated:true,account:identity,identityMatches:Boolean(identity.discordUserId&&identity.discordUserId===application!.applicantDiscordId),accepted:acceptance?{acceptanceId:acceptance.acceptanceId,acceptedAt:acceptance.acceptedAt,receiptUrl:this.agreementApiUrl(applicationId,token,"receipt")}:null});
  }

  private async agreementAccept(request:IncomingMessage,response:ServerResponse,context:SessionContext,body:Record<string,unknown>){
    const identity=await this.identity(request,context);if(!identity.discordUserId)throw new Error("Link the approved Discord account to SPMT before accepting");
    const result=this.store.acceptAgreement({tenantId:context.tenantId,applicationId:text(body.applicationId,"applicationId",300),token:text(body.token,"token",300),spmtUserId:String(context.session.actorId),discordUserId:identity.discordUserId,username:identity.displayName,reviewedTerms:body.reviewedTerms===true,electronicConsent:body.electronicConsent===true},this.now());return sendJson(response,200,{schemaVersion:1,alreadyAccepted:result.alreadyAccepted,acceptance:result.acceptance,receiptUrl:this.agreementApiUrl(result.application.id,String(body.token),"receipt")});
  }

  private async identity(request:IncomingMessage,context:SessionContext){const snapshot=await fetchAppPlatformSnapshot({appId:"discord-stream-hub",spmtOrigin:this.options.spmtOrigin,request,sources:["providerLinks"],liveRead:null});const links=Array.isArray(snapshot.providerLinks)?snapshot.providerLinks as Array<Record<string,unknown>>:[],link=links.find(item=>item.provider==="discord"&&!item.revokedAt);return{spmtUserId:String(context.session.actorId),discordUserId:link?String(link.providerUserId??""):"",displayName:String(link?.displayName??link?.username??context.session.displayName??context.session.username??context.session.actorId)};}
  ...
  private agreementUrl(applicationId:string,token:string){const origin=cleanOrigin(this.options.publicOrigin??this.options.spmtOrigin),url=new URL("/apps/discord-stream-hub/",origin);url.searchParams.set("agreement",applicationId);url.searchParams.set("token",token);url.hash="applications";return url.toString();}
