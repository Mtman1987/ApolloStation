...
import { buildAppRegistry, type AppRegistryFiltersV1 } from './app-registry.js';
...
const OAUTH_CLIENT_CREDENTIAL_SCOPES_BY_CLIENT: Record<string, string[]> = {
  'streamweaver': ['entitlements:read', 'events:write', 'account-recovery:write', 'chat-tag:blacklist:read'],
  'spacemountain-live': ['xp:write'],
  'discord-stream-hub': ['discord:control', 'athena:write', 'identity:write', 'events:write', 'xp:write'],
};
...
function optionalSessionUserId(req: express.Request) {
  ...
  try {
    ...
    return String(payload.id || '') || undefined;
  } catch {
    return undefined;
  }
}

function appRegistryFilters(query: express.Request['query']): AppRegistryFiltersV1 {
  const first = (value: unknown) => Array.isArray(value) ? value[0] : value;
  return {
    surface: String(first(query.surface) || '').trim() || undefined,
    category: String(first(query.category) || '').trim() || undefined,
    parentAppId: String(first(query.parentAppId || query.parent_app_id) || '').trim() || undefined,
    capability: String(first(query.capability) || '').trim() || undefined,
  };
}

function appRegistryForRequest(req: express.Request) {
  return buildAppRegistry(buildAppsForUser(optionalSessionUserId(req)), appRegistryFilters(req.query));
}

function sendAppRegistry(req: express.Request, res: express.Response) {
  const registry = appRegistryForRequest(req);
  const viewRevision = crypto
    .createHash('sha256')
    .update(JSON.stringify(registry.filters))
    .digest('hex')
    .slice(0, 8);
  const etag = `\"${registry.version}-${registry.revision}-${viewRevision}\"`;
  res.setHeader('ETag', etag);
  res.setHeader('Cache-Control', optionalSessionUserId(req) ? 'private, max-age=0, must-revalidate' : 'public, max-age=15, must-revalidate');
...
app.get('/api/platform/mcp', (_req, res) => {
  res.json({
    ...
    tools: ['spmt.apps.list', 'spmt.apps.get', 'spmt.surfaces.list', 'spmt.components.list', 'spmt.components.register'],
  });
});
...
app.post('/api/mcp', async (req: any, res) => {
  ...
  if (method === 'tools/list') return ok({ tools: [
    { name: 'spmt.apps.list', description: 'List approved apps from the canonical SPMT registry.', inputSchema: { type: 'object', properties: { surface: { type: 'string' }, category: { type: 'string' }, parentAppId: { type: 'string' }, capability: { type: 'string' } }, additionalProperties: false } },
    { name: 'spmt.apps.get', description: 'Get one approved app from the canonical SPMT registry.', inputSchema: { type: 'object', required: ['appId'], properties: { appId: { type: 'string' } }, additionalProperties: false } },
  ...
  if (name === 'spmt.apps.list') return toolResult(buildAppRegistry(buildAppsForUser(), {
    surface: args.surface,
    category: args.category,
    parentAppId: args.parentAppId,
    capability: args.capability,
  }));
  if (name === 'spmt.apps.get') {
    const registry = buildAppRegistry(buildAppsForUser());
    const appRecord = registry.apps.find((item) => item.id === String(args.appId || ''));
    if (!appRecord) return fail(-32602, 'Unknown app');
    return toolResult({ version: registry.version, revision: registry.revision, app: appRecord });
  }
...
app.get('/api/platform/apps/public', authenticatePlatformKey('apps:read'), (req: any, res) => {
  const registry = buildAppRegistry(buildAppsForUser(req.platformKey.userId), appRegistryFilters(req.query));
  res.json({ ...registry, key: req.platformKey });
});
...
app.post('/api/platform/apps/:submissionId/review', authenticate, requirePlatformAdmin, (req: any, res) => {
  ...
  if (status === 'approved') {
    ...
    createPlatformEvent({
      ...
      links: [{ label: 'App registry', url: '/api/apps', kind: 'details' }],
    });
  }
  createNotification(
    row.user_id,
...
app.get('/api/apps', sendAppRegistry);
...
app.get('/api/apps/:appId', (req, res) => {
  ...
  const registry = buildAppRegistry(buildAppsForUser(optionalSessionUserId(req)));
  const app = registry.apps.find((item) => item.id === appId);
  if (!app) return res.status(404).json({ error: 'Unknown app' });
  res.json({ version: registry.version, revision: registry.revision, app });
});
...
// ticket used by the app onboarding flow. No secondary identity is involved.
