...
- `GET /api/apps` — public, versioned `app-registry.v1` discovery; optionally filter by `surface`, `parentAppId`, `category`, or `capability`.
- `GET /api/apps/:appId` — inspect one approved registry entry.
...
The list response carries a stable `revision` and `ETag`. Consumers should send `If-None-Match`, retain a bounded last-known approved snapshot, and visibly mark that snapshot stale when SPMT cannot be reached. Catalog, runtime, and user-access state are separate fields. Approval emits `app.registry.changed`; a stopped app remains discoverable.

The same read contract is available through `client.apps.list()`, `spmt apps list`, and the `spmt.apps.list` MCP tool. The old static `/app-catalog.json` path is only a deprecation pointer and contains no competing app records.
