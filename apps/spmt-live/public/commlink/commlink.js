...
const state = {
  ...
  activeFilter: 'all',
  activeView: 'focus',
  sources: false ? structuredClone(defaultSources) : [],
  sourceHealth: [],
  feedMode: 'loading',
  feedCursor: null,
  feedPollTimer: null,
  ...
  messages: [],
  chatSpaces: structuredClone(accountChatSpaces),
  desks: structuredClone(accountDesks),
...
const DEFAULT_PRESENTATION_CATEGORIES = ['chat', 'activity', 'notification'];
...
const activeSources = () => {
  ...
  if (space.sourceMode === 'all') return [...state.sources];
...
function messageBelongsToSpace(message, space, sourceIds, aggregateProviders, hiddenSourceIds = null) {
  ...
  if (space.sourceMode === 'all') return true;
  ...
  $('#context-content').innerHTML = `
    ...
    <section class="context-section">
      ...
      <div class="capability-list">
        ...
        <div class="capability"><span>Connection</span><span>${state.feedMode === 'real' ? 'Live contract' : 'Unavailable'}</span></div>
      </div>
    </section>
    <section class="context-section">
...
function renderDesk() {
  ...
  const deskSourceIds = new Set(panels.flatMap((panel) => {
    ...
    return normalized.sourceMode === 'all'
  ...
  $('#desk-grid').innerHTML = panels.map((panel, index) => {
    ...
    if (!space) return '';
    const normalized = normalizedSpace(space);
    const sourceIds = new Set((normalized.sourceMode === 'all'
      ? state.sources.map((source) => source.id)
...
function normalizeFeedItem(item) {
  ...
  return {
    ...
    sourceId: canonicalCommlinkSourceId(provider, item),
    provider,
...
function applyFeedSources(payload) {
  ...
  const channelSources = (Array.isArray(payload.channels) ? payload.channels : []).map((channel) => {
    ...
    return {
      id: canonicalCommlinkSourceId(provider, channel),
      provider,
      channel: humanChannelLabel(provider, channel.channelName || channel.displayName || channel.channelId || channel.sourceName || 'Unknown channel'),
  ...
  const retainedIds = new Set(state.chatSpaces.flatMap((space) => [
    ...(space.sources || []),
  ...
  const eventSources = (Array.isArray(payload.items) ? payload.items : []).map((item) => {
    const provider = canonicalProvider(item);
    const sourceId = canonicalCommlinkSourceId(provider, item);
...
function scheduleFeedPoll() {
  ...
  if (state.feedMode === 'signed-out' || state.searchQuery || state.replayActive) return;
...
async function saveCommlinkWorkspace(create = false) {
  ...
  try {
    ...
    const response = await fetch('/api/app-state/cosmo-commlink/workspace', {
      ...
      headers,
      credentials: 'include',
      body: JSON.stringify({ schemaVersion: 4, data: currentWorkspaceData() }),
    });
