...
async function apps(positional, flags) {
  const context = await projectContext(flags);
  const client = new SpaceMountainClient({ apiKey: context.apiKey, appId: context.manifest?.appId, baseUrl: context.baseUrl });
  const action = positional[1] || 'list';
  if (action === 'get') {
    const appId = String(positional[2] || flags.id || '').trim();
    if (!appId) throw new Error('app id required, for example: spmt apps get streamweaver');
    console.log(JSON.stringify(await client.apps.get(appId), null, 2));
    return;
  }
  if (action !== 'list') throw new Error('apps action must be list or get');
  const result = await client.apps.list({
    surface: flags.surface ? String(flags.surface) : undefined,
    category: flags.category ? String(flags.category) : undefined,
    parentAppId: flags['parent-app-id'] ? String(flags['parent-app-id']) : undefined,
    capability: flags.capability ? String(flags.capability) : undefined,
  });
...
Commands:
  ...
  spmt apps list              List the canonical app registry
  spmt apps get <app-id>      Inspect one approved app
...
try {
  ...
  else if (command === 'apps' || command === 'app') await apps(positional, flags);
