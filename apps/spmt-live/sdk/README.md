...
This is the `@spmt/sdk` partner SDK and `spmt` developer CLI. Version 0.4.0 is distributed from the canonical SPMT mirror; the older npm tag remains available only for legacy consumers.
...
```powershell
npm exec --yes --package=https://spmt.live/sdk/spmt-sdk-0.4.0.tgz -- spmt install
```
...
```bash
npm exec --yes --package=https://spmt.live/sdk/spmt-sdk-0.4.0.tgz -- spmt event server.status --data-file status.json
```
...
const registry = await spmt.apps.list({ surface: "spacemountain" });
console.log(registry.revision, registry.apps);
```

The registry keeps approval (`catalogState`), service readiness (`runtimeState`), and tenant install/access (`accessState`) separate. It can also be inspected without writing code:

```bash
npx spmt apps list --surface spacemountain
npx spmt apps get streamweaver
```

Agent integrations use the same logical operation through `spmt.apps.list` and `spmt.apps.get` at `/api/mcp`.

## Modules

- `identity`
- `apps`
  - `list({ surface, parentAppId, category, capability })` returns the versioned canonical registry and revision.
  - `get(appId)` returns one approved registry record.
- `developer`
- `surfaces`
