// Game rules adapted from deployed Nebula revision 42cb6401; no provider or session dependencies.
import { quackverseCards, type QuackverseCard } from './quackverse-data.js';
import { chooseQuackverseNpcAction } from './quackverse-npc.js';
import { findQuackverseStructuredEffect, getQuackverseAbilityCost, summarizeQuackverseGear } from './quackverse-effects.js';
import { normalizeQuackverseState, QUACKVERSE_GRID_SIZE as quackverseGridSize, QUACKVERSE_SQUAD_SIZE as quackverseSquadSize, type QuackverseBattlePileState, type QuackverseCardInstance, type QuackversePlayerId, type QuackverseSavedPiece, type QuackverseSavedState } from './quackverse-state.js';
...
export function recordDeckResult(state: QuackverseSavedState, winner: PlayerId) {
...
export function detectFormations(grid: Array<QuackverseSavedPiece | null>): DetectedFormation[] {
...
export function scoreNewFormations(state: QuackverseSavedState, owner: PlayerId) {
...
export function refreshTurnResources(state: QuackverseSavedState, owner: PlayerId) {
...
export function getEffectiveStats(piece: QuackverseSavedPiece) {
  const card = cardFor(piece);
  const equipment = (piece.equipmentIds || []).map((id) => quackverseCards.find((item) => item.id === id)).filter(Boolean) as QuackverseCard[];
...
export function resolveAbility(state: QuackverseSavedState, sourceIndex: number, requestedAbility?: string) {
  ...
  if(!sourceCard.abilities.includes(ability))throw new Error('Choose an ability printed on that duck.');
...
export function endTurn(state: QuackverseSavedState, actedOverride?: boolean) {
...
export function loadMockGame(state: QuackverseSavedState) {
...
export function resetMatchState(state: QuackverseSavedState, clearSeats = true) {
...
export type QuackverseGameAction = { type: string; playerId?: PlayerId; userId?: string; from?: number; to?: number; targetIndex?: number; attackerIndex?: number; sourceIndex?: number; instanceId?: string; ability?: string };
function getClaimedSeat(state: QuackverseSavedState, userId: string): PlayerId | null { return playerIds.find(id => state.claimedPlayers[id] === userId) ?? null; }
function ensureClaimedSeat(state: QuackverseSavedState, userId: string): PlayerId | null { const existing=getClaimedSeat(state,userId); if(existing)return existing; const seat=playerIds.find(id=>!state.claimedPlayers[id]&&!state.npcPlayers[id]);if(seat)state.claimedPlayers[seat]=userId;return seat??null; }
export function applyQuackverseGameAction(input: QuackverseSavedState, action: QuackverseGameAction, actor: {userId: string; moderator?: boolean}, now = new Date()): QuackverseSavedState {
 const state=normalizeQuackverseState(input,now), body={...action}, actionType=body.type, requestUserId=actor.userId, adminRequest=actor.moderator===true, allowLocalSetup=false;
 if(!requestUserId)throw new Error('A signed-in player is required.');
 if(!['claimSeat','leaveSeat','clearSeat','endMatch','reset','loadMockGame','toggleNpc','setClaimedPlayer','move','place','attack','useAbility','attachGear','pass'].includes(actionType))throw new Error('Unknown Quackverse action');
 const before=JSON.stringify(state);
    ...
    if (state.winner && !explicitSeatAction) throw new Error('The match is complete. Reset or end the match first.');
    ...
    const reject = (error: string, _status = 403): never => { throw new Error(error); };
    ...
    if (body.type === 'move') {
      ...
      if (piece && piece.owner === state.activePlayer && Number.isSafeInteger(to) && to >= 0 && to < state.grid.length && isAdjacent(from,to) && !state.grid[to] && !state.turnActions[piece.owner].deployedOrMoved) {
 ...
 if(JSON.stringify(state)===before && !['toggleNpc','setClaimedPlayer'].includes(actionType))throw new Error('That action is not legal. Check your hand, turn, target and available actions.');
