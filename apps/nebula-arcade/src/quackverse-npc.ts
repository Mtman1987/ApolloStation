// Adapted from the deployed Nebula source, revision 42cb6401.
import { quackverseCards, type QuackverseCard } from './quackverse-data.js';
import {
  QUACKVERSE_GRID_SIZE as quackverseGridSize,
...
export function chooseQuackverseNpcAction(state: QuackverseSavedState): QuackverseNpcAction {
  ...
  if (!actions.deployedOrMoved) {
    ...
    if (duck && openEntrySquares.length > 0) {
      ...
      return { type: 'place', targetIndex: targetIndex!, instanceId: duck.entry.instanceId };
    }
  }
