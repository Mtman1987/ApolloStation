...
import { assertChatTagStateV1, type ChatTagInboundMessageV1, type ChatTagPlayerStateV1, type ChatTagStateV1 } from "./chat-tag.js";
import { ChatTagRuntime, type StoredChatTagCommandV1, type ChatTagDeliveryReportV1 } from "./chat-tag-runtime.js";

export interface ChatTagChannelSettingsV1 {
  schemaVersion: 1;
  tenantId: string;
  channelId: string;
  overlayMode: boolean;
...
export interface ChatTagSupportTicketV1 {
  schemaVersion: 1;
  ticketId: string;
  tenantId: string;
  channelId: string;
...
export interface ChatTagOverlayMessageV1 {
  schemaVersion: 1;
  sequence: number;
  tenantId: string;
  channelId: string;
...
export interface ChatTagExperienceStore {
  getChannelSettings(tenantId: string, channelId: string): ChatTagChannelSettingsV1;
  setOverlayMode(tenantId: string, channelId: string, enabled: boolean, occurredAt: string): ChatTagChannelSettingsV1;
  optOutChannel(tenantId: string, channelId: string, occurredAt: string): ChatTagChannelSettingsV1;
  createSupportTicket(input: Omit<ChatTagSupportTicketV1, "schemaVersion" | "status" | "resolvedAt">): ChatTagSupportTicketV1;
  listSupportTickets(tenantId: string, status?: ChatTagSupportTicketV1["status"]): ChatTagSupportTicketV1[];
  enqueueOverlayMessage(input: Omit<ChatTagOverlayMessageV1, "schemaVersion" | "sequence">): ChatTagOverlayMessageV1;
  listOverlayMessages(tenantId: string, channelId: string, afterSequence?: number, limit?: number): ChatTagOverlayMessageV1[];
}

export class SqliteChatTagExperienceStore implements ChatTagExperienceStore {
  private readonly db: DatabaseSync;

  constructor(path: string) {
    if (!path) throw new Error("Chat Tag experience database path is required");
  ...
  getChannelSettings(tenantId: string, channelId: string): ChatTagChannelSettingsV1 {
    ...
    const row = this.db.prepare("SELECT overlay_mode, opted_out, updated_at FROM chat_tag_channels WHERE tenant_id=? AND channel_id=?").get(tenantId, channelId) as { overlay_mode: number; opted_out: number; updated_at: string } | undefined;
  ...
  createSupportTicket(input: Omit<ChatTagSupportTicketV1, "schemaVersion" | "status" | "resolvedAt">): ChatTagSupportTicketV1 {
    ...
    const ticket: ChatTagSupportTicketV1 = { schemaVersion: 1, ...input, note: input.note?.slice(0, 500) ?? null, status: "open", resolvedAt: null };
    this.db.prepare("INSERT OR IGNORE INTO chat_tag_support_tickets(ticket_id,tenant_id,channel_id,requester_user_id,requester_username,note,status,created_at,resolved_at) VALUES(?,?,?,?,?,?,?, ?,NULL)")
  ...
  listSupportTickets(tenantId: string, status?: ChatTagSupportTicketV1["status"]): ChatTagSupportTicketV1[] {
    ...
    const rows = (status
      ? this.db.prepare("SELECT * FROM chat_tag_support_tickets WHERE tenant_id=? AND status=? ORDER BY created_at DESC LIMIT 200").all(tenantId, status)
      : this.db.prepare("SELECT * FROM chat_tag_support_tickets WHERE tenant_id=? ORDER BY created_at DESC LIMIT 200").all(tenantId)) as TicketRow[];
  ...
  enqueueOverlayMessage(input: Omit<ChatTagOverlayMessageV1, "schemaVersion" | "sequence">): ChatTagOverlayMessageV1 {
    ...
    const result = this.db.prepare("INSERT INTO chat_tag_overlay_messages(tenant_id,channel_id,code,text,created_at) VALUES(?,?,?,?,?)").run(input.tenantId, input.channelId, input.code, input.text.slice(0, 1_000), input.createdAt);
    return { schemaVersion: 1, sequence: Number(result.lastInsertRowid), ...input, text: input.text.slice(0, 1_000) };
  }

  listOverlayMessages(tenantId: string, channelId: string, afterSequence = 0, limit = 50): ChatTagOverlayMessageV1[] {
    requireId(tenantId, "tenantId"); requireId(channelId, "channelId");
    ...
    const rows = this.db.prepare("SELECT sequence,tenant_id,channel_id,code,text,created_at FROM chat_tag_overlay_messages WHERE tenant_id=? AND channel_id=? AND sequence>? ORDER BY sequence LIMIT ?").all(tenantId, channelId, afterSequence, limit) as Array<{ sequence: number; tenant_id: string; channel_id: string; code: string; text: string; created_at: string }>;
  ...
  private writeChannel(tenantId: string, channelId: string, occurredAt: string, patch: { overlayMode?: boolean; optedOut?: boolean }): ChatTagChannelSettingsV1 {
    ...
    this.db.prepare("INSERT INTO chat_tag_channels(tenant_id,channel_id,overlay_mode,opted_out,updated_at) VALUES(?,?,?,?,?) ON CONFLICT(tenant_id,channel_id) DO UPDATE SET overlay_mode=excluded.overlay_mode,opted_out=excluded.opted_out,updated_at=excluded.updated_at")
      .run(tenantId, channelId, Number(next.overlayMode), Number(next.optedOut), occurredAt);
    return next;
  }

  private getTicket(ticketId: string): ChatTagSupportTicketV1 | undefined {
    const row = this.db.prepare("SELECT * FROM chat_tag_support_tickets WHERE ticket_id=?").get(ticketId) as TicketRow | undefined;
    return row ? ticketFromRow(row) : undefined;
  }

  private migrate(): void {
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS chat_tag_channels(tenant_id TEXT NOT NULL,channel_id TEXT NOT NULL,overlay_mode INTEGER NOT NULL DEFAULT 0,opted_out INTEGER NOT NULL DEFAULT 0,updated_at TEXT NOT NULL,PRIMARY KEY(tenant_id,channel_id)) STRICT;
      CREATE TABLE IF NOT EXISTS chat_tag_support_tickets(ticket_id TEXT PRIMARY KEY,tenant_id TEXT NOT NULL,channel_id TEXT NOT NULL,requester_user_id TEXT NOT NULL,requester_username TEXT NOT NULL,note TEXT,status TEXT NOT NULL,created_at TEXT NOT NULL,resolved_at TEXT) STRICT;
      CREATE INDEX IF NOT EXISTS chat_tag_support_status ON chat_tag_support_tickets(tenant_id,status,created_at);
      CREATE TABLE IF NOT EXISTS chat_tag_overlay_messages(sequence INTEGER PRIMARY KEY AUTOINCREMENT,tenant_id TEXT NOT NULL,channel_id TEXT NOT NULL,code TEXT NOT NULL,text TEXT NOT NULL,created_at TEXT NOT NULL) STRICT;
      CREATE INDEX IF NOT EXISTS chat_tag_overlay_feed ON chat_tag_overlay_messages(tenant_id,channel_id,sequence);
    `);
  }
}

export interface ChatTagPresenceV1 { liveUserIds?: string[]; channelByUserId?: Record<string, string>; }
export interface ChatTagDirectoryPageV1 { kind: "players" | "live"; page: number; totalPages: number; playerCount: number; liveCount: number; chattingCount: number; entries: string[]; message: string; }

export function buildChatTagDirectoryPage(stateValue: ChatTagStateV1, input: { kind: "players" | "live"; page?: number; now: string; presence?: ChatTagPresenceV1; maxCharacters?: number }): ChatTagDirectoryPageV1 {
  const state = assertChatTagStateV1(stateValue); const nowMs = Date.parse(input.now); requireIso(input.now, "now");
...
export function getChatTagPinRanking(stateValue: ChatTagStateV1, pinUserId: string, limit = 5): Array<{ userId: string; username: string; count: number }> {
  const state = assertChatTagStateV1(stateValue); requireId(pinUserId, "pinUserId");
...
export type ChatTagExperienceOutcomeV1 =
  | { kind: "ignored"; code: string }
  | { kind: "reply"; code: string; message: string; route: "chat" | "overlay" }
  | { kind: "executed"; code: string; message: string; route: "chat" | "overlay"; execution: StoredChatTagCommandV1 & { kind: "result"; delivery: ChatTagDeliveryReportV1 } };

export class ChatTagExperienceService {
  private readonly cursors = new Map<string, { kind: "players" | "live"; page: number; expiresAt: number }>();
  constructor(private readonly runtime: ChatTagRuntime, private readonly experience: ChatTagExperienceStore, private readonly pinUserId: string, private readonly now: () => string = () => new Date().toISOString()) {}

  async ingest(message: ChatTagInboundMessageV1, presence: ChatTagPresenceV1 = {}): Promise<ChatTagExperienceOutcomeV1> {
    ...
    if (parsed?.kind === "optout") {
      ...
      return { kind: "reply", code: "channel-opted-out", message: "This channel is now permanently opted out of Chat Tag.", route: "chat" };
    }
    ...
    if (parsed?.kind === "players" || parsed?.kind === "live" || parsed?.kind === "more") {
      ...
      const result = buildChatTagDirectoryPage(this.runtime.getState(message.tenantId).state, { kind, page, now: message.occurredAt, presence });
    ...
    if (parsed?.kind === "pinrank") {
      const ranking = getChatTagPinRanking(this.runtime.getState(message.tenantId).state, this.pinUserId);
  ...
  private reply(settings: ChatTagChannelSettingsV1, message: ChatTagInboundMessageV1, code: string, text: string, forceRoute?: "chat" | "overlay"): Extract<ChatTagExperienceOutcomeV1, { kind: "reply" }> {
