...
export function saveQuackverseDeck(collection:QuackverseCollectionState,input:{deckId?:string;name?:string;cardIds?:readonly number[]},now=new Date()){
  if(!collection.savedDecks.some(deck=>deck.id===input.deckId)&&collection.savedDecks.length>=12)throw new Error("You can save up to 12 decks. Remove one first.");
...
export function activateQuackverseDeck(collection:QuackverseCollectionState,deckId:string){if(deckId==="default"){collection.activeDeckId="default";return collection.deck;}const saved=collection.savedDecks.find((deck)=>deck.id===deckId);if(!saved)throw new Error("Quackverse saved deck not found");for(const id of new Set(saved.cardIds)){if(saved.cardIds.filter(value=>value===id).length>collection.cards.filter(value=>value===id).length)throw new Error("The collection no longer owns every card in that deck.");}collection.deck=[...saved.cardIds];collection.activeDeckId=saved.id;return collection.deck;}
