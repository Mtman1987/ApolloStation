...
export function createChatTagSandboxHost(options: ChatTagSandboxHostOptions) {
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/health/ready") return json(response, 200, { ready: true, app: "chat-tag", runtimeMode: "sandbox", outboundIntegrations: "disabled", buildSha: options.buildSha ?? "dev" });
...
function renderSandboxPage(nonce: string, tenantId: string, channelId: string) { return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>Chat Tag Green Sandbox</title><link rel="stylesheet" href="/assets/chat-tag-sandbox.css"><script src="/assets/chat-tag-sandbox.js" defer nonce="${nonce}"></script></head><body><header><div><b>CHAT TAG · GREEN</b><span>private Sprite sandbox · outbound providers off</span></div><a href="/v1/nebula/chat-tag/overlay" target="_blank" rel="noreferrer">Open OBS output</a></header><main><section class="hero"><p>FIRST COMPLETE APP COHORT</p><h1>Play the original persistent Chat Tag.</h1><span>Tenant ${escapeHtml(tenantId)} · Channel ${escapeHtml(channelId)}</span></section><section class="console"><form id="command"><label>User ID<input name="userId" value="player-alpha" required></label><label>Username<input name="username" value="Alpha" required></label><label>Role<select name="role"><option value="member">Player</option><option value="moderator">Moderator</option><option value="broadcaster">Broadcaster</option></select></label><label class="wide">Chat message<input name="text" value="spmt join" required></label><button>Send</button></form><div class="quick"><button data-command="spmt join">Join</button><button data-command="spmt status">Status</button><button data-command="spmt score">Score</button><button data-command="spmt players">Players</button><button data-command="spmt live">Live</button><button data-command="spmt sleep">Sleep</button><button data-command="spmt wake">Wake</button><button data-command="spmt pinrank">Pin rank</button></div><pre id="reply" aria-live="polite">Ready.</pre></section><section class="state"><div><span>Current IT</span><strong id="it">—</strong></div><div><span>Players</span><strong id="players">0</strong></div><div><span>Last tag</span><strong id="last">—</strong></div></section><section><h2>Leaderboard</h2><ol id="leaderboard"></ol></section></main></body></html>`; }

const SANDBOX_CSS = `:root{color-scheme:dark;font-family:Inter,ui-sans-serif,system-ui;background:#050816;color:#eef6ff}*{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(circle at 15% 0,#28134d 0,transparent 35%),radial-gradient(circle at 95% 20%,#00364a 0,transparent 32%),#050816}header{display:flex;justify-content:space-between;align-items:center;padding:1rem 4vw;border-bottom:1px solid #2d426d;background:#071020dd;position:sticky;top:0;backdrop-filter:blur(14px)}header div{display:flex;flex-direction:column}header b,.hero p{color:#54e5ff;letter-spacing:.14em}header span{color:#9cb0cf;font-size:.8rem}a,button{color:#fff;background:#6f3cff;border:1px solid #a78bfa;border-radius:.7rem;padding:.7rem 1rem;text-decoration:none;cursor:pointer}main{width:min(1050px,92vw);margin:3rem auto}.hero h1{font-size:clamp(2.2rem,7vw,5.8rem);line-height:.9;margin:.4rem 0 1rem;max-width:900px}.hero span{color:#9cb0cf}.console{margin:3rem 0;padding:1.4rem;border:1px solid #355080;border-radius:1.2rem;background:#091327dd;box-shadow:0 0 50px #5c24c522}form{display:grid;grid-template-columns:1fr 1fr 1fr;gap:1rem}label{display:flex;flex-direction:column;gap:.4rem;color:#9cb0cf;font-size:.8rem}.wide{grid-column:1/4}input,select{width:100%;background:#030816;color:#fff;border:1px solid #334b76;border-radius:.6rem;padding:.8rem;font:inherit}form button{grid-column:1/4}.quick{display:flex;flex-wrap:wrap;gap:.5rem;margin:1rem 0}.quick button{background:#0c203b;border-color:#31537f;padding:.5rem .75rem}pre{white-space:pre-wrap;min-height:4rem;padding:1rem;border-radius:.7rem;background:#02050d;color:#7fffd4}.state{display:grid;grid-template-columns:repeat(3,1fr);gap:1rem}.state div{padding:1.2rem;border:1px solid #253d66;border-radius:1rem;background:#081121}.state span{display:block;color:#8ca1c4;font-size:.75rem;text-transform:uppercase}.state strong{font-size:1.4rem}ol{display:grid;gap:.6rem;padding-left:1.5rem}li{padding:.7rem 1rem;background:#0a162a;border-radius:.6rem}@media(max-width:650px){form,.state{grid-template-columns:1fr}.wide,form button{grid-column:auto}header{align-items:flex-start;gap:1rem}header a{font-size:.75rem}}`;
