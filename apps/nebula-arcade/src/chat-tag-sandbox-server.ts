...
function renderSandboxPage(nonce: string, tenantId: string, channelId: string) { const games = NEBULA_ARCADE_GAMES.map((game) => `<article data-game="${escapeHtml(game.id)}"><span>COMMUNITY GAME</span><h2>${escapeHtml(game.name)}</h2><small>${game.commands.map((command) => `!${escapeHtml(command)}`).join(" · ")}</small></article>`).join(""); return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>Nebula Arcade · Games Hub</title><link rel="stylesheet" href="/assets/chat-tag-sandbox.css"><script src="/assets/chat-tag-sandbox.js" defer nonce="${nonce}"></script></head><body class="nebula-arcade-surface spmt-product-surface"><div class="spmt-product-backdrop" aria-hidden="true"><span class="spmt-product-backdrop-image"></span><span class="spmt-product-backdrop-tint"></span><span class="spmt-product-backdrop-shade"></span><span class="spmt-star-layer"><i></i><i></i><i></i></span></div><header><div><b>NEBULA ARCADE</b><span>Games Hub · private Sprite sandbox · outbound providers off</span></div><a href="/v1/nebula/chat-tag/overlay" target="_blank" rel="noreferrer">Open Chat Tag OBS output</a></header><main><section class="hero"><p>GAMES HUB · 20 EQUAL TITLES</p><h1>Choose what your community plays.</h1><span>Tenant ${escapeHtml(tenantId)} · Channel ${escapeHtml(channelId)}</span></section><section class="games" aria-label="Nebula Arcade game catalog">${games}</section><section class="console"><header><div><b>CHAT TAG</b><span>Selected game console</span></div></header><form id="command"><label>User ID<input name="userId" value="player-alpha" required></label><label>Username<input name="username" value="Alpha" required></label><label>Role<select name="role"><option value="member">Player</option><option value="moderator">Moderator</option><option value="broadcaster">Broadcaster</option></select></label><label class="wide">Chat message<input name="text" value="!join" required></label><button>Send</button></form><div class="quick"><button data-command="!join">Join</button><button data-command="spmt status">Status</button><button data-command="spmt score">Score</button><button data-command="spmt players">Players</button><button data-command="spmt live">Live</button><button data-command="spmt sleep">Sleep</button><button data-command="spmt wake">Wake</button><button data-command="spmt pinrank">Pin rank</button></div><pre id="reply" aria-live="polite">Ready.</pre></section><section class="state"><div><span>Current IT</span><strong id="it">—</strong></div><div><span>Players</span><strong id="players">0</strong></div><div><span>Last tag</span><strong id="last">—</strong></div></section><section><h2>Chat Tag leaderboard</h2><ol id="leaderboard"></ol></section></main></body></html>`; }

const SANDBOX_CSS = `${PRODUCT_UI_CSS}:root{color-scheme:dark;font-family:Inter,ui-sans-serif,system-ui;background:#050816;color:#eef6ff}*{box-sizing:border-box}.nebula-arcade-surface{--spmt-accent:#9b5cff;--spmt-accent-secondary:#63ddff;--spmt-stars:.82;margin:0;min-height:100vh;background:#050816}.nebula-arcade-surface .spmt-product-backdrop-image{background-image:url('/assets/nebula-arcade/solar-system.webp');background-position:center;background-size:cover}.nebula-arcade-surface .spmt-product-backdrop-tint{opacity:.25}.nebula-arcade-surface .spmt-product-backdrop-shade{background:linear-gradient(rgba(3,4,12,.16),rgba(3,4,12,.74))}body>header{display:flex;justify-content:space-between;align-items:center;padding:1rem 4vw;border-bottom:1px solid #5e53a8;background:#071020d9;position:sticky;top:0;z-index:5;backdrop-filter:blur(14px)}header div{display:flex;flex-direction:column}header b,.hero p,.games span{color:#7de9ff;letter-spacing:.14em}header span{color:#b6b8d5;font-size:.8rem}a,button{color:#fff;background:#6f3cff;border:1px solid #a78bfa;border-radius:.7rem;padding:.7rem 1rem;text-decoration:none;cursor:pointer}main{position:relative;z-index:1;width:min(1180px,92vw);margin:3rem auto}.hero h1{font-size:clamp(2.2rem,7vw,5.8rem);line-height:.9;margin:.4rem 0 1rem;max-width:900px}.hero span{color:#c2c4dd}.games{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.8rem;margin:3rem 0}.games article{min-height:135px;padding:1rem;border:1px solid #6e64a9;border-radius:1rem;background:#091327c9;backdrop-filter:blur(16px)}.games span{font-size:.62rem}.games h2{margin:.65rem 0;font-size:1.1rem}.games small{color:#b4b5d0;font-size:.68rem;line-height:1.5}.console{margin:3rem 0;padding:1.4rem;border:1px solid #6759a5;border-radius:1.2rem;background:#091327dd;box-shadow:0 0 50px #5c24c544;backdrop-filter:blur(18px)}.console>header{margin:-1.4rem -1.4rem 1.4rem;padding:1rem 1.4rem;border-bottom:1px solid #6759a5}form{display:grid;grid-template-columns:1fr 1fr 1fr;gap:1rem}label{display:flex;flex-direction:column;gap:.4rem;color:#b6b8d5;font-size:.8rem}.wide{grid-column:1/4}input,select{width:100%;background:#030816;color:#fff;border:1px solid #595287;border-radius:.6rem;padding:.8rem;font:inherit}form button{grid-column:1/4}.quick{display:flex;flex-wrap:wrap;gap:.5rem;margin:1rem 0}.quick button{background:#0c203b;border-color:#4a5688;padding:.5rem .75rem}pre{white-space:pre-wrap;min-height:4rem;padding:1rem;border-radius:.7rem;background:#02050d;color:#7fffd4}.state{display:grid;grid-template-columns:repeat(3,1fr);gap:1rem}.state div{padding:1.2rem;border:1px solid #555080;border-radius:1rem;background:#081121d9;backdrop-filter:blur(14px)}.state span{display:block;color:#aaaac6;font-size:.75rem;text-transform:uppercase}.state strong{font-size:1.4rem}ol{display:grid;gap:.6rem;padding-left:1.5rem}li{padding:.7rem 1rem;background:#0a162ae8;border-radius:.6rem}@media(max-width:900px){.games{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:650px){form,.state,.games{grid-template-columns:1fr}.wide,form button{grid-column:auto}body>header{align-items:flex-start;gap:1rem}header a{font-size:.75rem}}`;
