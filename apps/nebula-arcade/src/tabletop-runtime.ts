...
export class SqliteNebulaTabletopRuntime {
  ...
  execute(message: NormalizedChatMessageV1): string | undefined {
    ...
    try {
      ...
      if(game === "bingo") {
        ...
        else if(action === "claim") { const square = Number(args[0]); if(!Number.isInteger(square)||square<1||square>25)throw new Error("Use spmt claim <square 1–25>.");const result=claimPersonalBingoSquare(state,user,square-1,{actorUserId:user},now);response=result.won?`${message.actor.username} has Bingo!`:`Bingo square ${square} marked.`; }
        ...
        else if(action === "ready") {const seat=this.seat(state,user);if(state.grid.some(Boolean)||state.turnNumber>1)throw new Error("A battle is already in progress. Finish it before readying again.");const collection=normalizeQuackverseCollection(state.collections[user],now);if(collection.deck.length<5)throw new Error("Add at least five owned cards to your deck before readying.");state.squads[seat]=collection.deck.filter(id=>catalog.get(id)?.type==="duck").slice(0,5);if(!state.squads[seat].length)throw new Error("Your deck needs at least one duck.");const pile=collection.deck.map((cardId,index)=>({cardId,instanceId:`${seat}:${index}:${cardId}`}));state.battlePiles[seat]={hand:pile.slice(0,5),drawPile:pile.slice(5),discardPile:[]};response=`${seat} is ready. Hand: ${state.battlePiles[seat].hand.map(card=>card.cardId).join(", ")}. Deploy on your back row with spmt quackverse deploy <card id> <A1–G7>.`;}
        else if(["deploy","move","attack","end"].includes(action)) {const playerId=this.seat(state,user);if(!state.claimedPlayers.playerOne||!state.claimedPlayers.playerTwo||!state.squads.playerOne.length||!state.squads.playerTwo.length)throw new Error("Both players must join and ready their decks before battling.");if(action==="deploy"&&(!state.battlePiles[playerId].hand.some(card=>card.cardId===Number(args[0]))||catalog.get(Number(args[0]))?.type!=="duck"))throw new Error("Deploy a duck currently in your hand. Use spmt quackverse hand to see your cards.");const result=applyQuackverseBattleAction(state,action==="end"?{kind:"end-turn",playerId}:action==="deploy"?{kind:"deploy",playerId,cardId:Number(args[0]),to:cell(args[1])}:{kind:action as "move"|"attack",playerId,from:cell(args[0]),to:cell(args[1])},catalog,now);state=result.state;response=result.events.map(event=>event.message).join(" ")||`Turn ${state.turnNumber}: ${state.activePlayer}.`;}
  ...
  observeBingo(message:NormalizedChatMessageV1){const parsed=parseNebulaMessage(message.text);if(!parsed||!parsed.body||message.actor.isBot)return;this.db.exec("BEGIN IMMEDIATE");try{const stored=this.read(message.tenantId,message.channelId,"bingo") as BingoStateV1|undefined;if(!stored){this.db.exec("COMMIT");return;}const state=normalizeBingoState(stored),text=(parsed.gameId?`${parsed.command} ${parsed.args.join(" ")}`:parsed.body).trim().toLowerCase();for(const user of Object.keys(state.personalBoards)){const board=state.personalBoards[user]!;state.templatePhrases.forEach((phrase,index)=>{const value=index===12?board.centerPhrase:phrase;if(value&&text.includes(value.toLowerCase()))claimPersonalBingoSquare(state,user,index,{actorUserId:message.actor.canonicalUserId||message.actor.providerUserId},new Date(message.occurredAt));});}this.put(message.tenantId,message.channelId,"bingo",state);this.db.exec("COMMIT");}catch(error){this.db.exec("ROLLBACK");throw error;}}
}
