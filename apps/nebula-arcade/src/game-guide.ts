...
const details: Record<string, { rules: string; actions?: string[]; answer?: string }> = {
  ...
  quackverse: { rules: "Open up to four packs per day, collect cards and build a deck of up to 20 owned cards. Claim one of two battle seats and ready at least five cards, including a duck. Collections and up to twelve saved decks follow you across chats. Deploy, move, attack, equip owned gear and use a printed ability on the A1–G7 board. Form Battle Lines, Eclipse Crosses, Cosmic Diamonds, Flying Vs or Medic Sanctuaries. Reach 6 VP to win, with up to 3 formation VP. NPC practice and match reset preserve collections.", actions: ["pack", "quackpack", "collection", "deck", "deck add <card id>", "deck remove <card id>", "join", "ready", "hand", "deploy <card id> <cell>", "move <from> <to>", "attack <from> <to>", "useAbility <cell> <ability number>", "attachGear <card id> <cell>", "saveDeck <name>", "activateDeck <deck id>", "deleteDeck <deck id>", "decks", "claimSeat <1 or 2>", "leave", "npc <1 or 2>", "endMatch", "loadMockGame", "end", "status", "reset"] },
  bingo: { rules: "Each player has a personal 5×5 board: 24 shared phrases and a personal center phrase. Joined players' spmt messages can mark matching phrases; ordinary chat is ignored. Each stream can contribute one square per personal card. Claim a square from 1–25 for 1 Game Point and 1 score point. Complete a row, column or diagonal for one win and 5 additional points. Cards follow you across chats. Moderators may replace the 24 shared phrases or reset progress.", actions: ["join", "card", "center <phrase>", "claim <1–25>", "phrases", "phrases <24 phrases separated by |>", "edit <square> <phrase>", "generate", "share", "reset", "leave", "status"], answer: "<phrase>" },
...
export function nebulaGuideReplies(text: string, activeGameIds: readonly string[], publicOrigin?: string): string[] | undefined {
  ...
  if(!parsed.gameId)replies.push("spmt join joins the player pool across chats; spmt leave leaves it. spmt <game> leave skips one game. spmt score shows active-game scores; spmt points shows your wallet; spmt leaderboard shows Game Points leaders.");
