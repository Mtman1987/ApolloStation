...
import { createChatTagSandboxHost as createLegacyChatTagSandboxHost, validateChatTagSandboxEnvironment, type ChatTagSandboxHostOptions } from "./legacy-chat-tag-sandbox-server.js";
...
export function createChatTagSandboxHost(options: ChatTagSandboxHostOptions) {
  const legacy = createLegacyChatTagSandboxHost({ ...options, port: 0, host: "127.0.0.1" });
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (request.method === "GET" && (url.pathname === APP_PATH || url.pathname === "/") && url.searchParams.get("surface") !== "overlay") return proxyLegacy(request, response, legacyPort, true);
      return proxyLegacy(request, response, legacyPort, false);
  ...
  return {
    server,
    async listen() { await legacy.listen(); const address = legacy.server.address(); if (!address || typeof address === "string") throw new Error("Legacy Chat Tag host did not bind a TCP port"); legacyPort = address.port; await listen(server, options.port ?? 8080, options.host ?? "0.0.0.0"); },
    async close() { if (server.listening) await close(server); mixes.close(); actions.close(); runtime.close(); await legacy.close(); },
  };
}

function handleAction(response: ServerResponse, body: Record<string, unknown>, options: ChatTagSandboxHostOptions, runtime: SqliteNebulaGameRuntimeStore, actions: SqliteNebulaGameActionStore) {
  ...
  const updated = runtime.update(options.tenantId, (state) => {
    ...
    else { const active = resolveNebulaChannelGameIds(state, channel); if (gameId !== "chat-tag" && !active.includes(gameId)) throw new NebulaHostError(409, `${gameId} is not active in #${channel}`); if (checked.action === "leave") leaveNebulaGame(state, normalizeNebulaPlayerId(userId, username), gameId, now); else joinNebulaGame(state, { userId, username, displayName, gameId }, now); }
...
export { validateChatTagSandboxEnvironment }; export type { ChatTagSandboxHostOptions };
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) { const checked=validateChatTagSandboxEnvironment(process.env);const host=createChatTagSandboxHost({...checked,port:Number(process.env.PORT??8080),host:process.env.HOST??"0.0.0.0",buildSha:process.env.BUILD_SHA??"dev",...(process.env.CHAT_TAG_PIN_USER_ID?{pinUserId:process.env.CHAT_TAG_PIN_USER_ID}:{})});await host.listen(); }
