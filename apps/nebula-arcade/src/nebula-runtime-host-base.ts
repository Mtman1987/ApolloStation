...
export function createNebulaArcadeSandboxHost(options: NebulaArcadeSandboxHostOptions) {
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (request.method === "GET" && url.pathname === "/v1/nebula/game-inputs") return json(response,200,{inputs:inputs.list(options.tenantId),tabletop:tabletop.snapshot(options.tenantId,inputs.list(options.tenantId).at(-1)?.channelId || options.channelId)});
      ...
      if (request.method === "POST" && url.pathname === "/v1/nebula/game-actions") { requireSameOrigin(request); return handleAction(response, await readJson(request), options, runtime, actions, inputs, tabletop); }
  ...
  return {
    ...
    async close() { if (server.listening) await close(server); mixes.close(); actions.close(); runtime.close(); inputs.close(); tabletop.close(); await core.close(); },
  };
}

function handleAction(response: ServerResponse, body: Record<string, unknown>, options: NebulaArcadeSandboxHostOptions, runtime: SqliteNebulaGameRuntimeStore, actions: SqliteNebulaGameActionStore, inputs: SqliteNebulaGameInputStore, tabletop: SqliteNebulaTabletopRuntime) {
