...
export interface SpaceMountainUiOptions {
  ...
  onInvokeStella?: (message: string, conversationId: string, routingPreference: "automatic" | "hosted" | "companion") => void;
...
const APP_DOCK_NAVIGATION: Readonly<Record<string, readonly AppDockItemV1[]>> = {
  commlink: [
    ...
    { label: "Compose", description: "Reply to the active source-locked conversation.", icon: "arrow", target: ".cosmo-composer" },
  ],
  "stellar-core": [
...
export class SpaceMountainShellUi {
  ...
  private render() {
    ...
    root.querySelector<HTMLFormElement>("[data-stella-form]")?.addEventListener("submit", (event) => { event.preventDefault(); const form = event.currentTarget as HTMLFormElement; const values = new FormData(form); const message = String(values.get("message") ?? "").trim(); const rawRoute = String(values.get("routingPreference") ?? "automatic"); const route = rawRoute === "companion" || rawRoute === "hosted" ? rawRoute : "automatic"; if (message) { this.options.onInvokeStella?.(message, `stella-${this.snapshot.userId}`, route); form.reset(); } });
