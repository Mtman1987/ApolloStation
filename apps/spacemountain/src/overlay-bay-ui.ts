...
import { OVERLAY_BROADCAST_SLOTS, OVERLAY_SOURCE_KINDS, createBroadcastOverlayScene, createOverlayScene, createOverlaySource, fitOverlaySourceToBroadcastSlot, normalizeOverlayScenes, mergeOverlaySceneEdits, saveOverlayScene, sourceLabel, type OverlayBroadcastSlotV1, type OverlaySceneSourceV1, type OverlaySceneV1, type OverlaySourceKindV1 } from "./overlay-scenes.js";
...
export class OverlayBayParityController {
  ...
  private render(bay: HTMLElement) {
    ...
    const headActions = [`<button type="button" data-ob-broadcast>24/7 channel</button>`, `<button type="button" data-ob-new>New scene</button>`];
    ...
    const body = scene ? this.editor(scene, owner) : `<div class="ob-first"><h3>Create your first overlay scene</h3><p>Combine app widgets, a Nebula Game Mix, URLs, images, video, transparent frames, text, alerts, camera, screen, Xbox, links, ticker, and weather in one final browser source.</p><button type="button" data-ob-broadcast class="primary">Create 24/7 channel</button><button type="button" data-ob-new>Create blank scene</button></div>`;
  ...
  private preview(source: OverlaySceneSourceV1) {
    ...
    if (source.kind === "video" && typeof config.url === "string") return `<video src="${esc(config.url)}" autoplay muted loop playsinline></video>`;
    if (source.kind === "frame") return broadcastFrameMarkup();
  ...
  private bind(bay: HTMLElement) {
    ...
    bay.querySelectorAll<HTMLElement>("[data-ob-broadcast]").forEach((node) => node.onclick = () => this.newBroadcastScene(bay));
    ...
    bay.querySelectorAll<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>("[data-ob-config]").forEach((node) => node.onchange = () => { const source = this.selectedSource(); if (!source) return; const key = node.dataset.obConfig ?? ""; const value: unknown = node instanceof HTMLInputElement && node.type === "checkbox" ? node.checked : node instanceof HTMLInputElement && node.type === "number" ? Number(node.value) : node.value; if(key==="layoutSlot"&&typeof value==="string"&&value in OVERLAY_BROADCAST_SLOTS)this.updateSource(source.id,fitOverlaySourceToBroadcastSlot(source,value as OverlayBroadcastSlotV1));else this.updateSelected({ config: { ...source.config, [key]: value } }); this.render(bay); });
  ...
  private newBroadcastScene(bay: HTMLElement) { const scene=createBroadcastOverlayScene();this.scenes.push(scene);this.activeSceneId=scene.id;this.selectedSourceId="";this.changed();this.render(bay); }
  ...
  private addKind(kind: OverlaySourceKindV1, bay: HTMLElement) { const source = createOverlaySource(kind, kind === "nebula" ? "Nebula Arcade Game Mix" : sourceLabel(kind)); if (kind === "nebula") source.config = { mixId: `mix-${crypto.randomUUID()}`, gameIds: ["tag"], mode: "simultaneous", rotationSeconds: 20, gameStyles: { tag: "full" } };if(kind==="video")source.config={url:"/assets/overlay-bay/starfield-pingpong.mp4",fit:"cover",loop:true,muted:true};if(kind==="frame")source.config={preset:"broadcast-v1",accent:"#21e6ff",secondary:"#7557ff"}; this.addSource(source, bay); }
  ...
  private addSource(source: OverlaySceneSourceV1, bay: HTMLElement) { const scene = this.activeScene(); if (!scene) return; const frameIndex=scene.sources.findIndex(item=>item.kind==="frame"),sources=[...scene.sources];if(source.kind!=="frame"&&frameIndex>=0)sources.splice(frameIndex,0,source);else sources.push(source);this.updateScene({ sources: sources.map((item,zIndex)=>({...item,zIndex})) }); this.selectedSourceId = source.id; this.render(bay); }
