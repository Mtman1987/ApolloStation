...
export class OverlayBayParityController {
  ...
  private async syncNebula() {
    for (const scene of this.scenes) for (const source of scene.sources.filter((item) => item.kind === "nebula")) {
      ...
      const body = { id: String(source.config.mixId ?? source.id), name: source.name, mode: String(source.config.mode ?? "simultaneous"), rotationSeconds: Number(source.config.rotationSeconds ?? 20), layers: gridLayers(gameIds, record(source.config.gameStyles) ?? {}) };
