...
export function ChatWidget({ id, position, size, onPositionChange, onSizeChange, opacity, onOpacityChange, onSaveLayout, onClose, roomId, source = 'discord' }: ChatWidgetProps) {
  ...
  const { data: roomUser } = useDoc<{ discordGuildId?: string; twitchChannel?: string; discordSelectedChannel?: string }>(
  ...
  useEffect(() => {
    if (roomUser?.discordGuildId) {
      setServerId(roomUser.discordGuildId);
    }
  }, [roomUser]);
  ...
  const channelId = roomUser?.discordSelectedChannel?.trim();
  const twitchChannel = roomUser?.twitchChannel?.trim().toLowerCase();
