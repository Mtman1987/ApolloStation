...
const INNERTUBE_API_KEY = process.env.NEXT_PUBLIC_YOUTUBE_INNERTUBE_API_KEY?.trim();
...
async function fetchInnertubePlayer(videoId: string, client: InnertubeClient): Promise<any> {
  if (!INNERTUBE_API_KEY) {
    throw new Error('NEXT_PUBLIC_YOUTUBE_INNERTUBE_API_KEY is required for browser-side YouTube playback.');
  }
