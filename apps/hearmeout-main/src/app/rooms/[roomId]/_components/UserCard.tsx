...
export default function UserCard({ participant, isLocal, isHost, roomId }: { participant: LivekitClient.Participant; isLocal: boolean; isHost?: boolean; roomId: string; }) {
  ...
  const { data: roomUser } = useDoc<RoomParticipantData>(userRecordId ? `rooms/${roomId}/users` : null, userRecordId || null);

  useEffect(() => {
    if (!isLocal) return;
    if (roomUser?.serverMuted) {
      void room.localParticipant.setMicrophoneEnabled(false).then(() => {
        setMicPauseReason('A room moderator muted your microphone. You are still listening.');
      });
    }
  }, [roomUser?.serverMuted, isLocal, room]);

  React.useEffect(() => {
    if (roomUser?.twitchChannel) setTwitchChannel(roomUser.twitchChannel);
    if (roomUser?.discordGuildId) setDiscordGuildId(roomUser.discordGuildId);
    if (typeof roomUser?.streamMode === 'boolean') setStreamMode(roomUser.streamMode);
  }, [roomUser]);
  ...
  React.useEffect(() => {
    ...
    if (!roomUser?.twitchChannel && su.twitchLogin) {
    ...
    if (!roomUser?.discordGuildId && su.discordGuildId) {
  ...
  }, [isLocal, userRecordId, user, roomUser, roomId]);
  ...
  const handleToggleMic = async () => {
    if (!isLocal || !room) return;
    if (roomUser?.serverMuted && !participant.isMicrophoneEnabled) {
      setMicPauseReason('A room moderator muted your microphone. You are still listening.');
      return;
    }
  ...
  const displayName = liveKitName || participantMeta.displayName || roomUser?.displayName || sessionName || 'User';
  const photoURL = participantMeta.photoURL || roomUser?.photoURL || sessionPhoto || `https://picsum.photos/seed/${userRecordId}/100/100`;
  ...
  const handleServerMute = () => {
    const isMutedByServer = roomUser?.serverMuted;
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {isLocal ? (
                        <div className="flex items-center gap-1 text-muted-foreground">
                            <Tooltip><TooltipTrigger asChild><Button variant={isMuted ? "destructive" : "ghost"} size="icon" onClick={handleToggleMic} disabled={!!roomUser?.serverMuted} className="h-7 w-7">{isMuted ? <MicOff className="h-4 w-4" /> : <Mic className="h-4 w-4" />}</Button></TooltipTrigger><TooltipContent><p>{roomUser?.serverMuted ? 'Muted by room moderator' : isMuted ? 'Unmute' : 'Mute'}</p></TooltipContent></Tooltip>
                        ...
                        <div className='flex items-center gap-1'>
                            ...
                            <Tooltip><TooltipTrigger asChild><Button variant={roomUser?.serverMuted ? 'destructive' : 'ghost'} size="icon" className="h-7 w-7" onClick={handleServerMute} disabled={adminActionPending}><MicOff className="h-4 w-4" /></Button></TooltipTrigger><TooltipContent><p>{roomUser?.serverMuted ? 'Unmute' : 'Server Mute'}</p></TooltipContent></Tooltip>
