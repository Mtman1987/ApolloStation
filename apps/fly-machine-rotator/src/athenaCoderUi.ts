...
async function proxyToDashboard(
  ...
  if (isCodexWrite && await requireSpmtAdmin(incoming, env)) {
...
${spmtSharedUiHead("athena-coder")}<style id="spmt-suite-background">
...
${spmtSharedUiScript("athena-coder", "/athena/api/settings")}</body>
</html>`;
}
