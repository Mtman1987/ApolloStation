...
import { decideRepairApproval } from "./repairApproval.js";
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (request.method === "GET" && url.pathname === "/athena/api/jobs") {
        if (!(await requireSpmtAdmin(request, env))) return sendJson(response, 401, { error: "Unauthorized" });
...
function redirectToLogin(response: ServerResponse, url: URL) {
  ...
  response.writeHead(302, { location: `/auth/spmt/login?next=${next}`, "cache-control": "no-store" });
  response.end();
}
...
async function authorizedProxyHeaders(
  ...
  delete headers["x-rotator-internal"];
