...
function isExpectedApplicationResponse(message: string): boolean {
  return [
    ...
    /Health check 'servicecheck-[^']+' on port \d+ has failed\. Your app is not responding properly\. Services exposed on ports \[[^\]]+\] will have intermittent failures until the health check passes\./i,
...
function isNonActionableErrorEcho(message: string): boolean {
  return [
    ...
    /\[DiscordTrace\]\s*\{.*"stage":"complete".*"error":null/i,
    ...
    /^\[\d{2}:\d{2}\] error: Reconnecting in \d+ seconds?\.\.$/i,
