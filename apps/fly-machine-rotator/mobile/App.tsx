...
type MainTab = "home" | "workspace" | "targets" | "apps" | "visual" | "logs" | "relay" | "memory" | "stream" | "devices" | "polling" | "logos" | "qr" | "roadmap";
...
export default function App() {
  ...
  const [wakeName, setWakeName] = useState("Athena");
  ...
  const wakeNameRef = useRef("Athena");
  ...
  async function runCommand(commandId: string, message = "MountainView mobile trigger", destination: VoiceDestination = voiceDestination, options: { speakReply?: boolean; visualContextOverride?: string; commandMode?: boolean } = {}) {
    try {
      const workspaceIdentity = String(workspace?.tenant ?? "").trim().toLowerCase();
        ...
        const routeData = await request("/voice/route", {
          ...
          body: JSON.stringify({
            ...
            context: {
              ...
              tenantId: workspaceIdentity,
              username: workspaceIdentity,
              channel: twitchTargetChannel.trim() || activeProfileChannel || extractTwitchChannelFromVisualContext(lockedVisualContext) || workspaceIdentity,
      ...
      const selectedChannel = actualDestination === "twitch"
        ? (intent.twitchChannel || twitchTargetChannel.trim() || activeProfileChannel || extractTwitchChannelFromVisualContext(lockedVisualContext) || workspaceIdentity)
        : workspaceIdentity;
      const voicePayload = {
  ...
  async function startWakeListener() {
    ...
    setStatusMessage("Hey " + wakeNameRef.current + " listener active. Keep SpaceMountain Companion open.");
    setLog("Wake listener active. Say: Hey Athena or Hey " + wakeNameRef.current + ", followed by your command.");
      ...
      try {
        ...
        if (transcript) {
          ...
          if (command) {
            ...
            setStatusMessage("Listening for Hey Athena / Hey " + wakeNameRef.current + ". Heard: " + transcript);
          }
        } else {
