...
const DSH_STATUS_SUCCESS_TTL_MS = 30_000;
...
async function startServer() {
  ...
  app.get('/api/spmt/apps', async (req, res) => {
    ...
    try {
      ...
      const response = await fetch(target, {
        headers: buildSpmtProxyHeaders(req.headers, token, false),
      });
      ...
      return res.status(502).json({ error: 'SPMT app registry unavailable' });
    }
  });
