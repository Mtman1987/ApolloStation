...
assert.match(rocketEasterEgg, /PORTAL_HINT = 'ENTER HERE'/, 'black-hole portal should carry the subtle entrance hint');
...
assert.match(app, /api\/apps\?surface=spacemountain/, 'Shipyard should discover approved apps from the filtered canonical SPMT registry');
assert.match(app, /If-None-Match/, 'Shipyard should use registry revisions for inexpensive polling');
assert.match(app, /API \{appRegistryConnection\.status\}/, 'Shipyard should expose registry API health as visual proof');
assert.match(app, /readCachedAppRegistry/, 'Shipyard should keep a last-known catalog available during a registry outage');
