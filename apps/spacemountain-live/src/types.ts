...
export interface SpmtRegistryAppV1 {
  schemaVersion: 1;
  id: string;
  name: string;
  description: string;
  ...
  runtimeState: 'ready' | 'cold' | 'degraded' | 'unavailable' | 'unknown';
  accessState: 'installed' | 'disabled' | 'available';
  installed: boolean;
  enabled: boolean;
  version?: string;
  latestVersion?: string;
...
export interface SpmtAppRegistryV1 {
  schemaVersion: 1;
  version: 'app-registry.v1';
  revision: string;
  generatedAt: string;
  filters: Record<string, string>;
  apps: SpmtRegistryAppV1[];
}

export interface AppRegistryConnectionState {
  status: 'loading' | 'online' | 'cached' | 'unavailable';
