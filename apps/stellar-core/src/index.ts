...
export class StellarCommunityAssistantRuntime {
  ...
  accept(input: CommunityAssistantInvocationV1) {
    ...
    const remember = input.remember !== false && (!input.presentation || input.presentation.memoryPolicy === "conversation");
    const existing = this.jobs.findIdempotent(input.tenantId, "stellar-core", input.requestedById, input.idempotencyKey);
    if (existing) {
      const replayIdentity = [input.requestedByType, input.userId, input.message, input.callerAppId, input.surface, routingPreference, remember, input.conversationId ?? null, input.presentation ?? null, input.research ?? null, input.correlationId ?? null];
      const originalIdentity = [existing.requestedByType, existing.billedUserId, existing.input.message, existing.input.callerAppId, existing.input.surface, existing.input.routingPreference, existing.input.remember !== false, existing.input.conversationId ?? null, existing.input.presentation ?? null, existing.input.research ?? null, existing.correlationId ?? null];
    ...
    const created = this.jobs.create({
      ...
      input: { kind: STELLAR_CHAT_REQUEST_KIND, message: input.message, userId: input.userId, callerAppId: input.callerAppId, surface: input.surface, routingPreference, remember, ...(input.presentation ? { presentation: input.presentation } : {}), ...(input.research?{research:input.research}:{}), ...(route.fallbackReason ? { fallbackReason: route.fallbackReason } : {}), ...(input.conversationId ? { conversationId: input.conversationId } : {}) },
