import { YouTubeLiveChatDriver } from "./youtube-driver.js";
...
type DriverOpenInput = Parameters<ProviderConnectionDriverV1["open"]>[0];
...
export class DiscordGatewayProviderDriver implements ProviderConnectionDriverV1, ChatProviderSenderV1 {
  ...
  async open(input: DriverOpenInput): Promise<ProviderConnectionHandleV1> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const finish = (error?: Error) => { if (settled) return; settled = true; clearTimeout(timer); error ? reject(error) : resolve(); };
      socket.addEventListener("message", (event) => { const p = json(socketText(event.data)); if (!p) return; const op = numberValue(p.op), seq = numberValue(p.s); if (seq !== undefined) sequence = seq; if (op === 10) { const hello = record(p.d), interval = numberValue(hello?.heartbeat_interval) ?? 45_000; if (heartbeat) clearInterval(heartbeat); heartbeat = setInterval(() => socket.send(JSON.stringify({ op: 1, d: sequence })), Math.max(1_000, interval)); socket.send(JSON.stringify(resume?.sessionId && resume.seq !== undefined ? { op: 6, d: { token: input.accessToken, session_id: resume.sessionId, seq: resume.seq } } : { op: 2, d: { token: input.accessToken, intents: 33_281, properties: { os: "linux", browser: "spmt-chat-gateway", device: "spmt-chat-gateway" } } })); return; } if (op === 1) { socket.send(JSON.stringify({ op: 1, d: sequence })); return; } if (op === 7 || op === 9) { input.onDisconnect({ kind: "transport", reason: op === 7 ? "Discord requested reconnect" : "Discord session became invalid" }); return; } if (op !== 0) return; const type = stringValue(p.t); if (type === "READY") { const ready = record(p.d); sessionId = stringValue(ready?.session_id) ?? sessionId; resumeGatewayUrl = stringValue(ready?.resume_gateway_url) ?? resumeGatewayUrl; saveCursor(); finish(); return; } if (type === "RESUMED") { finish(); return; } saveCursor(); if(type&&["MESSAGE_UPDATE","MESSAGE_DELETE","MESSAGE_DELETE_BULK"].includes(type)){for(const mutation of parseDiscordMutations(type,p.d,input.connection))void Promise.resolve(input.onMutation?.(mutation)).catch(()=>undefined);} if (type === "MESSAGE_CREATE") { const envelope = parseDiscord(p.d, input.connection); if (envelope) void Promise.resolve(input.onEnvelope(envelope)).catch(() => undefined); } function saveCursor() { if (sessionId && sequence !== null) input.onCursor(encodeDiscordCursor({ sessionId, seq: sequence, ...(resumeGatewayUrl ? { resumeGatewayUrl } : {}) })); } });
...
export function createFirstPartyChatProviderAdapters(deps: ProviderDriverDependencies = {}) { const youtube=new YouTubeLiveChatDriver(deps.fetch as typeof fetch|undefined), twitch = new TwitchIrcProviderDriver(deps), discord = new DiscordGatewayProviderDriver(deps), kick = new KickPusherProviderDriver(deps); return { drivers: [twitch, discord, kick, youtube] as ProviderConnectionDriverV1[], senders: [twitch, discord, kick, youtube] as ChatProviderSenderV1[], twitch, discord, kick }; }
...
export function parseDiscordMutations(type:string,value:unknown,c:ProviderConnectionConfigV1,now=new Date()):import("@spmt/contracts").CommlinkProviderMutationV1[]{
 const m=record(value);if(!["MESSAGE_UPDATE","MESSAGE_DELETE","MESSAGE_DELETE_BULK"].includes(type)||!m||c.provider!=="discord"||scalarString(m.channel_id)!==c.channelId)return [];
 const operation=type==="MESSAGE_UPDATE"?"edit":"delete",ids=type==="MESSAGE_DELETE_BULK"&&Array.isArray(m.ids)?m.ids.slice(0,100):[m.id];
 const attachments=Array.isArray(m.attachments)?m.attachments.slice(0,12).flatMap(value=>{const a=record(value);try{const url=new URL(String(a?.url));return url.protocol==="https:"&&!url.username&&!url.password&&url.href.length<=2048?[{url:url.href,name:String(a?.filename??"Attachment").slice(0,150)}]:[]}catch{return []}}):undefined;
 if(operation==="edit"&&typeof m.content!=="string"&&!attachments)return [];
 const edited=typeof m.edited_timestamp==="string"&&Number.isFinite(Date.parse(m.edited_timestamp))?new Date(m.edited_timestamp).toISOString():now.toISOString();
 return ids.flatMap(id=>typeof id==="string"&&/^[0-9]{1,30}$/.test(id)?[{schemaVersion:1 as const,tenantId:c.tenantId,provider:"discord" as const,connectionId:c.connectionId,channelId:c.channelId,messageId:id,operation,occurredAt:edited,...(operation==="edit"&&typeof m.content==="string"?{text:m.content.slice(0,8000)}:{}),...(operation==="edit"&&attachments?{rich:{source:"discord",eventType:"edit",attachments}}:{})}]:[]);
}
function parseDiscord(value: unknown, c: ProviderConnectionConfigV1): ProviderChatEnvelopeV1 | undefined {
  ...
  const attachments=Array.isArray(m.attachments)?m.attachments.slice(0,12).flatMap(value=>{const a=record(value);try{const u=new URL(String(a?.url));return u.protocol==="https:"&&!u.username&&!u.password&&u.href.length<=2048?[{url:u.href,name:String(a?.filename??"Attachment").slice(0,150)}]:[]}catch{return []}}):[];
  const embedText=Array.isArray(m.embeds)?m.embeds.slice(0,4).flatMap(value=>{const e=record(value);return [stringValue(e?.title),stringValue(e?.description)].filter(Boolean)}).join("\n").slice(0,2000):"";
  const content = stringValue(m.content)||embedText||(attachments.length?"[Attachment]":undefined), id = scalarString(m.id), author = record(m.author), providerUserId = scalarString(author?.id), username = stringValue(author?.username), timestamp = stringValue(m.timestamp);
  if (!content || !id || !providerUserId || !username || !timestamp || !Number.isFinite(Date.parse(timestamp))) return;
  const mentions = Array.isArray(m.mentions) ? m.mentions.flatMap(v => { const x = record(v), mid = scalarString(x?.id), name = stringValue(x?.username); return mid && name ? [{ token: `<@${mid}>`, providerUserId: mid, username: name }] : []; }) : [];
  const avatarHash = stringValue(author?.avatar);
  const avatarUrl = avatarHash && /^[A-Za-z0-9_]+$/.test(avatarHash) ? `https://cdn.discordapp.com/avatars/${providerUserId}/${avatarHash}.${avatarHash.startsWith("a_") ? "gif" : "webp"}?size=256` : undefined;
  return { schemaVersion: 1, tenantId: c.tenantId, provider: "discord", connectionId: c.connectionId, channelId: c.channelId, ...(scalarString(m.guild_id)?{guildId:scalarString(m.guild_id)!}:{}), messageId: id, text: content,...(attachments.length?{rich:{source:"discord",eventType:"message",attachments}}:{}), occurredAt: new Date(timestamp).toISOString(), providerUserId, username, ...(stringValue(author?.global_name) ? { displayName: stringValue(author?.global_name)! } : {}), ...(avatarUrl ? { avatarUrl } : {}), isBot: author?.bot === true, roles: ["member"], mentions };
}
function parseKick(value: unknown, c: ProviderConnectionConfigV1, now: Date): ProviderChatEnvelopeV1 | undefined { const m = record(value), sender = record(m?.sender), id = scalarString(m?.id), content = stringValue(m?.content), providerUserId = scalarString(sender?.id) ?? scalarString(sender?.user_id), username = stringValue(sender?.slug) ?? stringValue(sender?.username); if (!id || !content || !providerUserId || !username) return; const badges = new Set<string>(), identity = record(sender?.identity); if (Array.isArray(identity?.badges)) for (const b of identity.badges) { const type = stringValue(record(b)?.type); if (type) badges.add(type); } const roles: Array<"broadcaster"|"moderator"|"member"> = ["member"]; if (badges.has("broadcaster")) roles.unshift("broadcaster"); else if (badges.has("moderator")) roles.unshift("moderator"); const created = stringValue(m?.created_at), occurredAt = created && Number.isFinite(Date.parse(created)) ? new Date(created).toISOString() : now.toISOString(); const avatarCandidate = stringValue(sender?.profile_picture) ?? stringValue(sender?.profile_pic); let avatarUrl: string | undefined; try { if (avatarCandidate) avatarUrl = cleanHttpsUrl(avatarCandidate, "Kick sender avatar"); } catch {} return { schemaVersion: 1, tenantId: c.tenantId, provider: "kick", connectionId: c.connectionId, channelId: c.channelId, messageId: id, text: content, occurredAt, providerUserId, username, ...(stringValue(sender?.username) ? { displayName: stringValue(sender?.username)! } : {}), ...(avatarUrl ? { avatarUrl } : {}), isBot: badges.has("bot"), roles, mentions: [] }; }
