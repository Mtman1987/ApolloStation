...
function parseDiscord(value: unknown, c: ProviderConnectionConfigV1): ProviderChatEnvelopeV1 | undefined {
  ...
  return { schemaVersion: 1, tenantId: c.tenantId, provider: "discord", connectionId: c.connectionId, channelId: c.channelId, messageId: id, text: content, occurredAt: new Date(timestamp).toISOString(), providerUserId, username, ...(stringValue(author?.global_name) ? { displayName: stringValue(author?.global_name)! } : {}), ...(avatarUrl ? { avatarUrl } : {}), isBot: author?.bot === true, roles: ["member"], mentions };
}
