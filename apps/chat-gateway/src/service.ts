...
export interface ChatGatewayWorkerEnvironmentV1 {
  ...
  reconcileMs: number;
  streamweaver?: { databasePath: string; credential: string; image?: { token: string; modelNo: string; modelVerNo: string; binary: string; edenKey?:string;edenModel?:string;cloudflareAccountId?:string;cloudflareToken?:string } };
...
export function validateChatGatewayWorkerEnvironment(environment: NodeJS.ProcessEnv): ChatGatewayWorkerEnvironmentV1 {
  ...
  if (streamweaverEnabled) {
    ...
    const cloudflareAccountId=environment.STREAMWEAVER_CLOUDFLARE_ACCOUNT_ID,cloudflareToken=environment.STREAMWEAVER_CLOUDFLARE_API_TOKEN;
    if(Boolean(cloudflareAccountId)!==Boolean(cloudflareToken)||cloudflareAccountId&&!/^[a-f0-9]{32}$/i.test(cloudflareAccountId))throw Error("Configure a valid Cloudflare account ID and API token together");
    if(runtimeMode==="sandbox"&&cloudflareToken)throw Error("Sandbox StreamWeaver rejects external image generation");
    let image:NonNullable<NonNullable<ChatGatewayWorkerEnvironmentV1["streamweaver"]>["image"]>|undefined=imageValues.length?{token:String(environment.STREAMWEAVER_SEAART_CLI_TOKEN),modelNo:modelIdentifier(environment.STREAMWEAVER_SEAART_MODEL_NO,"STREAMWEAVER_SEAART_MODEL_NO"),modelVerNo:modelIdentifier(environment.STREAMWEAVER_SEAART_MODEL_VER_NO,"STREAMWEAVER_SEAART_MODEL_VER_NO"),binary:environment.STREAMWEAVER_SEAART_CLI_BINARY||"seaart",...(edenKey?{edenKey,edenModel:environment.STREAMWEAVER_EDENAI_IMAGE_MODEL||"image/generation/stabilityai"}:{})}:edenKey?{token:"",modelNo:"",modelVerNo:"",binary:"seaart",edenKey,edenModel:environment.STREAMWEAVER_EDENAI_IMAGE_MODEL||"image/generation/stabilityai"}:undefined;
    if(cloudflareAccountId&&cloudflareToken){image??={token:"",modelNo:"",modelVerNo:"",binary:"seaart"};image.cloudflareAccountId=cloudflareAccountId;image.cloudflareToken=cloudflareToken;}
...
export class SupervisedChatGatewayService {
  ...
  constructor(private readonly options: ChatGatewayWorkerEnvironmentV1, fetchImpl?: typeof fetch) {
    ...
    if (options.streamweaver) {
      ...
      if(options.streamweaver.image&&options.operationMode==="active") {
        const image=options.streamweaver.image,providers=[...(image.cloudflareAccountId&&image.cloudflareToken?[new CloudflareStreamWeaverImageProvider(image.cloudflareAccountId,image.cloudflareToken,fetchImpl)]:[]),...(image.token?[new SeaArtCliProvider(image.token,new NodeSeaArtCommandRunner(image.binary))]:[]),...(image.edenKey?[new EdenStreamWeaverImageProvider(image.edenKey,image.edenModel,fetchImpl)]:[])],tenantIds=[...new Set(options.connections.map(connection=>connection.tenantId))];
    ...
    if (options.nebulaArcade) {
      ...
      this.nebulaArcade = new NebulaArcadeProviderRuntime({ onLiveChannels:async(tenantId,names)=>{
        const template=options.connections.find(connection=>connection.tenantId===tenantId&&connection.provider==='twitch'&&connection.desired);if(!template)return[];
        const configured=new Set(options.connections.filter(connection=>connection.tenantId===tenantId&&connection.provider==='twitch').map(connection=>connection.channelId.replace(/^#/,'').toLowerCase()));
        const wanted=[...new Set(names.map(name=>name.replace(/^#/,'').toLowerCase()))].filter(name=>/^[a-z0-9_]{1,25}$/.test(name)&&!configured.has(name)).sort().slice(0,100),desired=new Set(wanted);let added=0;const active:Array<{connectionId:string;channelId:string}>=[];
        for(const prior of this.connectionStore.list(tenantId).filter(connection=>connection.provider==='twitch'&&connection.connectionId.startsWith('nebula-live-')))if(prior.desired&&!desired.has(prior.channelId))this.connectionStore.put({...prior,desired:false});
        for(const channelId of wanted){const connectionId=`nebula-live-${channelId}`,prior=this.connectionStore.get(tenantId,'twitch',connectionId);if(!prior?.desired){if(added>=5)continue;this.connectionStore.put({...template,connectionId,channelId,desired:true});added++;}active.push({connectionId,channelId});}return active;
      }, presence:async(tenantId,players)=>{
        const connection=options.connections.find(connection=>connection.tenantId===tenantId&&connection.provider==="twitch");if(!connection)throw new Error("No Twitch connection for presence");
        const grant=await new SpmtChatProviderGrantSource(client).getGrant(connection);if(grant.status!=="ready"||!grant.metadata?.clientId)throw new Error("Twitch presence grant unavailable");
        const liveUserIds:string[]=[],channelByUserId:Record<string,string>={};
        for(let offset=0;offset<players.length;offset+=100){const batch=players.slice(offset,offset+100),url=new URL("https://api.twitch.tv/helix/streams");for(const player of batch)url.searchParams.append("user_login",player.username);const response=await(fetchImpl??fetch)(url,{headers:{"client-id":grant.metadata!.clientId!,authorization:`Bearer ${grant.accessToken}`},signal:AbortSignal.timeout(10000)});if(!response.ok)throw new Error(`Twitch presence unavailable (${response.status})`);const payload=await response.json() as {data?:Array<{user_login:string}>};if(!Array.isArray(payload.data))throw new Error("Invalid Twitch presence snapshot");for(const stream of payload.data){const player=batch.find(player=>player.username.toLowerCase()===stream.user_login.toLowerCase());if(player){liveUserIds.push(player.userId);channelByUserId[player.userId]=stream.user_login;}}}
        return{liveUserIds,channelByUserId,observedAt:new Date().toISOString()};
      }, databasePath: options.nebulaArcade.databasePath, config: options.nebulaArcade.config, client: nebulaClient, simulation: options.operationMode === "read-only", egress: { send: (message) => { if (!connectedGateway) throw new Error("Chat Gateway egress is not ready"); return connectedGateway.send(message); } }, ...(options.nebulaArcade.publicOrigin ? { discordDashboard: { egress: discordDashboard, publicOrigin: options.nebulaArcade.publicOrigin, ...(options.nebulaArcade.gameplayOrigin ? { gameplayOrigin: options.nebulaArcade.gameplayOrigin } : {}), webhookName: options.nebulaArcade.webhookName, ...(options.nebulaArcade.avatarUrl ? { avatarUrl: options.nebulaArcade.avatarUrl } : {}) } } : {}) });
      consumers.push(...this.nebulaArcade.consumers);
    }
    this.gateway = new ChatGatewayRuntime(this.chatStore, consumers, senders, observers,mutation=>client.mutateCommlinkLiveChat(mutation.tenantId,mutation));
  ...
  async reconcile() {
    ...
    const mutations=await this.gateway.flushMutations();
