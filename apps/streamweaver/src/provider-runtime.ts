...
export class StreamWeaverProviderRuntime {
  ...
  constructor(private readonly options: StreamWeaverProviderRuntimeOptionsV1) {
    ...
    const services = new DefaultStreamWeaverDonorCommandServices({
      ...
      socialEffects:new StreamWeaverSocialActionExecutor(options.client,this.socialReactions),
    ...
    const flows = this.installedFlows = new StreamWeaverInstalledFlowConsumer(this.flows, this.commandState, egress, options.botActions, commands, options.nowMs, {
      ...
      getJob:(tenantId,jobId)=>options.simulation&&jobId.startsWith("simulation:")?Promise.resolve({id:jobId,tenantId,state:"succeeded",result:{text:jobId.includes(":assistant:")?"Simulated assistant response.":"Simulated speech playback."}} as unknown as import("@spmt/contracts").ExecutionJobV1):options.client.getExecutionJob(tenantId,jobId),
      ...
      speech:async({delivery,text,voice,requestId})=>{
        if(options.simulation)return{jobId:`simulation:speech:${createHash("sha256").update(requestId+text+String(voice??"")).digest("hex")}`};
        if(options.allowAssistant===false||options.allowProviderWrites!==true)throw new Error("External speech is disabled in this environment");
