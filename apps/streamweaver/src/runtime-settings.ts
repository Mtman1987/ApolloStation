...
export class StreamWeaverRuntimeSettingsStore {
  ...
  close() { this.db.close(); }
  appearance(tenantId:string):{avatarUrl:string;talkingUrl:string;modelUrl:string;modelAssetId:string} {const row=this.db.prepare("SELECT body FROM streamweaver_appearance WHERE tenant_id=?").get(tenantId) as {body:string}|undefined;return{avatarUrl:"",talkingUrl:"",modelUrl:"",modelAssetId:"",...(row?JSON.parse(row.body):{})};}
  saveAppearance(tenantId:string,value:Record<string,unknown>) {const prior=this.appearance(tenantId),result={...prior};for(const key of ["avatarUrl","talkingUrl","modelUrl"] as const){if(!(key in value))continue;const text=String(value[key]??"").trim();result[key]="";if(!text)continue;const url=new URL(text);if(url.protocol!=="https:"||url.username||url.password||text.length>2000)throw new Error("Avatar assets must use public HTTPS URLs");result[key]=url.href;}if("modelAssetId" in value){const id=String(value.modelAssetId??"").trim();if(id&&!/^[a-f0-9-]{36}$/.test(id))throw Error("Avatar model asset id is invalid");result.modelAssetId=id;}this.db.prepare("INSERT OR REPLACE INTO streamweaver_appearance VALUES(?,?)").run(tenantId,JSON.stringify(result));return result;}
