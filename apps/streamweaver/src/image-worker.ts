import {validateCloudflareReference} from "./cloudflare-image-provider.js";
  ...
  private async execute(job: ExecutionJobV1) {
    ...
    try {
      ...
      if(!media){
        ...
        if(!Array.isArray(ids)||ids.length>4||ids.some(id=>typeof id!=="string"||!/^\w[\w-]{1,100}$/.test(id)))throw Error("Invalid reference image selection");
        const referenceImages:import("./binary-image.js").StreamWeaverBinaryImage[]=[];
        if(ids.length&&(settings.provider!=="cloudflare"||!settings.cloudflareModel?.includes("/flux-2-klein-")))throw Error("Choose Cloudflare Klein before attaching reference images");
        for(const id of ids){
          if(!this.client.getMediaAsset||!this.client.readMediaAsset)throw Error("Reference image storage is unavailable");
          const jobContext={jobId:job.id,leaseId:job.leaseId,fencingEpoch:job.fencingEpoch},asset=await this.client.getMediaAsset(job.tenantId,id,jobContext);
          if(asset.tenantId!==job.tenantId||asset.ownerUserId!==job.billedUserId||asset.contentType!=="image/png")throw Error("Reference image ownership or format mismatch");
          const bytes=await this.client.readMediaAsset(job.tenantId,id,jobContext),image={contentType:asset.contentType,base64:Buffer.from(bytes).toString("base64")};validateCloudflareReference(image);referenceImages.push(image);
        }
        media=await this.service.image({referenceImages, surface:scope==="public"?"public":"private",prompt, modelNo:settings.modelNo||String(job.input.model||this.options.modelNo),modelVerNo:settings.modelVerNo||String(job.input.modelVerNo||this.options.modelVerNo),count:job.input.generationSettings?settings.count:count,seed:settings.seed,resolution:settings.resolution,provider:settings.provider,contentModeration,...(settings.edenModel?{edenModel:settings.edenModel}:{}),...(settings.cloudflareModel?{cloudflareModel:settings.cloudflareModel}:{}),...(settings.pollinationsModel?{pollinationsModel:settings.pollinationsModel}:{}),providerParams:settings.providerParams,enhancePrompt:settings.enhance,tenantId:job.tenantId,userId:job.billedUserId,requestId:job.id,promptTemplate:settings.promptTemplate });this.options.settings?.saveGenerated(job.tenantId,job.id,media);}else if(contentModeration)await this.service.moderatePrompt(media.prompt);
