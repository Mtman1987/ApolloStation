...
export class StreamWeaverImageWorker {
  private completedJobs=0;private failedJobs=0;private readonly startedAt=new Date().toISOString();private lastReportAt=0;
  ...
  private async execute(job: ExecutionJobV1) {
    ...
    try {
      ...
      const settings=normalizeGenerationSettings(job.input.generationSettings&&typeof job.input.generationSettings==="object"?job.input.generationSettings:this.options.settings?.read(job.tenantId,job.input.mediaVisibility==="private"?`private:${job.billedUserId}`:"public")??{});
