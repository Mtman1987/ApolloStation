...
export class KeenToolsAvatarProvider {
  ...
  async reconstruct(views:KeenToolsAvatarViewV1[]):Promise<KeenToolsHeadV1>{
    ...
    const avatarId=id(initialized.avatar_id),uploads=Array.isArray(initialized.img_urls)?initialized.img_urls.map(url=>httpsUrl(url)):[];
    ...
    for(let attempt=0;attempt<30;attempt++){
      ...
      if(result.event==="redirect")return{avatarId,glb:await binary(this.fetchImpl,httpsUrl(record(result.data).url),"KeenTools GLB",64*1024*1024)};
...
function httpsUrl(value:unknown){const url=new URL(String(value??""));if(url.protocol!=="https:"||url.username||url.password)throw Error("KeenTools asset URL is invalid");return url.toString();}
