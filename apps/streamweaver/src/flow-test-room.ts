...
export class StreamWeaverFlowTestRoom {
  ...
  async start(tenant:string,user:string,pkg:StreamWeaverFlowPackageV1){
    ...
    const id=randomUUID(),base=this.path(tenant,user,id),roomId=`streamweaver:flow-test:${user}`,db=join(base,"flow.sqlite"),startedAt=Date.now();let time=startedAt;mkdirSync(base,{recursive:true});
    ...
    const runtime=new StreamWeaverProviderRuntime({databasePath:db,client:localClient,egress:{send:async message=>{await this.emit(tenant,roomId,message.text,`flow-test:${id}:${message.idempotencyKey}`);return{providerMessageId:randomUUID()};}},botActions:{execute:async request=>({response:`Simulated ${request.action}.`,result:{state:"succeeded",simulation:true}})},allowAssistant:true,simulation:true,publicOrigin:"https://simulation.invalid",now:()=>new Date(time).toISOString(),nowMs:()=>time});
