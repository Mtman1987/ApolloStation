...
import { StreamWeaverFlowPackageStore, type StreamWeaverFlowActionV1, type StreamWeaverFlowPackageV1 } from "./flow-packages.js";
...
export class StreamWeaverInstalledFlowConsumer {
  ...
  async deliver(delivery: NormalizedChatDeliveryV1) {
    ...
    const actionIds = new Set(match.command.actionIds);
    for (const action of match.package.actions.filter((item) => item.enabled && actionIds.has(item.id))) {
  ...
  async preview(item: StreamWeaverFlowPackageV1, commandId: string, delivery: NormalizedChatDeliveryV1) {
    ...
    const actionIds = new Set(command.actionIds), outputs: Array<{ actionId: string; type: StreamWeaverFlowActionV1["type"]; text: string }> = [];
    for (const action of item.actions.filter((candidate) => actionIds.has(candidate.id))) {
      const text = await this.run(action, delivery, true);
  ...
  private async run(action: StreamWeaverFlowActionV1, delivery: NormalizedChatDeliveryV1, preview = false) {
    ...
    if (preview) return `Would run ${action.type}`;
