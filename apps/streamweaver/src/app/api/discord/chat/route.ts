...
export async function resolveMentionedBot(messageLower: string, _guildTenantId?: string): Promise<BotMatch | null> {
  ...
  const addCandidate = (tenantId: string | undefined) => {
    ...
    // Discord routes a configured persona name, not the Twitch transport
    // identity. Tenants using the shared community Twitch account still own
    // distinct Discord bot names and aliases.
    const botName = config.AI_BOT_NAME || getBotName(tenantId);
    const configAliases = splitAliases(config.AI_BOT_ALIASES);
