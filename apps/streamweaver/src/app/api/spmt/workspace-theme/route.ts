...
export async function GET(request: NextRequest) {
  ...
  let [profileResponse, tenantResponse, surfacesResponse] = await loadWorkspace(token);
  if (profileResponse.status === 401 || profileResponse.status === 403) {
    refreshed = await refreshSpmtConnection(request);
    if (refreshed) {
      token = refreshed.accessToken;
      [profileResponse, tenantResponse, surfacesResponse] = await loadWorkspace(token);
    }
  }
  const [payload, tenantPayload, surfacesPayload] = await Promise.all([
