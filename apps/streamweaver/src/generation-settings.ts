...
export interface StreamWeaverGenerationSettings {provider:"automatic"|"seaart-cli"|"edenai";publicAccess:"everyone"|"mods"|"off";modelNo:string;modelVerNo:string;resolution:string;count:number;seed:number;enhance:boolean;promptTemplate:string;providerParams:Record<string,string|number|boolean>;}
...
const defaults=():StreamWeaverGenerationSettings=>({provider:"automatic",publicAccess:"everyone",modelNo:"",modelVerNo:"",resolution:"1024x1024",count:1,seed:0,enhance:true,promptTemplate:GENERATION_TEMPLATES.general,providerParams:{}});
export class StreamWeaverGenerationStore {
  private readonly db:DatabaseSync;
  constructor(path:string){this.db=new DatabaseSync(path);this.db.exec("PRAGMA journal_mode=WAL; PRAGMA busy_timeout=5000; CREATE TABLE IF NOT EXISTS sw_generation_settings(tenant TEXT NOT NULL,surface TEXT NOT NULL,body TEXT NOT NULL,PRIMARY KEY(tenant,surface)); CREATE TABLE IF NOT EXISTS sw_generated_checkpoints(tenant TEXT NOT NULL,job TEXT NOT NULL,body TEXT NOT NULL,PRIMARY KEY(tenant,job))");}
