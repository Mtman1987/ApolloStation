import {CLOUDFLARE_IMAGE_MODELS,CLOUDFLARE_PRIVATE_IMAGE_MODEL} from "./cloudflare-image-provider.js";
...
export class StreamWeaverGenerationStore {
  ...
  read(tenant:string,surface="public"):StreamWeaverGenerationSettings {const row=this.db.prepare("SELECT body FROM sw_generation_settings WHERE tenant=? AND surface=?").get(tenant,surface);return {...defaults(),contentModeration:surface==="public",...(row?JSON.parse(String(row.body)):{})};}
  save(tenant:string,surface:string,input:Partial<StreamWeaverGenerationSettings>){if(surface!=="public"&&!/^private:[A-Za-z0-9._:@/-]{1,200}$/.test(surface))throw new Error("Invalid generation settings scope");const value=normalizeGenerationSettings({...this.read(tenant,surface),...input});if(surface==="public"&&value.cloudflareModel===CLOUDFLARE_PRIVATE_IMAGE_MODEL)throw Error("Klein 9B is restricted to private generation");this.db.prepare("INSERT INTO sw_generation_settings VALUES(?,?,?) ON CONFLICT(tenant,surface) DO UPDATE SET body=excluded.body").run(tenant,surface,JSON.stringify(value));return value;}
  ...
  saveCatalog(models:Array<{provider:string;id:string;label:string}>){const refreshed=new Set(models.map(m=>m.provider)),merged=[...models,...this.catalog().models.filter(m=>!refreshed.has(m.provider))];const value={models:merged.slice(0,2000).map(m=>({provider:m.provider,id:m.id,label:m.label.slice(0,200)})),updatedAt:new Date().toISOString()};this.db.prepare("INSERT OR REPLACE INTO sw_generation_model_catalog VALUES(1,?)").run(JSON.stringify(value));return value;}
  close(){this.db.close();}
...
export function normalizeGenerationSettings(input:Partial<StreamWeaverGenerationSettings>):StreamWeaverGenerationSettings {
  ...
  if(v.cloudflareModel!==undefined&&v.cloudflareModel!==""&&!CLOUDFLARE_IMAGE_MODELS.some(m=>m.id===v.cloudflareModel))throw Error("Cloudflare image model is invalid");
  ...
  if(typeof v.contentModeration!=="boolean")throw new Error("Choose whether to check image prompts");
