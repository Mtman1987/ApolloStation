...
export class StreamWeaverWebControls {
    ...
    try {
      ...
      if(url.pathname==="/api/streamweaver/control/stream-operations"){
        ...
        if(body.action==="redeem-request"){
          ...
          return sendJson(response,200,await this.communityRuntime().redeemReward({tenantId:context.tenantId,userId:this.actor(context).id,displayName:this.actor(context).displayName,rewardId:String(body.rewardId??""),requestId:String(body.requestId??""),currency,...(body.maxSpmtCost===undefined?{}:{maxSpmtCost:Number(body.maxSpmtCost)})}));
        }
        ...
        if(body.action==="ride-settings"){
          const rewardId=String(body.rewardId??"");if(body.enabled===true&&(!this.community.redeems(context.tenantId).some(r=>r.id===rewardId&&r.enabled)||this.community.partners(context.tenantId).some(p=>p.rewardId===rewardId)))throw Error("Choose an enabled reward that is not assigned to an individual check-in");
          return sendJson(response,200,this.community.rides.configure(context.tenantId,{enabled:body.enabled as boolean,rewardId}));
        }
      ...
      if(["/api/streamweaver/control/pokemon/pokedex","/api/streamweaver/control/pokemon/export","/api/streamweaver/control/pokemon/card"].includes(url.pathname)){
        if(!this.pokemon)throw Error("Pokémon storage is not configured");if(request.method!=="GET")return sendJson(response,405,{message:"Use GET"});const actor={...this.actor(context),owner:this.role(context)==="owner"};
        if(url.pathname.endsWith("/pokedex"))return sendJson(response,200,{...this.pokemon.pokedex(context.tenantId,actor,url.searchParams.get("search")??"",Number(url.searchParams.get("offset")??0)),downloadsEnabled:this.operationMode==="active"});
        if(url.pathname.endsWith("/export")){response.writeHead(200,{"content-type":"application/json","content-disposition":'attachment; filename="pokemon-collection.json"',"cache-control":"no-store","x-content-type-options":"nosniff"});response.end(JSON.stringify(this.pokemon.exportCollection(context.tenantId,actor),null,2));return true;}
        const card=this.pokemon.ownedCard(context.tenantId,actor,url.searchParams.get("id")??"");if(url.searchParams.get("download")!=="png")return sendJson(response,200,{card});if(this.operationMode!=="active")throw Error("External card downloads are disabled here; collection JSON is available");
        if(!/^[A-Za-z0-9_-]{1,40}$/.test(card.setCode)||!/^[A-Za-z0-9_-]{1,40}$/.test(card.number))throw Error("This imported card has no supported TCG image identifier");
        const asset=await(this.options.fetchImpl??fetch)(`https://images.pokemontcg.io/${card.setCode}/${card.number}_hires.png`,{redirect:"error",signal:AbortSignal.timeout(20000)});if(!asset.ok||!asset.body)throw Error("Card image is unavailable");if(Number(asset.headers.get("content-length")??0)>12*1024*1024){await asset.body.cancel();throw Error("Card image is too large");}const reader=asset.body.getReader(),chunks:Uint8Array[]=[];let size=0;try{for(;;){const part=await reader.read();if(part.done)break;size+=part.value.length;if(size>12*1024*1024)throw Error("Card image is too large");chunks.push(part.value);}}finally{await reader.cancel().catch(()=>{})}const bytes=Buffer.concat(chunks);if(!bytes.subarray(0,8).equals(Buffer.from([137,80,78,71,13,10,26,10])))throw Error("Card provider did not return a PNG image");response.writeHead(200,{"content-type":"image/png","content-disposition":`attachment; filename="${card.setCode}-${card.number}.png"`,"content-length":String(bytes.length),"cache-control":"private, no-store","x-content-type-options":"nosniff"});response.end(bytes);return true;
      }
      if(url.pathname==="/api/streamweaver/control/pokemon") {
        ...
        if(body.action==="fetch-set"){
          ...
          for(let page=1;page<=8;page++){
            ...
            for(const c of payload.data){name=String(c.set?.name??set);cards.push({name:String(c.name),number:String(c.number),setCode:set,rarity:String(c.rarity??"Common"),...(c.images?.small?{imageUrl:String(c.images.small)}:{}),...(c.supertype?{supertype:String(c.supertype)}:{}),...(c.hp?{hp:String(c.hp)}:{}),...(Array.isArray(c.nationalPokedexNumbers)?{nationalPokedexNumbers:c.nationalPokedexNumbers.filter((n:unknown)=>Number.isSafeInteger(n))}:{}),...(Array.isArray(c.types)?{types:c.types}:{}),...(Array.isArray(c.attacks)?{attacks:c.attacks}:{}),...(Array.isArray(c.weaknesses)?{weaknesses:c.weaknesses}:{}),...(Array.isArray(c.resistances)?{resistances:c.resistances}:{})});}
  ...
  private async previewFlow(response: ServerResponse, context: SessionContext, body: Record<string, unknown>) {
    ...
    for(const [index,output] of preview.outputs.entries()){
      ...
      events.push(await client.publishSimulationRoomEvent(context.tenantId,{roomId,roomName:"StreamWeaver flow previews",lane,direction:lane==="chat"?"egress":"preview",title:`${command.trigger} · ${output.type}`,body:output.text,provider,connectionId:"simulation",channelId:roomId,replyToMessageId:delivery.message.messageId,data:{packageId,commandId:command.id,actionId:output.actionId,actionType:output.type}},`flow-preview:${nonce}:output:${index}`));
    }
    if(!preview.outputs.length)events.push(await client.publishSimulationRoomEvent(context.tenantId,{roomId,roomName:"StreamWeaver flow previews",lane:"app",direction:"preview",title:`${command.trigger} completed`,body:"This command completed without a visible output.",data:{packageId,commandId:command.id}},`flow-preview:${nonce}:empty`));
  ...
  private communityRuntime(){if(!this.community||!this.economy||!this.client)throw Error("Community runtime is unavailable");const active=this.operationMode==="active",grants=new SpmtStreamWeaverTwitchGrantSource(this.client,tenant=>this.runtimeSettings?.twitchBroadcaster(tenant),active),twitch=new StreamWeaverTwitchCommandAdapter(grants,this.options.fetchImpl);return new StreamWeaverCommunityRuntime(this.community,this.economy,this.client,active,active,activeRideLookup(this.community,twitch,this.client));}
