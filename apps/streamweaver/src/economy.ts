...
export type StreamWeaverEconomyReceiptV1 = { operationId: string; kind: "give" | "steal" | "gamble" | "roll" | "admin" | "provider-award"|"ride-front-seat" | "redeem" | "redeem-claim"; result: Record<string, unknown>; createdAt: string; };
...
export interface StreamWeaverEconomyStoreV1 {
  transaction<T>(fn:()=>T,context?:StreamWeaverCurrencyOperationV1):T;
...
export class MemoryStreamWeaverEconomyStore implements StreamWeaverEconomyStoreV1 {
  ...
  private jackpotAt = 0;
  transaction<T>(fn:()=>T,_context?:StreamWeaverCurrencyOperationV1):T {
    const maps=[this.cooldowns,this.receipts,this.wallets,this.settings,this.exchanges] as Map<string,unknown>[],copies=maps.map(map=>structuredClone(map)),jackpot=this.jackpotAt;
    try{return fn();}catch(error){maps.forEach((map,index)=>{map.clear();for(const [key,value] of copies[index]!)map.set(key,value);});this.jackpotAt=jackpot;throw error;}
  }
...
export class SqliteStreamWeaverEconomyStore implements StreamWeaverEconomyStoreV1 {
  ...
  private operation:StreamWeaverCurrencyOperationV1|undefined;
...
CREATE TABLE IF NOT EXISTS streamweaver_currency_ledger(id INTEGER PRIMARY KEY,tenant_id TEXT NOT NULL,user_id TEXT NOT NULL,balance_before INTEGER NOT NULL,balance_after INTEGER NOT NULL,delta INTEGER NOT NULL,reason TEXT NOT NULL,operation_id TEXT,actor_id TEXT,occurred_at TEXT NOT NULL) STRICT;
CREATE INDEX IF NOT EXISTS streamweaver_currency_ledger_tenant ON streamweaver_currency_ledger(tenant_id,id);
...
CREATE TABLE IF NOT EXISTS streamweaver_currency_exchange(tenant_id TEXT NOT NULL,operation_id TEXT NOT NULL,user_id TEXT NOT NULL,body TEXT NOT NULL,status TEXT NOT NULL CHECK(status IN ('pending','complete')),local_spent INTEGER NOT NULL CHECK(local_spent>0),updated_at TEXT NOT NULL,PRIMARY KEY(tenant_id,operation_id)) STRICT;`);
  ...
  transaction<T>(fn:()=>T,context?:StreamWeaverCurrencyOperationV1):T {
    ...
    this.db.exec("BEGIN IMMEDIATE");this.transactionDepth++;const previous=this.operation;this.operation=context;
    try{const result=fn();this.db.exec("COMMIT");return result;}catch(error){this.db.exec("ROLLBACK");throw error;}finally{this.transactionDepth--;this.operation=previous;}
  }
  listLedger(tenantId:string,options:{userId?:string;before?:number;limit?:number}={}) {
    const limit=options.limit??50,before=options.before??Number.MAX_SAFE_INTEGER;if(!Number.isSafeInteger(limit)||limit<1||limit>200||!Number.isSafeInteger(before)||before<1)throw new Error("Invalid currency ledger page");
    return this.db.prepare("SELECT id,tenant_id AS tenantId,user_id AS userId,balance_before AS before,balance_after AS after,delta,reason,operation_id AS operationId,actor_id AS actorId,occurred_at AS occurredAt FROM streamweaver_currency_ledger WHERE tenant_id=? AND id<? AND (? IS NULL OR user_id=?) ORDER BY id DESC LIMIT ?").all(tenantId,before,options.userId??null,options.userId??null,limit) as unknown as StreamWeaverCurrencyLedgerEntryV1[];
  }
  ...
  adjustOnce(tenantId:string,userId:string,mode:"add"|"set",amount:number,operationId:string,actorId:string) {
    ...
    }, {reason:`owner-${mode}`,operationId,actorId});
  }
  ...
  private upsert(w:StreamWeaverCurrencyWalletV1){const before=this.getWallet(w.tenantId,w.userId).balance;if(before!==w.balance)this.db.prepare("INSERT INTO streamweaver_currency_ledger(tenant_id,user_id,balance_before,balance_after,delta,reason,operation_id,actor_id,occurred_at) VALUES(?,?,?,?,?,?,?,?,?)").run(w.tenantId,w.userId,before,w.balance,w.balance-before,this.operation?.reason??"balance-update",this.operation?.operationId??null,this.operation?.actorId??null,new Date().toISOString());this.db.prepare("INSERT INTO streamweaver_currency_wallet(tenant_id,user_id,balance,total_earned,updated_at) VALUES(?,?,?,?,?) ON CONFLICT(tenant_id,user_id) DO UPDATE SET balance=excluded.balance,total_earned=excluded.total_earned,updated_at=excluded.updated_at").run(w.tenantId,w.userId,w.balance,w.totalEarned,new Date().toISOString());}
  ...
  private tx<T>(fn:()=>T,context?:StreamWeaverCurrencyOperationV1){return this.transaction(fn,context);}
}
...
/** Exact integer settlement against outstanding supplies: floor payouts, ceil costs. */
export function calculateStreamWeaverSupplyRate(localSupplyInput:number,spmtSupplyInput:number) {
  const localSupply=positiveAmount(localSupplyInput),spmtSupply=positiveAmount(spmtSupplyInput);
  const ratio=(amount:number,numerator:number,denominator:number,roundUp:boolean)=>{
    const product=BigInt(safeNonNegative(amount,"amount"))*BigInt(numerator),d=BigInt(denominator);
    const value=(product+(roundUp?d-1n:0n))/d;
    if(value>BigInt(Number.MAX_SAFE_INTEGER))throw new Error("Exchange amount exceeds the supported range");
    return Number(value);
  };
  return {localSupply,spmtSupply,localPerSpmt:localSupply/spmtSupply,spmtPerLocal:spmtSupply/localSupply,
    localToSpmt:(amount:number)=>ratio(amount,spmtSupply,localSupply,false),
    spmtToLocal:(amount:number)=>ratio(amount,localSupply,spmtSupply,true),
    localCostInSpmt:(amount:number)=>ratio(amount,spmtSupply,localSupply,true)};
}
