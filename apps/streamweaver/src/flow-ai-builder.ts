...
export function buildStreamWeaverAiFlowPrompt(idea: string, context: StreamWeaverAiFlowPromptContextV1 = {}) {
  ...
  const prompt = [
    ...
    "Actions: send-chat{text}; send-discord{text,connectionId?,channelId?}; wait{milliseconds}; run-action{action,args,saveAs?,sendResult?}; run-native{donorId}; set-variable{key,value,scope}; condition{left,operator,right}; ai-response{input,saveAs?}; obs-scene{deviceId,sceneName}; obs-source{deviceId,sceneName,sourceName,visible}; device-command{deviceId,action,payload}. Never emit http-request or execute-code.",
