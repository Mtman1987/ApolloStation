...
export function buildStreamWeaverAiFlowPrompt(idea: string, context: StreamWeaverAiFlowPromptContextV1 = {}) {
  ...
  const prompt = [
    ...
    "Each action is {id,type,enabled:true,config:{...}}. Config by type: send-chat{text}; send-discord{text,connectionId?,channelId?}; wait{milliseconds}; run-action{action,args,saveAs?,sendResult?}; run-native{capability:streamweaver.donor-command.v1,donorId,...settings}; set-variable{key,value,scope}; condition{left,operator,right}; ai-response{input,saveAs?}; obs-scene{deviceId,sceneName}; obs-source{deviceId,sceneName,sourceName,visible}; device-command{deviceId,action,payload}. Never emit http-request or execute-code.",
