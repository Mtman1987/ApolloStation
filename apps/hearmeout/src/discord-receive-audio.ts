...
export class HearMeOutDiscordPcmJitterSource {
  ...
  constructor(profile: HearMeOutDiscordReceiveProfileV1 = "clean") {
    this.config = HEARMEOUT_DISCORD_RECEIVE_PROFILES[normalizeHearMeOutDiscordReceiveProfile(profile)];
    this.targetFrames = this.config.targetFrames;
    this.metrics = { starts: 0, speechEnds: 0, underruns: 0, rebuffers: 0, lateFrames: 0, droppedFrames: 0, concealedFrames: 0, currentBufferedFrames: 0, targetFrames: this.targetFrames, arrivalJitterMs: 0 };
  }
  ...
  push(chunk: Buffer | Uint8Array, now = Date.now()): void {
    ...
    if (this.starved && this.started) { this.metrics.lateFrames += Math.max(1, Math.floor(evenLength / HEARMEOUT_DISCORD_BYTES_PER_FRAME)); this.needsAttack = true; }
    ...
    if (this.buffer.length > maxBytes) {
      const wholeFrames = Math.ceil((this.buffer.length - maxBytes) / HEARMEOUT_DISCORD_BYTES_PER_FRAME);
      this.buffer = this.buffer.subarray(Math.min(this.buffer.length, wholeFrames * HEARMEOUT_DISCORD_BYTES_PER_FRAME));
      this.metrics.droppedFrames += wholeFrames;
    }
    this.updateBufferedMetric();
  }

  pullFrame(now = Date.now()): Buffer<ArrayBufferLike> {
    if (!this.started) {
      ...
      this.started = true; this.starved = false; this.emptyPulls = 0; this.needsAttack = true; this.metrics.starts += 1;
    }
    if (this.buffer.length < HEARMEOUT_DISCORD_BYTES_PER_FRAME) {
      this.emptyPulls += 1; this.metrics.concealedFrames += 1; this.starved = true;
      if (this.emptyPulls >= 2) {
        this.started = false; this.emptyPulls = 0; this.firstQueuedAt = undefined; this.metrics.underruns += 1; this.metrics.rebuffers += 1;
        this.targetFrames = Math.min(this.config.adaptiveMaxFrames, this.targetFrames + 2); this.metrics.targetFrames = this.targetFrames; this.needsAttack = true; this.stableFrames = 0;
      }
      return this.silence();
    }
    let frame: Buffer<ArrayBufferLike> = Buffer.from(this.buffer.subarray(0, HEARMEOUT_DISCORD_BYTES_PER_FRAME));
    this.buffer = this.buffer.subarray(HEARMEOUT_DISCORD_BYTES_PER_FRAME);
    this.emptyPulls = 0; this.starved = false;
    if (this.needsAttack) { frame = fadePcm16Edge(frame, this.config.fadeSamples, "in"); this.needsAttack = false; }
    if (this.buffer.length < HEARMEOUT_DISCORD_BYTES_PER_FRAME) { frame = fadePcm16Edge(frame, this.config.fadeSamples, "out"); this.metrics.speechEnds += 1; }
    this.stableFrames += 1;
    if (this.stableFrames >= 1_500 && this.targetFrames > this.config.targetFrames) { this.targetFrames -= 1; this.metrics.targetFrames = this.targetFrames; this.stableFrames = 0; }
    this.updateBufferedMetric();
    return frame;
  }

  snapshot(): HearMeOutDiscordReceiveMetricsV1 { this.updateBufferedMetric(); return { ...this.metrics, arrivalJitterMs: Math.round(this.metrics.arrivalJitterMs * 100) / 100 }; }
  bufferedFrames(): number { return Math.floor(this.buffer.length / HEARMEOUT_DISCORD_BYTES_PER_FRAME); }
  private silence(): Buffer<ArrayBufferLike> { this.updateBufferedMetric(); return Buffer.alloc(HEARMEOUT_DISCORD_BYTES_PER_FRAME); }
  private updateBufferedMetric() { this.metrics.currentBufferedFrames = this.bufferedFrames(); }
}
...
export function fadePcm16Edge(frame: Buffer | Uint8Array, fadeSamples: number, direction: "in" | "out"): Buffer<ArrayBufferLike> {
  const output: Buffer<ArrayBufferLike> = Buffer.from(frame);
  const stereoFrames = Math.floor(Math.floor(output.length / 2) / HEARMEOUT_DISCORD_CHANNELS);
