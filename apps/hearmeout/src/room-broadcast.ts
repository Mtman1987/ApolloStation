...
export class HearMeOutRoomBroadcast {
  ...
  private async start(run:Run,signature:string){
    ...
    const args=['-hide_banner','-loglevel','error','-nostdin','-y','-threads','2',...inputArgs(source,hlsSource),...(audioSource?inputArgs(audioSource):[]),...(media.hasVideo?['-map','0:v:0']:[]),...media.audio.flatMap(track=>['-map',track.sourceSpecifier??'0:'+track.sourceIndex]),'-c:v','libx264','-threads:v','2','-filter_threads','1','-preset','veryfast','-pix_fmt','yuv420p','-force_key_frames','expr:gte(t,n_forced*2)','-c:a','aac','-ac','2','-b:a','128k',...(audioSource?['-shortest']:[]),'-f','hls','-hls_time','2','-hls_list_size','30','-hls_delete_threshold','10','-hls_flags','delete_segments+discont_start+omit_endlist+temp_file','-var_stream_map',variants,'-master_pl_name','index.m3u8','-hls_segment_filename',join(dir,epoch+'_%v_%06d.ts'),join(dir,'stream_%v.m3u8')];
  ...
  async serve(tenantId:string,roomId:string,lane:'music'|'movie',file:string,response:ServerResponse){
    ...
    try{const bytes=await readFile(path);response.writeHead(200,{'content-type':file.endsWith('.m3u8')?'application/vnd.apple.mpegurl':'video/mp2t','cache-control':file.endsWith('.m3u8')?'no-store':'public, max-age=120, immutable','x-content-type-options':'nosniff'});response.end(bytes);}catch{this.unavailable(response,503);}
  }
