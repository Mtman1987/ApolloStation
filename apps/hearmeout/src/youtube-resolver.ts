...
import { preparedHearMeOutUrl } from "./prepared-media.js";
...
export class HearMeOutYoutubeResolverCoordinator {
  constructor(private readonly adapter:HearMeOutYoutubeResolverAdapterV1,private readonly options:{now?:()=>string;cookiesAvailable?:()=>boolean;preparedMediaOrigin?:string}={}){}
  ...
  private async tryStage(stage:HearMeOutYoutubeResolverStageV1,run:()=>Promise<HearMeOutResolvedYoutubeV1|null>,attempts:HearMeOutYoutubeResolveAttemptV1[]){try{const value=await run();if(!value){attempts.push({stage,outcome:"miss"});return null;}const normalized=normalizeHearMeOutResolvedYoutube({...value,stage},this.options.now?.()??new Date().toISOString(),stage==='upstream'?this.options.preparedMediaOrigin:undefined);attempts.push({stage,outcome:"hit"});return normalized;}catch(error){attempts.push({stage,outcome:"error",message:safeError(error)});return null;}}
}

export function normalizeHearMeOutResolvedYoutube(input:Partial<HearMeOutResolvedYoutubeV1>&{videoId:string;videoUrl:string;audioUrl:string},now=new Date().toISOString(),preparedMediaOrigin?:string):HearMeOutResolvedYoutubeV1{
  const videoId=cleanVideoId(input.videoId),videoUrl=allowedMediaUrl(input.videoUrl,"videoUrl",preparedMediaOrigin),audioUrl=allowedMediaUrl(input.audioUrl,"audioUrl",preparedMediaOrigin),durationMs=input.durationMs===undefined?undefined:Math.max(0,Math.min(24*60*60*1000,Math.trunc(Number(input.durationMs)||0))),stage=input.stage??"browser-resolved";return{videoId,videoUrl,audioUrl,...(input.title?{title:String(input.title).trim().slice(0,300)}:{}),...(durationMs!==undefined?{durationMs}:{}),stage,resolvedAt:timestamp(input.resolvedAt??now)};
}
...
function allowedMediaUrl(value:string,name:string,preparedOrigin?:string){if(!isAllowedHearMeOutYoutubeMediaUrl(value)&&!(preparedOrigin&&preparedHearMeOutUrl(value,preparedOrigin)))throw new Error(`${name} host is not allowed`);return new URL(value).toString();}
