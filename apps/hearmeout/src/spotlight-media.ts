...
function renderSpotlightPlayer() {
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Spotlight Media player</title><style>html,body,#player{margin:0;width:100%;height:100%;background:#000;overflow:hidden}#readonly{position:fixed;z-index:2;inset:0;background:transparent;pointer-events:none}#sound{position:fixed;z-index:3;left:50%;top:50%;transform:translate(-50%,-50%);padding:12px 18px;border:0;border-radius:999px;background:#ff7a3d;color:#fff;font:700 15px system-ui;cursor:pointer;box-shadow:0 6px 24px #0008}#sound[hidden]{display:none}#notice{position:fixed;z-index:4;left:12px;bottom:12px;padding:8px 10px;border-radius:8px;background:#071025d9;color:#dceaff;font:13px system-ui;pointer-events:none}</style></head><body><div id="player"></div><div id="readonly" aria-hidden="true"></div><button id="sound" type="button" disabled>Loading Spotlight…</button><div id="notice">Loading Spotlight Media…</div><script>
const ROTATE_MS=600000,REFRESH_MS=60000;let list=[],index=-1,player,rotateTimer,refreshTimer,audioStarted=false,pendingStart=false;const notice=document.querySelector('#notice'),sound=document.querySelector('#sound'),readonly=document.querySelector('#readonly');
