...
function twitchParents(){const hosts=[location.hostname];try{const ref=new URL(document.referrer).hostname;if(ref&&!hosts.includes(ref))hosts.push(ref)}catch{}for(const host of ['spmt.live','www.spmt.live'])if(!hosts.includes(host))hosts.push(host);return hosts}\nasync function start(){list=await load();if(!list.length){notice.textContent='No approved live creators right now.';sound.hidden=true;return}const first=list[slotIndex()];const script=document.createElement('script');script.src='https://player.twitch.tv/js/embed/v1.js';script.onload=()=>{player=new Twitch.Player('player',{channel:first.username,width:'100%',height:'100%',parent:twitchParents(),autoplay:true,muted:true,controls:false});player.addEventListener(Twitch.Player.READY,()=>{index=slotIndex();player.play();if(pendingStart||audioStarted)startPlayback();notice.textContent='@'+list[index].username+' · Spotlight Media';schedule();refreshTimer=setInterval(refresh,REFRESH_MS)})};document.head.append(script)}
function startPlayback(){audioStarted=true;pendingStart=true;if(!player)return;pendingStart=false;player.setMuted(false);player.setVolume(1);player.play();sound.hidden=true;readonly.style.pointerEvents='auto'}\nsound.addEventListener('click',startPlayback);readonly.addEventListener('click',startPlayback);
