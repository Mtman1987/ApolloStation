...
function openWatch(){const watch=document.querySelector('[data-hmo-watch-drawer]');if(!watch)return;watch.hidden=false;document.querySelector('[data-hmo-watch-icon]')?.setAttribute('aria-expanded','true');if(watch.dataset.hmoBroadcastWindow==='1'&&!watch.querySelector('iframe')){const frame=document.createElement('iframe');frame.dataset.hmoBroadcastFrame='1';frame.src='/watch?embed=1&appRoomId='+encodeURIComponent(watch.closest('.hmo-console')?.dataset.roomId||'');frame.title='Music and movie window';frame.allow='autoplay; fullscreen';frame.allowFullscreen=true;frame.style.cssText='display:block;border:0;width:100%;height:480px;max-height:65vh';watch.querySelector('[data-hmo-broadcast-host]').replaceChildren(frame)}watch.scrollIntoView({block:'nearest'})}
