...
export async function handleHearMeOutBroadcastWindow(request:IncomingMessage,response:ServerResponse,url:URL,program:HearMeOutBroadcastProgram,worker:HearMeOutRoomBroadcast|undefined,media:HearMeOutSuiteMediaResolverV1|undefined,clientId='',readOnly=false,hosting?:{guildIds?:string[];authorizeRoom:(request:IncomingMessage,roomId:string)=>Promise<void>},screens?:HearMeOutScreenBroadcast){
  ...
  const partyClose=url.pathname.match(/^\/api\/watch\/broadcast\/rooms\/([^/]+)$/);
  ...
  if(!screenFeed&&!parties&&!partyClose&&!entry&&!state&&!feed&&!defaults&&!requestVideo&&!movieSearch&&!browserMedia&&!control)return false;
  try{
    ...
    if(partyClose){
      if(request.method!=='DELETE')return send(response,405,{error:'Method not allowed'});
      if(readOnly)return send(response,503,{error:'Closing watch parties is unavailable in this preview'});
      ...
      const roomId=decodeURIComponent(partyClose[1]!);program.getRoom(roomId);await authorizeHost(roomId);
      screens?.stopRoom(roomId);
      const deleted=program.deleteRoom(roomId);
      return send(response,deleted?200:404,{deleted,roomId,voiceRoomPreserved:true});
    }
    if(parties){
      const viewer=guest(request,response);program.pruneExpiredDiscordRooms();
      const currentChannel=channel();if(currentChannel)program.touchChannelRoom(currentChannel);
    ...
    if(roomId){program.getRoom(roomId);const currentChannel=channel();if(currentChannel&&program.channelRoom(currentChannel)?.roomId===roomId)program.touchRoom(roomId);}
...
(()=>{
  ...
  const movieResults=document.getElementById('movie-results'),requestForm=document.getElementById('request-form'),queueControls=document.getElementById('queue-controls'),closeParty=document.getElementById('close-party');
  ...
  function applyState(state){
    ...
    const canManage=state.canManage===true;requestForm.hidden=!canManage;queueControls.hidden=!canManage;closeParty.hidden=!canManage;skip.disabled=!canManage||controlBusy||!state.current;clearQueue.disabled=!canManage||controlBusy||!state.queue.length;
  ...
  closeParty.addEventListener('click',async()=>{
    if(!activeParty||latestState?.canManage!==true||closeParty.disabled)return;
    if(!window.confirm('Close this watch party? Your HearMeOut room or Discord voice channel will stay open.'))return;
    closeParty.disabled=true;error.textContent='';
    try{const path='/api/watch/broadcast/rooms/'+encodeURIComponent(activeParty)+(hostingParams.size?'?'+hostingParams:'');await api(path,{method:'DELETE'});hostedRoomId='';showParties();}
    catch(e){error.textContent=e.message;}finally{closeParty.disabled=false;}
  });
