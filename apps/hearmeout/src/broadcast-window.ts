...
export function broadcastView(program:HearMeOutBroadcastProgram,configured:boolean,ready=false,epoch?:string,roomId?:string){
  ...
  const radio=program.radio(roomId);
  return {sessionId:roomId,current:visible(session.current),queue:session.queue.map(request=>visible(request)!),playback:session.playback,revision:session.revision,...(radio?{autoRadio:{enabled:radio.enabled}}:{}),broadcast:{configured,ready,...(epoch?{epoch}:{}),playbackUrl:'/api/watch/sessions/'+encodeURIComponent(roomId)+'/broadcast/index.m3u8'}};
}
...
export async function handleHearMeOutBroadcastWindow(request:IncomingMessage,response:ServerResponse,url:URL,program:HearMeOutBroadcastProgram,worker:HearMeOutRoomBroadcast|undefined,media:HearMeOutSuiteMediaResolverV1|undefined,clientId='',readOnly=false,hosting?:{guildIds?:string[];authorizeRoom:(request:IncomingMessage,roomId:string)=>Promise<void>},screens?:HearMeOutScreenBroadcast){
  ...
  if(!twitchRelay&&!screenFeed&&!parties&&!entry&&!state&&!feed&&!defaults&&!requestVideo&&!movieSearch&&!control)return false;
  try{
    if(twitchRelay){
    ...
    if(control){
      ...
      const body=JSON.parse(Buffer.concat(chunks).toString('utf8'));
      if(!body||!['skip','clear','autoplay'].includes(body.action))throw Error('Choose skip, clear queue, or autoplay');
      if(body.expectedRequestId!==undefined&&typeof body.expectedRequestId!=='string')throw Error('Invalid request reference');
      if(body.action==='autoplay'){
        if(typeof body.enabled!=='boolean')throw Error('Choose whether autoplay is enabled');
        program.configureRadio(roomId,body.enabled);
      }else program.control({tenantId:program.binding.tenantId,userId:guest(request,response),displayName:'Viewer',roles:[]},{roomId,action:body.action,...(body.expectedRequestId?{expectedRequestId:body.expectedRequestId}:{})});
    ...
    if(requestVideo){
      ...
      await program.request({roomId,requesterId,displayName:typeof body.displayName==='string'?body.displayName.replace(/[\r\n\0]/g,' ').slice(0,120):'Viewer',query:body.query,lane:body.lane??'movie',...(body.selectedItemId?{selectedItemId:body.selectedItemId}:{}),operationId:rawKey??randomUUID()},media);
...
(()=>{
  ...
  const playbackError=document.getElementById('playback-error'),disconnect=document.getElementById('disconnect'),retry=document.getElementById('retry'),audioToggle=document.getElementById('audio-toggle'),videoToggle=document.getElementById('video-toggle'),autoplayToggle=document.getElementById('autoplay-toggle');
  ...
  let level=Number(preferences.getItem('hmo-broadcast-volume')||0),lastAudible=level||85,sourceUrl='',busy=false,disposed=false,pendingRequest,requestInFlight=false,lastRevision=-1,playPending,playbackUnlocked=false,selectedMovie,connected=true,currentRequest='',currentEpoch='',retryAt=0,latestState,controlBusy=false,statePollFailed=false;
  ...
  function updateSoundLabel(){sound.textContent=!playbackUnlocked?'Play / enable sound':level?'Mute locally':'Enable sound';}
  function setVolume(value){level=Math.max(0,Math.min(100,value));if(level)lastAudible=level;video.volume=level/100;volume.value=String(level);updateSoundLabel();preferences.setItem('hmo-broadcast-volume',String(level));}
  ...
  function play(userInitiated=false){
    ...
    if(playPending&&!userInitiated)return playPending;
    const attempt=video.play().then(()=>{if(userInitiated){playbackUnlocked=true;updateSoundLabel();}}).catch(e=>{if(disposed||!sourceUrl||e.name==='AbortError')return;if(e.name==='NotAllowedError')status.textContent='Tap Play / enable sound once; later items will start automatically';else error.textContent=e.message;});
    if(userInitiated)return attempt;
    playPending=attempt.finally(()=>{playPending=undefined});return playPending;
  }
  function applyState(state){
    ...
    const canManage=state.canManage===true;requestForm.hidden=!canManage;queueControls.hidden=!canManage;skip.disabled=!canManage||controlBusy||!state.current;clearQueue.disabled=!canManage||controlBusy||!state.queue.length;autoplayToggle.hidden=!canManage||!state.autoRadio;autoplayToggle.disabled=!canManage||controlBusy||!state.autoRadio;autoplayToggle.textContent='Autoplay: '+(state.autoRadio?.enabled?'on':'off');autoplayToggle.setAttribute('aria-pressed',String(Boolean(state.autoRadio?.enabled)));
  ...
  audioToggle.addEventListener('click',()=>{setAudioEnabled(!audioEnabled);if(audioEnabled&&level===0)setVolume(lastAudible);void play(audioEnabled)});
  videoToggle.addEventListener('click',()=>setVideoEnabled(!videoEnabled));
  sound.addEventListener('click',()=>{if(!playbackUnlocked){if(!audioEnabled)setAudioEnabled(true);if(!level)setVolume(lastAudible);void play(true);return;}if(!audioEnabled)setAudioEnabled(true);setVolume(level?0:lastAudible);void play(true)});
  volume.addEventListener('input',event=>{if(!audioEnabled&&Number(event.target.value)>0)setAudioEnabled(true);setVolume(Number(event.target.value));void play(true)});
  ...
  autoplayToggle.addEventListener('click',async()=>{if(controlBusy||!latestState?.autoRadio||latestState.canManage!==true)return;controlBusy=true;autoplayToggle.disabled=true;error.textContent='';try{applyState(await api('/api/watch/broadcast/control',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'autoplay',enabled:!latestState.autoRadio.enabled})}));}catch(e){error.textContent=e.message;}finally{controlBusy=false;if(latestState)applyState(latestState);}});
  requestForm.addEventListener('submit',async event=>{
    ...
    try{
      ...
      const state=await api('/api/watch/broadcast/requests',{method:'POST',headers:{'content-type':'application/json','idempotency-key':pendingRequest.key},body:JSON.stringify({query,lane,...(selectedItemId?{selectedItemId}:{})})});
