...
export class HearMeOutRoomAssistantJobs {
 ...
 async submit(scope:Scope,input:{message:string;personaId:string;displayName:string;speak:boolean}|{mediaAssetId:string},key:string,request:RoomAssistantRequest){
  ...
  if(!binding.jobId){const accepted='message' in input?(input.personaId.startsWith('swpublic_')?await request('/v1/assistant/public-personas/invocations',{personaId:input.personaId,roomId:scope.roomId,message:input.message},`hmo-reply:${id}`):await request('/v1/assistants/community/invocations',{userId:scope.userId,message:input.message,surface:'app',conversationId:`hearmeout:${scope.roomId}`,routingPreference:'automatic',remember:false},`hmo-reply:${id}`)):await request('/v1/assistant/speech/transcribe',{mediaAssetId:input.mediaAssetId},`hmo-transcribe:${id}`);
   if(binding.kind==='reply'&&typeof accepted.voice==='string')binding.voice=accepted.voice;if(binding.kind==='reply'&&typeof accepted.displayName==='string')binding.displayName=accepted.displayName;binding.jobId=accepted.jobId??accepted.job?.id;if(!binding.jobId)throw Error(accepted.reason??'The assistant did not accept this request');this.save(scope,binding);
  }
 ...
 async read(scope:Scope,id:string,request:RoomAssistantRequest,append:(id:string,personaId:string,name:string,text:string)=>void){
  ...
  try {
  if(!binding.speechJobId){const accepted=await request('/v1/assistant/speech/synthesize',{text:reply.slice(0,20000),...(binding.voice?{voice:binding.voice}:{})},`hmo-speech:${id}`);binding.speechJobId=accepted.job?.id;if(!binding.speechJobId)throw Error('The speech request was not accepted');this.save(scope,binding)}
  ...
  return {requestId:id,personaId:binding.personaId,displayName:binding.displayName,state:speech.state,reply,...(mediaAssetId?{audioUrl:'/v1/media/assets/'+encodeURIComponent(mediaAssetId)+'/content?tenantId='+encodeURIComponent(scope.tenantId)}:{}),...(['failed','cancelled','dead-letter'].includes(speech.state)?{error:'The text reply is ready, but speech could not complete'}:{})};
 }
