...
try{
 ...
 provider=createServer(async(req,res)=>{try{assert.equal(req.headers.authorization,'Bearer test-cached-audio-worker');const path=new URL(req.url,'http://fixture').pathname;assert.ok(path.startsWith(`/watch/youtube/hls/${videoId}/`));const name=path.split('/').at(-1);assert.match(name,/^(?:index\.m3u8|seg_\d+\.ts)$/);const bytes=await readFile(join(dir,name));res.writeHead(200,{'content-type':name.endsWith('.ts')?'video/mp2t':'application/vnd.apple.mpegurl'});res.end(bytes);}catch(error){res.writeHead(500);res.end(String(error));}});
 ...
 const bundle=await readFile(new URL('../apps/hearmeout/dist/playback-source-client.js',import.meta.url));
