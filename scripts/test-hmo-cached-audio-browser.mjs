...
const program=new HearMeOutBroadcastProgram(join(dir,'program.sqlite'),{tenantId:'tenant',executionUserId:'owner'}),party=program.createRoom({name:'Cached audio party',requesterId:'owner',operationId:'create-party',sourceRoomId:'cached-audio-room'}),partyId=party.roomId;
...
try{
 ...
 await program.request({roomId:partyId,requesterId:'guest:test',displayName:'Viewer',query:'Cached song',lane:'music',operationId:'cached-song'},{async resolve(){return {itemId:videoId,title:'Cached song',type:'music',source:'fixture',playbackUrl:resolved.audioUrl};}});
