...
try{
 ...
 const roomId='view-room',party=program.createRoom({name:'Viewing room',requesterId:'owner',operationId:'create-party',sourceRoomId:roomId}),partyId=party.roomId;
 const view=()=>{const session=program.getSession('hls',partyId);return {room:rooms.getRoom('hls',roomId),member:true,viewer:{userId:'owner',canManage:true},music:session,movie:{...session,broadcast:{configured:true,playbackUrl:'/api/watch/sessions/'+encodeURIComponent(partyId)+'/broadcast/index.m3u8'}}};};
 const [bundle,source,youtube]=await Promise.all(['media-client.js','playback-source-client.js','youtube-browser-client.js'].map(file=>readFile(new URL('../apps/hearmeout/dist/'+file,import.meta.url))));
 server=createServer(async(req,res)=>{try{const url=new URL(req.url,'http://local');
  ...
  if(url.pathname==='/api/hearmeout/youtube-browser.js'){res.setHeader('content-type','application/javascript');res.end(youtube);return;}
 ...
 await program.request({roomId:partyId,requesterId:'guest:test',displayName:'Viewer',query:'Film',operationId:'request'},{async resolve(){return {itemId:'hls-film',title:'Multilingual film',type:'movie',source:'fixture',playbackUrl:'https://media.example/v1/media/public/'+'a'.repeat(43),durationSeconds:90};}});
 ...
 await activate(pages[1].getByRole('button',{name:'Enable sound',exact:true}));
 ...
 for(let i=0;i<pages.length;i++){const page=pages[i],other=pages[1-i],otherVolume=await other.evaluate(()=>document.querySelector('video').volume),position=await page.evaluate(()=>document.querySelector('video').currentTime);await activate(page.getByRole('button',{name:'Mute locally',exact:true}));await page.waitForFunction(t=>{const v=document.querySelector('video');return v.volume===0&&!v.muted&&!v.paused&&v.currentTime>t+.2;},position);assert.equal(await other.evaluate(()=>document.querySelector('video').volume),otherVolume);await activate(page.getByRole('button',{name:i?'Enable sound':'Restore sound',exact:true}));}
 assert.equal(program.getSession('hls',partyId).revision,revision);
 ...
 const returned=await open('/activity');await returned.waitForFunction(previous=>document.querySelector('video').currentTime>previous+2,position);assert.equal(worker.status().startedProcesses,starts);
 assert.deepEqual(errors,[]);console.log('PASS: one room-owned encoder supplies native and Activity windows with two audio languages. Local volume changes never alter shared state; active playback continues without viewers and a returning viewer joins the current broadcast.');
