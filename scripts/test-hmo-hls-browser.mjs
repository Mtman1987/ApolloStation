...
try{
 ...
 worker=new HearMeOutRoomBroadcast(rooms,{ffmpegBinary:mediaBinary('ffmpeg'),ffprobeBinary:mediaBinary('ffprobe'),cachePath:join(dir,'broadcast'),spmtOrigin:appOrigin});await worker.listen();timer=setInterval(()=>rooms.advanceRoomTimelines(),500);
