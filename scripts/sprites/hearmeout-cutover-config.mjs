...
export async function hearMeOutCutoverEnvironment(dataRoot) {
  ...
  if (config.broadcast?.lounge && (!config.broadcast.singleProgram || !/^[A-Za-z0-9._:-]{1,160}$/.test(config.broadcast.lounge.roomId ?? 'system-spacemountainlive-lounge') || !/^\w{1,25}$/.test(config.broadcast.lounge.twitchChannel ?? 'mtman1987') || (Boolean(config.broadcast.lounge.guildId) !== Boolean(config.broadcast.lounge.channelId)) || [config.broadcast.lounge.guildId,config.broadcast.lounge.channelId].filter(Boolean).some(id => !/^\d{5,30}$/.test(id)))) throw new Error('24-Hour Lounge broadcast binding is incomplete');
  ...
  return {
    ...
    ...(config.broadcast?.lounge ? { HEARMEOUT_LOUNGE_ENABLED:'1', HEARMEOUT_LOUNGE_ROOM_ID:config.broadcast.lounge.roomId ?? 'system-spacemountainlive-lounge', HEARMEOUT_LOUNGE_NAME:config.broadcast.lounge.name ?? '24-Hour Lounge', HEARMEOUT_LOUNGE_RADIO_SEED:config.broadcast.lounge.radioSeed ?? 'A varied mix of music matching recent 24-Hour Lounge requests', HEARMEOUT_LOUNGE_TWITCH_CHANNEL:config.broadcast.lounge.twitchChannel ?? 'mtman1987', ...(config.broadcast.lounge.guildId?{HEARMEOUT_LOUNGE_GUILD_ID:config.broadcast.lounge.guildId,HEARMEOUT_LOUNGE_CHANNEL_ID:config.broadcast.lounge.channelId}:{}) } : {}),
