...
mapfile -t http_services < <(jq -r 'if type == "array" then .[] else (.services // [])[] end | select(.http_port != null) | .name' <<<"$services_json")
for stale_service in "${http_services[@]}" spmt-qwen; do
  [[ -z "$stale_service" ]] && continue
  if sprite-env services get "$stale_service" >/dev/null 2>&1; then
    printf 'Replacing stale Sprite service: %s\n' "$stale_service"
