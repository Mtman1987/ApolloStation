...
import { manifest as dshManifest, requestNebulaArcadeAnnouncements } from "../apps/discord-stream-hub/dist/index.js";
import { manifest as nebulaManifest, completeNebulaArcadeRound } from "../apps/nebula-arcade/dist/index.js";
...
test("Nebula Arcade tag game outcome crosses Nebula, SPMT, DSH, and StreamWeaver only through public contracts",async()=>{
  const calls=[];
  let eventReads=0;
  const fetchImpl=async(url,init={})=>{
    ...
    if(String(url).includes("/v1/events?"))return Response.json([{id:"event-announcement-7",payload:{kind:"nebula-arcade-round"}}]);
    return Response.json({ok:true});
  };
  const arcade=new SpmtClient({baseUrl:"https://spmt.example",appId:"nebula-arcade",fetchImpl});
  await completeNebulaArcadeRound(arcade,{tenantId:"tenant-a",channelId:"channel-a",roundId:"round-7",winnerUserId:"user-a",taggedUserId:"user-b",completedAt:"2026-08-23T00:00:00.000Z",xpAward:25});
  await requestNebulaArcadeAnnouncements(new SpmtClient({baseUrl:"https://spmt.example",appId:"discord-stream-hub",fetchImpl}),"tenant-a");
  ...
  assert.equal(calls[0].headers["idempotency-key"],"nebula-arcade-round:round-7");
  assert.equal(calls[1].headers["idempotency-key"],"nebula-arcade-round:round-7");
  assert.equal(calls[3].headers["idempotency-key"],"dsh-nebula-arcade:event-round-7");
...
test("tenant context cannot bleed between app clients",async()=>{
  ...
  await requestNebulaArcadeAnnouncements(client,"tenant-one");
  await requestNebulaArcadeAnnouncements(client,"tenant-two");
