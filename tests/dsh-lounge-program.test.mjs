...
test('Lounge main panel embeds the permanent Spotlight Media source instead of rotating URLs',async t=>{
 ...
 assert.equal(response.status,200);assert.match(html,/src="\/spotlight-media\/player"/);assert.match(html,/class="spotlight-media"/);assert.doesNotMatch(html,/streamweaver-new\.fly\.dev|spotlight-lab\/player/);
});
