...
test("live create posts a donor-style Twitch embed and persists the Discord message id",async()=>{const f=fixture();await f.publisher.publish(action("shoutout.create",{member,stream}));const tracked=f.store.get("tenant-a","shoutout","user-a");assert.equal(tracked.channelId,"22222");assert.match(f.calls[0].url,/channels\/22222\/messages$/);const body=JSON.parse(f.calls[0].body);assert.equal(body.embeds[0].title,"🚀 **Space Night**");assert.match(body.embeds[0].author.name,/Captain.*LIVE/);assert.equal(body.embeds[0].footer.text,"Twitch • Space Mountain Crew Shoutout");assert.equal(body.embeds[0].color,0x00D9FF);f.close();});
