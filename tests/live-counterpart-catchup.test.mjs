...
test("HearMeOut shares exact wake routing, bounded explicit Talk, healthy persona command, and padded PCM", async () => {
  ...
  assert.match(HEARMEOUT_PERSONA_TALK_BROWSER_JS, /new MediaRecorder/); assert.match(HEARMEOUT_PERSONA_TALK_BROWSER_JS, /8000/); assert.doesNotMatch(HEARMEOUT_PERSONA_TALK_BROWSER_JS, /continuous\s*=\s*true/); // Explicit bounded browser dictation is now allowed only when server STT is unavailable; behavioral coverage lives in hearmeout-persona-browser.test.mjs.
