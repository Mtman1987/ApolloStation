...
test('local OBS adapter completes the v5 challenge and maps bounded scene/media operations',async()=>{
 ...
 try{
  ...
  await obs.execute({action:'obs.source.visibility.set',payload:{sceneName:'Gameplay',sourceName:'Camera',visible:false}});assert.equal(socket.messages.at(-2).d.requestType,'GetSceneItemId');assert.deepEqual(socket.messages.at(-2).d.requestData,{sceneName:'Gameplay',sourceName:'Camera'});assert.equal(socket.messages.at(-1).d.requestType,'SetSceneItemEnabled');assert.deepEqual(socket.messages.at(-1).d.requestData,{sceneName:'Gameplay',sceneItemId:17,sceneItemEnabled:false});
