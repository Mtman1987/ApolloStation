...
test('the public overlay views the room player without creating sessions, joining voice or controlling playback',async()=>{
 ...
 try{
  ...
  const player=await fetch(base+'/spotlight-media/player');assert.equal(player.status,200);const playerHtml=await player.text();assert.match(playerHtml,/\/api\/spotlight-media\/live\.mp4/);assert.match(playerHtml,/video\.muted=monitor/);assert.doesNotMatch(playerHtml,/new Twitch\.Player|player\.setChannel|source\.syncLive|\.m3u8|Start Spotlight|id="sound"/);
