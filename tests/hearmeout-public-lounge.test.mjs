...
test('the public overlay views the room player without creating sessions, joining voice or controlling playback',async()=>{
 ...
 try{
  ...
  const player=await fetch(base+'/spotlight-media/player');assert.equal(player.status,200);const playerHtml=await player.text();assert.match(playerHtml,/new Twitch\.Player/);assert.match(playerHtml,/\/api\/spotlight-media\/channels/);assert.match(playerHtml,/muted:false/);assert.match(playerHtml,/setMuted\(false\)/);assert.doesNotMatch(playerHtml,/id="sound"|Start Spotlight|spotlight-lab\/player|<iframe/i);
