...
test('Stellar flow-coder prompt is sourced from Apollo tools and teaches branches, AI, devices and Flow Code',()=>{
  ...
  for(const value of ['condition{left,operator,right}','ai-response{input,saveAs?}','obs-source{deviceId,sceneName,sourceName,visible}','hmo.media.request','dsh.shoutouts.post','sw.image.generate','clip=!clip','media.volume.set:media.playback','companion-main:companion','discord:discord-main:chat','{{= argsText | trim | upper }}']) assert.ok(prompt.includes(value),`missing ${value}`);
