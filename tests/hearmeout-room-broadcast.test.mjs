...
test('one actual ffmpeg room broadcast serves many windows, runs without viewers and has a fenced worker lease',{timeout:30000},async()=>{
 ...
 const file=join(dir,'source.mp4');execFileSync(mediaBinary('ffmpeg'),['-hide_banner','-loglevel','error','-f','lavfi','-i','color=c=blue:s=96x64:r=25','-f','lavfi','-i','sine=frequency=440:sample_rate=44100','-t','30','-c:v','libx264','-preset','ultrafast','-g','50','-c:a','aac','-movflags','+faststart',file],{timeout:10000});const bytes=await readFile(file);let reads=0;
