...
test('documentation-heavy edits can read three guide sections, validate and shadow-test before submitting',async()=>{
 let count=0;const names=['read_capabilities','read_flow','read_flow_doc','read_flow_doc','read_flow_doc','validate_flow','run_shadow_tests','submit_flow'];
 const author=new OpenAiToolAuthor({apiKey:'mock-key',fetchImpl:async()=>Response.json({status:'completed',output:[{type:'function_call',name:names[count++],call_id:'call-'+count,arguments:'{}'}]})});
 const result=await author.run({model:'gpt-5.6-luna',instructions:'Test edit',request:'Change title',signal:new AbortController().signal,tools:[...new Set(names)].map(name=>({name,description:name,parameters:{},run:()=>name==='submit_flow'?{accepted:true}:{passed:true}}))});assert.equal(result.requests,8);
});
