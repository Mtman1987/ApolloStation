...
import { CompanionExecutionWorker, CompanionSuiteActionRouter, SqliteCompanionDeviceRelay } from "../apps/companion/dist/index.js";
...
test("one-time Companion bootstrap creates a tenant-scoped revocable worker and carries a MountainView command end to end", async () => {
  ...
  try{
    ...
    const suite=new CompanionSuiteActionRouter(machineClient,{tenantId:"tenant-a",userId:"spoofed-user",username:"spoofed-name",role:"member",deviceId:"companion-pc-1"});
    const suiteAction=await suite.route("hey athena refresh admin calendar","companion-suite-1");
    assert.equal(suiteAction.status,"accepted");assert.equal(suiteAction.action,"dsh.calendar.refresh");assert.equal(suiteAction.job.ownerAppId,"companion");assert.equal(suiteAction.job.executionOwner,"discord-stream-hub");assert.equal(suiteAction.job.billedUserId,"user-a");assert.deepEqual(suiteAction.job.input.actor,{userId:"user-a",username:"user-a",role:"owner"});
    const mismatchedSuite=new CompanionSuiteActionRouter(machineClient,{tenantId:"tenant-a",userId:"user-a",deviceId:"another-device"});
    await assert.rejects(()=>mismatchedSuite.route("hey athena show the admin calendar","companion-suite-mismatch"),/status 403/);
