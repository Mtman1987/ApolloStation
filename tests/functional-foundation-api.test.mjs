...
function setup() {
  ...
  for (const appId of ["stellar-core", "chat-gateway"]) { control.registerApp({ appId, name: appId, description: "Functional foundation fixture", version: "1.0.0", launchUrl: `https://${appId}.example.test/`, allowedScopes: ["jobs:read", "jobs:write", "jobs:work"], surfaces: ["standalone"], status: "active" }); control.installApp("tenant-a", appId); }
  ...
  const stellarToken = auth.issueServiceAccess("stellar-core", "stellar-worker-secret-123456").accessToken, otherWorkerToken = auth.issueServiceAccess("chat-gateway", "chat-worker-secret-123456").accessToken;
  const client = (appId, token) => new SpmtClient({ baseUrl: "https://green.spmt.invalid", appId, getAccessToken: () => token, fetchImpl });
  return { dir, databasePath, store, platformStore, usage, data, api, operations, userAToken: userA, userA: client("spacemountain", userA), userB: client("spacemountain", userB), worker: client("stellar-core", stellarToken), otherWorker: client("chat-gateway", otherWorkerToken) };
}
