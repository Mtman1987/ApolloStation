...
test("SpaceMountain presentation remains finished and dynamically catalog-backed after shell split", () => {
  for (const pattern of [/One command bridge for every creator tool/, /space-logo-header\.png/, /space-logo-main\.png/, /model-rocket\.png/, /SPACEMOUNTAIN_SCENE/, /installProductBackdrop/, /bindProductRocketNavigation/, /Custom scene override/, /app\.iconUrl/, /this\.snapshot\.apps\.filter/]) assert.match(shellSource, pattern);
