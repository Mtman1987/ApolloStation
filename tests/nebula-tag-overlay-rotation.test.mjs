...
import { NEBULA_TAG_OVERLAY_CLIENT_JS, NebulaTagOverlayHttpAdapter, buildNebulaTagOverlaySnapshot, createNebulaTagState, executeNebulaTagCommand, planNebulaTagRotation } from "../apps/nebula-arcade/dist/index.js";
...
const apply = (state, value) => executeNebulaTagCommand(state, value).state;
...
test("rotation preserves the donor 40-minute active/offline behavior", () => {
  ...
  assert.equal(planNebulaTagRotation(state, { now: notDueAt, channelId: CHANNEL, liveUserIds: ["user-a"] }).reason, "not-due");
  const dueAt = new Date(Date.parse(state.lastTagAt ?? at(0)) + 40 * 60 * 1000).toISOString();
  const active = planNebulaTagRotation(state, { now: dueAt, channelId: CHANNEL, liveUserIds: ["user-a", "user-b"], random: () => 0 });
  ...
  const inactive = planNebulaTagRotation(state, { now: dueAt, channelId: CHANNEL, liveUserIds: [], random: () => 0 });
...
test("free-for-all reminder repeats on the donor 60-minute cadence", () => {
  ...
  const early = planNebulaTagRotation(state, { now: new Date(Date.parse(state.lastTagAt) + 59 * 60 * 1000).toISOString(), channelId: CHANNEL });
  assert.equal(early.reason, "not-due");
  const reminder = planNebulaTagRotation(state, { now: new Date(Date.parse(state.lastTagAt) + 60 * 60 * 1000).toISOString(), channelId: CHANNEL });
...
test("forced rotation prefers a live eligible player and restarts the timer", () => {
  ...
  const plan = planNebulaTagRotation(state, { now: forcedAt, channelId: CHANNEL, liveUserIds: ["user-b"], random: () => 0.9 });
...
test("overlay projection is viewer-scoped, crown-aware, and provider-secret free", () => {
  ...
  const snapshot = buildNebulaTagOverlaySnapshot(state, { viewerUserId: "user-a", generatedAt: at(4) });
...
test("visual overlay is controls-free and reads only the authenticated tenant principal", () => {
  let tenantA = createNebulaTagState("tenant-a");
  ...
  let tenantB = createNebulaTagState("tenant-b");
  ...
  const adapter = new NebulaTagOverlayHttpAdapter({ getState: (tenantId) => ({ revision: 7, state: states.get(tenantId) }) }, () => at(4));
  const principal = { schemaVersion: 1, tenantId: "tenant-a", appId: "nebula-arcade", widgetId: "tag", viewerUserId: "user-a" };

  assert.equal(adapter.handle({ method: "GET", path: "/v1/nebula-arcade/tag/overlay" }).status, 401);
  assert.equal(adapter.handle({ method: "GET", path: "/v1/nebula-arcade/tag/overlay" }, { ...principal, appId: "spoofed-app" }).status, 403);
  const page = adapter.handle({ method: "GET", path: "/v1/nebula-arcade/tag/overlay" }, principal);
  ...
  const state = adapter.handle({ method: "GET", path: "/v1/nebula-arcade/tag/overlay/state?tenantId=tenant-b" }, principal);
  assert.equal(state.status, 404);
  const exactState = adapter.handle({ method: "GET", path: "/v1/nebula-arcade/tag/overlay/state" }, principal);
  ...
  assert.match(NEBULA_TAG_OVERLAY_CLIENT_JS, /setInterval\(poll,1000\)/);
  assert.match(NEBULA_TAG_OVERLAY_CLIENT_JS, /leaderboard/);
  assert.match(NEBULA_TAG_OVERLAY_CLIENT_JS, /AudioContext/);
  assert.match(NEBULA_TAG_OVERLAY_CLIENT_JS, /confetti/);
});

function applyFor(tenantId, state, value) {
  assert.equal(value.tenantId, tenantId);
  return executeNebulaTagCommand(state, value).state;
}
