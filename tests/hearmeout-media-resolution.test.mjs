...
test('title and YouTube page requests choose a current capable worker and resolve actual media', async () => {
  ...
  assert.equal(item.playbackUrl, 'https://rr1.googlevideo.com/video'); assert.equal(item.metadata.videoId, 'abcdefghijk'); assert.equal(item.metadata.audioPlaybackUrl, 'https://rr1.googlevideo.com/audio');
  ...
  const movie = await resolver.resolve({ tenantId: 'tenant', query: 'https://www.youtube.com/watch?v=abcdefghijk', lane: 'movie' }); assert.equal(movie.playbackUrl, 'https://rr1.googlevideo.com/video'); assert.equal(movie.metadata.audioPlaybackUrl, 'https://rr1.googlevideo.com/audio');
...
test('room requests use the same resolver, replay once and preserve media after provider failure', async () => {
  ...
  try {
    ...
    const before = rooms.getSession('tenant', 'room', 'music'); assert.equal(before.current.item.source, 'youtube'); assert.equal(before.current.item.metadata.audioPlaybackUrl, 'https://rr1.googlevideo.com/audio'); assert.equal(before.queue.length, 0);
...
test('yt-dlp search treats the query as data and selects one video track plus one audio track', async () => {
