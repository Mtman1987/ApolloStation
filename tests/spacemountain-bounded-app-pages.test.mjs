...
const EXPECTED = [
  ["/apps/discord-stream-hub", "Discord Stream Hub", "theme-nebula-purple-background.webp"],
  ["/apps/hearmeout", "HearMeOut", "theme-oceanic-blue-background.webp"],
  ["/apps/mountainview", "MountainView", "theme-aurora-green-background.webp"],
  ["/apps/companion", "SpaceMountain Companion", "theme-solar-flare-background.webp"],
  ["/apps/streamweaver", "StreamWeaver", "stellar-core-background.webp"],
];
...
test("bounded donor app pages are explicit, themed, and honest about incomplete parity", () => {
  for (const [path, name, artwork] of EXPECTED) {
    ...
    assert.match(html, /parity remain active porting work|parity remain active porting work|parity work continues/);
