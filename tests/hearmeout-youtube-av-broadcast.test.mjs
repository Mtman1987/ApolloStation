...
test('separate YouTube video and audio inputs become one advancing music broadcast encoder without preflight probes',{timeout:30000},async()=>{
  const dir=await mkdtemp(join(tmpdir(),'hmo-youtube-av-broadcast-')),videoFile=join(dir,'video.mp4'),audioFile=join(dir,'audio.m4a');let server,rooms,broadcast;const diagnostics=[];
  try{
    ...
    // process.execPath is deliberately not ffprobe. The test can only pass if
    ...
    await waitFor(()=>Promise.resolve({video:videoReads.count,audio:audioReads.count}),counts=>counts.video>=1&&counts.audio>=1);
    ...
    assert.ok(diagnostics.some(value=>value.phase==='prepare'&&value.message.includes('without preflight probes')));
