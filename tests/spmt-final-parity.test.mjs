...
test("Commlink, notifications, signed webhooks and Athena metadata use scoped public operations",async(t)=>{
  ...
  f.service.auth.registerServiceIdentity({serviceId:appId,credential,scopes:["commlink:read","commlink:write","commlink:any","notifications:send","notifications:read","notifications:any","webhooks:read","webhooks:write","events:write","athena:context:read","athena:context:write","athena:context:any","athena:commands:read","athena:catalog:write"],tenantMode:"allow-list",tenantIds:[f.tenantId]});
  ...
  const command=await jsonRequest(`${f.base}/v1/athena/commands`,{method:"PUT",headers,body:JSON.stringify({id:"stream.status",title:"Stream status",description:"Reads stream status",requiredScopes:["stream:read"],availability:"unavailable",unavailableReason:"StreamWeaver Green is not migrated yet"})}); assert.equal(command.response.status,200); assert.equal(command.body.availability,"unavailable");
  const context=await jsonRequest(`${f.base}/v1/athena/context`,{method:"PUT",headers,body:JSON.stringify({userId:f.userId,kind:"summary",text:"Prefers concise stream summaries",tags:["preference"]})}); assert.equal(context.response.status,200);
  const contextList=await jsonRequest(`${f.base}/v1/athena/context?userId=${encodeURIComponent(f.userId)}`,{headers}); assert.equal(contextList.response.status,200); assert.equal(contextList.body[0].text,"Prefers concise stream summaries");
});
