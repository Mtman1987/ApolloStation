...
test("StreamWeaver exposes a wired Voice Commander, searchable bot catalog, integrations, persona and economy", async () => {
  await fixture(async ({ cookie, tenantId, spmtBase, streamBase }) => {
    ...
    assert.match(browserSource, /data-sw-form="avatar-build-upload"/); assert.match(browserSource, /accept="image\/png,image\/jpeg"/);
    assert.match(browserSource, /Portrait uploaded\. Starting the Meshy avatar build/); assert.match(browserSource, /portraitAssetId:asset\.id/);
    assert.match(browserSource, /Advanced avatar URLs/); assert.match(browserSource, /Choose picture and create avatar/);
    ...
    assert.equal(control.role, "owner"); assert.equal(control.operationMode, "read-only"); assert.equal(control.avatarBuildEnabled,true); assert.equal(control.connections[0].provider, "twitch"); assert.equal(control.botRuntime.publicCommands, "connected"); assert.equal(control.botRuntime.suiteActions, "partial"); assert.ok(control.botActions.length >= 20); assert.equal(control.botActions.find((action) => action.id === "sw.image.generate").availability, "connected"); assert.ok(control.botActions.some((action) => action.policy === "simulated")); assert.ok(control.botActions.every((action) => action.policy !== "blocked"));
...
test('owner check-in greeting controls persist and reject invalid voices and anonymous writes',async()=>{
 await fixture(async({cookie,streamBase})=>{const path=streamBase+'/api/streamweaver/control/stream-operations',headers={cookie,origin:streamBase,'content-type':'application/json'},body={action:'checkin-settings',enabled:true,aiEnabled:false,ttsEnabled:true,discordEnabled:false,greeting:'Welcome {user} to {partner}',prompt:'Be warm',voice:'deepgram:aura-2:athena'};assert.equal((await fetch(path,{method:'POST',headers,body:JSON.stringify(body)})).status,200);const state=await (await fetch(path,{headers:{cookie}})).json();assert.equal(state.checkinSettings.ttsEnabled,true);assert.notEqual((await fetch(path,{method:'POST',headers,body:JSON.stringify({...body,voice:'invalid'})})).status,200);assert.notEqual((await fetch(path,{method:'POST',headers:{origin:streamBase,'content-type':'application/json'},body:JSON.stringify(body)})).status,200);const {streamWeaverOperationsBrowserJs}=await import('../apps/streamweaver/dist/stream-operations-client.js');new Function(streamWeaverOperationsBrowserJs());});
});
