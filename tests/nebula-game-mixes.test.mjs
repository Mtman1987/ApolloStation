...
test("Nebula Game Mix keeps one stable source id while games and layout change", () => {
  ...
  try {
    ...
    const first = store.save("tenant-a", {
      ...
      mode: "simultaneous",
      layers: [
        { gameId: "chat-tag", x: 0, y: 0, width: 60, height: 100, opacity: 1, style: "full", zIndex: 2 },
    ...
    assert.deepEqual(first.layers.map((layer) => [layer.gameId, layer.zIndex]), [["quackverse", 0], ["chat-tag", 1]]);
    ...
    const updated = store.save("tenant-a", {
      ...
      mode: "activity",
      layers: [
        { gameId: "chat-tag", enabled: true, x: 0, y: 0, width: 100, height: 100, style: "minimal" },
    ...
    assert.deepEqual(updated.layers.map((layer) => layer.gameId), ["chat-tag", "bingo"]);
    assert.deepEqual(nebulaGameMixToLegacyScene(updated).layers.map((layer) => layer.gameId), ["chat-tag", "bingo"]);
    store.close();
  } finally {
  ...
  try {
    ...
    const manual = store.save("tenant-a", {
      ...
      layers: [
        ...
        { gameId: "chat-tag", enabled: true },
      ],
    });
    assert.equal(manual.activeGameId, "wordstorm");
    assert.throws(() => store.save("tenant-a", { id: "bad", name: "Bad", mode: "manual", activeGameId: "bingo", layers: [{ gameId: "chat-tag" }] }), /must be enabled/);
  } finally {
    store.close();
    rmSync(directory, { recursive: true, force: true });
  }
