...
test('HTTP guides expose all games, optional activity mixes persist, and opted-out actions are blocked',async()=>{
 ...
 await s.send('spmt wordchain stop',{roles:['moderator'],at:Date.now()+1});await s.send('spmt chatgarden stop',{roles:['moderator'],at:Date.now()+2});const stopped=await(await fetch(`${origin}/v1/nebula/game-mix-state?mix=public`)).json();assert.deepEqual(stopped.visibleGameIds,[]);
