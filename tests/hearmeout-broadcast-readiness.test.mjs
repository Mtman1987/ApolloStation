...
test('broadcast readiness waits for a real variant segment instead of sending viewers to an empty HLS feed',t=>{
  ...
  const key=createHash('sha256').update(JSON.stringify([session.tenantId,session.roomId,session.lane,'instance'])).digest('hex'),dir=join(root,key);mkdirSync(dir);
  ...
  assert.equal(broadcast.ready(session.tenantId,session.roomId,session.lane),false);
  writeFileSync(join(dir,'segment_000001.ts'),'media');
