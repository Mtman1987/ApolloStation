...
function dashboardState() {
  ...
  const player = (userId, username, score, tagsMade, timesTagged) => ({ userId, username, joinedAt: at(0), lastActiveAt: at(4), score, tagsMade, timesTagged, passCount: 0, sleeping: false, offline: false, timedImmunityUntil: null, noTagbackFromUserId: null });
...
test("Nebula dashboard ports the compact two-row embed, 20-game link, and webhook identity", () => {
  ...
  assert.equal(embed.image.url, "https://apollo.example/assets/nebula-arcade/games-showcase.gif");
...
test("animated Discord showcase contains all 20 slow frames at the donor's improved timing", () => {
  ...
  assert.equal(NEBULA_DISCORD_SHOWCASE_FRAME_DURATION_MS, 1_450);
  assert.equal(delays.length, 20);
  assert.deepEqual([...new Set(delays)], [1_450]);
...
test("live Discord Tag mutations publish the durable Nebula dashboard through injected gateway egress", async () => {
  ...
  const runtime = new NebulaArcadeProviderRuntime({ databasePath: path, config, client: { publishEvent: async () => ({}), awardXp: async () => ({}) }, egress: { send: async (message) => { replies.push(message); return { providerMessageId: "777771" }; } }, discordDashboard: { publicOrigin: "https://apollo.example", egress: { upsertDiscordDashboard: async (message) => { dashboards.push(message); return { providerMessageId: "888881", transport: "webhook" }; } } }, now: () => at(5) });
  try {
    await runtime.consumers[0].deliver({ schemaVersion: 1, deliveryId: "delivery-1", consumerId: "nebula.arcade.provider-ingress", attempts: 1, message: { schemaVersion: 1, tenantId: "tenant-a", provider: "discord", connectionId: "discord-main", channelId: "123456", messageId: "444441", text: "spmt join", occurredAt: at(1), actor: { providerUserId: "user-1", canonicalUserId: "alpha", username: "alpha", displayName: "Alpha", isBot: false, roles: ["member"] }, mentions: [] } });
