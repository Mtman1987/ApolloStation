...
test("Nebula chat scoring and game points use independent cooldowns", () => {
  ...
  let activity = recordNebulaGameChatActivity(state, { channel: "captain", userId: "123", username: "duck", message: "hello" }, start);
  assert.deepEqual(activity.scoredGameIds, ["wordstorm"]);
  assert.equal(activity.pointsAwarded, 1);
  activity = recordNebulaGameChatActivity(state, { channel: "captain", userId: "123", username: "duck", message: "again" }, start + NEBULA_GAME_SCORE_INTERVAL_MS - 1);
  assert.equal(activity.scoredGameIds.length, 0);
  activity = recordNebulaGameChatActivity(state, { channel: "captain", userId: "123", username: "duck", message: "score again" }, start + NEBULA_GAME_SCORE_INTERVAL_MS);
  assert.equal(activity.scoredGameIds.length, 1);
  assert.equal(activity.pointsAwarded, 0);
  activity = recordNebulaGameChatActivity(state, { channel: "captain", userId: "123", username: "duck", message: "points again" }, start + NEBULA_GAME_POINTS_INTERVAL_MS);
  assert.equal(activity.pointsAwarded, 1);
