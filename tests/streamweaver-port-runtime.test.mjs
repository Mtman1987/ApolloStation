...
test('a featured message survives chat eviction, restart and zero-duration hold',()=>{
  const dir=mkdtempSync(join(tmpdir(),'chat-hold-')),path=join(dir,'chat.sqlite');let now=1,store=new CommlinkOperatorStore(path,()=>now);
  try{const message={id:'old',text:'Keep this on stream',username:'Captain',provider:'twitch',channelId:'channel'};let state=store.apply('tenant',{action:'feature',revision:0,eventId:'old'},['old'],[message]);state=store.apply('tenant',{action:'settings',revision:state.revision,autoAdvance:true,durationSeconds:0,style:'minimal'},[]);store.close();now=1000000;store=new CommlinkOperatorStore(path,()=>now);state=store.read('tenant');assert.equal(state.featured,'old');assert.equal(state.snapshots.old.text,message.text);assert.equal(state.style,'minimal');state=store.apply('tenant',{action:'pin',eventId:'old',revision:state.revision},[]);assert.deepEqual(state.pinned,['old']);assert.throws(()=>store.apply('another',{action:'feature',eventId:'old',revision:0},[]),/outside/);}finally{store.close();rmSync(dir,{recursive:true,force:true})}
});
