...
- **D-32:** **Nebula Arcade is the sole current app, service, integration owner, and public product identity for the Games Hub.** The former ChatTag name is historical donor-repository evidence only. Tagging behavior is an internal Nebula Arcade game with canonical game ID `tag`; it is not a separately named app, service, worker, route, source app, overlay owner, or product. Quackverse, Bingo, Arena, Tag, and future games remain bounded modules behind Nebula Arcade contracts rather than one giant runtime.
...
| D-32 | Games Hub ownership and naming | Nebula Arcade is the sole current product/technical owner; retain the old donor name only in frozen provenance and one-way storage migration evidence. | Keeping a separately named original game would preserve donor recognition but would continue leaking obsolete app identity into contracts. | CHANGE |
...
## Change record — 2026-08-28 Nebula Arcade canonical ownership

- **Owner position:** The former ChatTag identity does not exist in the current ecosystem; any current code that points to it instead of Nebula Arcade is wrong.
- **Changed decision:** D-32 now makes `nebula-arcade` the only app, service, source-app, worker, route, environment, and overlay owner. The tagging mechanic remains available as Nebula Arcade game ID `tag`, without a separate product name.
